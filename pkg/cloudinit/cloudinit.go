...
func (g *Generator) generateUserData(config *Config) (string, error) {
	...
	// Appended literally, after rendering, so Go template syntax in it is text
	// rather than a template to execute.
	//
	// It is NOT escaped as a YAML scalar, unlike every other field: this is the
	// escape hatch for writing cloud-config by hand, so it joins the document as
	// cloud-config and can add runcmd, write_files and the rest. That is what it
	// is for, and it is the reason a manifest carries operator-level trust — the
	// content here runs as root in the guest at first boot. The q/doas escaping
	// elsewhere keeps ordinary fields from reaching that level by accident; it
	// does not, and cannot, contain this one.
	result := buf.String()
...
func (g *Generator) generateNetworkConfig(config *Config) (string, error) {
	// NetworkConfig is exported and Config.Networks is assignable, so a caller
	// that skips NetworkFromProviderSpec can hand over a zero value. Anything but
	// "dhcp" takes the static branch, and a zero value would render dhcp4:false
	// with an empty address: a guest with no address at all, failing only at
	// first boot with nothing to point at.
	for i, n := range config.Networks {
		if n.Name == "" {
			return "", fmt.Errorf("network %d: interface name is required", i)
		}
		switch n.Type {
		case "dhcp":
		case "static":
			if n.Address == "" {
				return "", fmt.Errorf("network %q: a static configuration needs an address", n.Name)
			}
		default:
			return "", fmt.Errorf("network %q: type must be \"dhcp\" or \"static\", got %q", n.Name, n.Type)
		}
	}
