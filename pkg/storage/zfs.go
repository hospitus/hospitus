...
func (z *ZFSBackend) CreateVolume(ctx context.Context, name string, opts VolumeOptions) (*Volume, error) {
	...
	if opts.CopyFrom != "" {
		...
		if mpErr != nil || mountpoint == "" || mountpoint == "none" || mountpoint == "legacy" {
			...
			// mpErr is nil on the common path — a zvol has no mountpoint at all,
			// and CopyFrom with Size lands here. Wrapping it then printed
			// "%!w(<nil>)" and left errors.Unwrap with nothing to return.
			err := fmt.Errorf("cannot copy content: dataset %s has no usable mountpoint (%q)", ds, mountpoint)
			if mpErr != nil {
				err = fmt.Errorf("cannot copy content: dataset %s has no usable mountpoint: %w", ds, mpErr)
			}
			return nil, &StorageError{Op: "create volume", Volume: name, Backend: "zfs", Err: err}
		}
...
func (z *ZFSBackend) DeleteVolume(ctx context.Context, name string, opts DeleteOptions) error {
	...
	// KeepSnapshots and Recursive contradict each other: "zfs destroy -r"
	// destroys every snapshot of the dataset, which is exactly what the caller
	// asked to keep. Refuse rather than silently destroy them.
	if opts.KeepSnapshots && opts.Recursive {
		return &StorageError{
			Op: "delete volume", Volume: name, Backend: "zfs",
			Err: fmt.Errorf("KeepSnapshots and Recursive cannot both be set: zfs destroy -r removes the snapshots"),
		}
	}
...
// volumeState reads the state from the properties already fetched, instead of
// answering "mounted" for everything — which was wrong for an unmounted
// filesystem and meaningless for a zvol, and disagreed with GetDataset, which
// reads the same property correctly.
func volumeState(props map[string]string) string {
...
func (z *ZFSBackend) listVolumesNoLock(ctx context.Context) ([]Volume, error) {
	// filesystem,volume: CreateVolume makes a zvol whenever a size is given,
	// which is exactly how VM disks are created. Listing only filesystems hid
	// every one of them, here and in ListClones, which reuses this.
	output, err := z.cmd().Output(ctx, "zfs", "list", "-H", "-r", "-t", "filesystem,volume",
...
//
// The property depends on the dataset type: a filesystem is bounded by quota,
// a zvol by volsize. Verified on OpenZFS 2.4.2 — "zfs set quota" on a volume
// answers "'quota' does not apply to datasets of this type", so resizing any
// VM disk failed.
func (z *ZFSBackend) ResizeVolume(ctx context.Context, name, newSize string) error {
	z.mu.Lock()
	ds := z.fullDatasetName(name)
	out, err := z.cmd().Output(ctx, "zfs", "get", "-H", "-o", "value", "type", ds)
	z.mu.Unlock()
	if err != nil {
		return &StorageError{Op: "resize volume", Volume: name, Backend: "zfs", Err: err}
	}

	if strings.TrimSpace(string(out)) == "volume" {
		z.mu.Lock()
		defer z.mu.Unlock()
		if out, err := z.cmd().CombinedOutput(ctx, "zfs", "set", "volsize="+newSize, ds); err != nil {
			return &StorageError{
				Op: "resize volume", Volume: name, Backend: "zfs",
...
func (z *ZFSBackend) ReplicateVolume(ctx context.Context, name, target string, opts ReplicationOptions) error {
	zfsOpts := SendReceiveOptions{
		Incremental: opts.IncrementalFrom,
...
func (z *ZFSBackend) listSnapshotsNoLock(ctx context.Context, volumeName string) ([]Snapshot, error) {
	...
	if err != nil {
		// A dataset that is not there has no snapshots; anything else — a
		// permission failure, a missing pool, a canceled context — must not be
		// reported as "none", because a retention caller acts on that answer.
		if !z.datasetExistsNoLock(ctx, ds) {
			return nil, nil
		}
		return nil, &StorageError{Op: "list snapshots", Volume: volumeName, Backend: "zfs", Err: err}
	}
...
// ensureOwnSnapshotName refuses a <dataset>@<snapshot> name whose dataset is
// outside the parent this backend manages, and whose snapshot component is not
// a plain name.
func (z *ZFSBackend) ensureOwnSnapshotName(name string) error {
	ds, snap, found := strings.Cut(name, "@")
	if !found || snap == "" {
		return &StorageError{
			Op: "clone", Backend: "zfs",
			Err: fmt.Errorf("refusing %q: not a snapshot name", name),
		}
	}
	if err := validation.ValidateSnapshotName(snap); err != nil {
		return &StorageError{
			Op: "clone", Backend: "zfs",
			Err: fmt.Errorf("refusing %q: %w", name, err),
		}
	}
	if ds != z.parentDataset && !strings.HasPrefix(ds, z.parentDataset+"/") {
		return &StorageError{
			Op: "clone", Backend: "zfs",
			Err: fmt.Errorf("refusing %q: %s is outside %s", name, ds, z.parentDataset),
		}
	}
	return nil
}
...
func (z *ZFSBackend) CreateClone(ctx context.Context, snapshotFullName, cloneName string, opts CloneOptions) (*Volume, error) {
	...
	// The source is caller-supplied. Without this it can name a snapshot of any
	// dataset on the pool, and its contents are cloned into the managed tree.
	if err := z.ensureOwnSnapshotName(snapshotFullName); err != nil {
		return nil, err
	}
...
func (z *ZFSBackend) getProperties(ctx context.Context, ds string) (map[string]string, error) {
	...
	for _, line := range lines {
		// "zfs get -H" separates its columns with tabs, and a value can hold a
		// space — a mountpoint under "/data/my volume", for one. Splitting on
		// whitespace would cut that value in half and store the first word.
...
func (z *ZFSBackend) GetDataset(ctx context.Context, name string) (*ZFSDataset, error) {
	...
	ds := &ZFSDataset{}
	ds.FullName = lines[0]
	// Name was left empty while FullName carried the whole path; a caller asking
	// for the volume name got nothing.
	if idx := strings.LastIndex(ds.FullName, "/"); idx >= 0 {
		ds.Name = ds.FullName[idx+1:]
	} else {
		ds.Name = ds.FullName
	}
	ds.Type = lines[1]
	// "zfs get origin" answers "-" for anything that is not a clone. Storing it
	// literally made every plain dataset look like a clone of a dataset called
	// "-"; getCloneOriginNoLock normalizes the same value.
	if lines[2] != "-" {
		ds.Origin = lines[2]
	}
...
func (z *ZFSBackend) SendReceive(ctx context.Context, source, destination string, opts SendReceiveOptions) error {
	// No lock around the transfer. A full replication runs for hours, and an
	// RWMutex queues later readers behind any waiting writer, so holding it
	// here stalls the whole backend — not just writers — for the duration.
	// Nothing in this function touches shared state; it only spawns commands.
	...
	var output, sendStderr bytes.Buffer
	recvCmd.Stdout = &output
	recvCmd.Stderr = &output
	// Without this the send side writes its diagnostics nowhere, and a failure
	// on that side is reported with the receive side's output instead.
	...
	if err := sendCmd.Start(); err != nil {
		// recvCmd is already running. cancel() kills it, but nothing would ever
		// reap it: the child would stay a zombie and its pipe descriptors held,
		// one pair leaked per failed call.
		cancel()
		_ = recvCmd.Wait()
	...
	if sendErr != nil {
		return fmt.Errorf("zfs send failed: %s: %w", strings.TrimSpace(sendStderr.String()), sendErr)
	}
	if recvErr != nil {
