...
func TestParseBool(t *testing.T) {
	...
	for _, tc := range tests {
		got := boolOr("test_key", tc.input, tc.def)
		if got != tc.want {
			t.Errorf("boolOr(%q, %v) = %v, want %v", tc.input, tc.def, got, tc.want)
		}
	}
}

// TestParseBoolReportsUnknownValues covers the flag boolOr needs to tell a
// configured "false" from a value it did not understand.
func TestParseBoolReportsUnknownValues(t *testing.T) {
	for _, in := range []string{"true", "yes", "1", "on", "enabled", "false", "no", "0", "off", "disabled", "TRUE"} {
		if _, ok := parseBool(in); !ok {
			t.Errorf("parseBool(%q) reported the value as unrecognized", in)
		}
	}
	for _, in := range []string{"", "maybe", "ture", "y", "2"} {
		if _, ok := parseBool(in); ok {
			t.Errorf("parseBool(%q) reported an unknown value as recognized", in)
		}
	}
}
