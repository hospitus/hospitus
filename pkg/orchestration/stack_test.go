...
func TestDeployStackSuccessful(t *testing.T) {
	...
	if len(created) != 2 {
		t.Fatalf("expected 2 creates, got %d: %v", len(created), created)
	}
	if len(started) != 2 {
		t.Fatalf("expected 2 starts, got %d: %v", len(started), started)
	}
	if len(stopped) != 0 {
		t.Fatalf("expected 0 stops (no rollback), got %d: %v", len(stopped), stopped)
	}
	if len(deleted) != 0 {
		t.Fatalf("expected 0 deletes (no rollback), got %d: %v", len(deleted), deleted)
	}
...
func TestDeployStackCreateInstanceFailsRollback(t *testing.T) {
	...
	created, started, stopped, deleted := mock.callRecords()
	if len(created) != 2 {
		// Fatal, not Errorf: the assertions below index into this slice, so a
		// short one panicked instead of reporting the length it actually had.
		t.Fatalf("expected 2 creates (app succeeded, web attempted), got %d: %v", len(created), created)
	}
	...
	if len(started) != 1 {
		// Fatal, not Errorf: the assertions below index into this slice, so a
		// short one panicked instead of reporting the length it actually had.
		t.Fatalf("expected 1 start (app only), got %d: %v", len(started), started)
	}
	...
	if len(stopped) != 1 {
		t.Fatalf("expected 1 stop (rollback of app), got %d: %v", len(stopped), stopped)
	}
	if stopped[0] != "teststack_app" {
		t.Errorf("expected app stopped in rollback, got %s", stopped[0])
	}
	if len(deleted) != 1 {
		t.Fatalf("expected 1 delete (rollback of app), got %d: %v", len(deleted), deleted)
	}
...
func TestDeployStackStartInstanceFailsRollback(t *testing.T) {
	...
	// Both were created and app was started, but web failed to start → roll back both
	created, started, stopped, deleted := mock.callRecords()
	if len(created) != 2 {
		t.Fatalf("expected 2 creates, got %d: %v", len(created), created)
	}
	if len(started) != 2 {
		t.Fatalf("expected 2 start attempts (app succeeded, web failed), got %d: %v", len(started), started)
	}
	// Rollback: stop and delete both (app was already started, web was just created)
	if len(stopped) != 2 {
		t.Fatalf("expected 2 stops (rollback), got %d: %v", len(stopped), stopped)
	}
	if len(deleted) != 2 {
		t.Fatalf("expected 2 deletes (rollback), got %d: %v", len(deleted), deleted)
	}
}
...
func TestDeployStackRollbackStopFails(t *testing.T) {
	...
	created, started, stopped, deleted := mock.callRecords()
	if len(stopped) != 1 {
		t.Fatalf("expected 1 stop attempt (even though it failed), got %d: %v", len(stopped), stopped)
	}
	if len(deleted) != 1 {
		t.Fatalf("expected 1 delete (should proceed after stop failure), got %d: %v", len(deleted), deleted)
	}
	if deleted[0] != "teststack_app" {
		t.Errorf("expected app deleted in rollback, got %s", deleted[0])
	}
...
func TestDeployStackRollbackDeleteFails(t *testing.T) {
	...
	created, started, stopped, deleted := mock.callRecords()
	if len(stopped) != 1 {
		t.Fatalf("expected 1 stop, got %d: %v", len(stopped), stopped)
	}
	if stopped[0] != "teststack_app" {
		t.Errorf("expected app stopped in rollback, got %s", stopped[0])
	}
	if len(deleted) != 1 {
		t.Fatalf("expected 1 delete attempt (even though it failed), got %d: %v", len(deleted), deleted)
	}
	if deleted[0] != "teststack_app" {
		t.Errorf("expected app deleted in rollback, got %s", deleted[0])
	}
...
func TestDeployStackDependencyFailureRollback(t *testing.T) {
	...
	if len(started) != 3 {
		t.Fatalf("expected 3 start attempts, got %d: %v", len(started), started)
	}
	// C failed to start → all three should be rolled back
	if len(stopped) != 3 {
		t.Fatalf("expected 3 stops (rollback all), got %d: %v", len(stopped), stopped)
	}
	if len(deleted) != 3 {
		t.Fatalf("expected 3 deletes (rollback all), got %d: %v", len(deleted), deleted)
	}
...
func TestRollbackCreatedInstances(t *testing.T) {
	...
	created, started, stopped, deleted := mock.callRecords()
	if len(created) != 0 {
		t.Fatalf("expected 0 creates (rollback only), got %d", len(created))
	}
	if len(started) != 0 {
		t.Fatalf("expected 0 starts, got %d", len(started))
	}
	if len(stopped) != 3 {
		t.Fatalf("expected 3 stops, got %d: %v", len(stopped), stopped)
	}
	if len(deleted) != 3 {
		t.Fatalf("expected 3 deletes, got %d: %v", len(deleted), deleted)
	}
}
...
func TestRollbackCreatedInstancesStopFailsDeleteProceeds(t *testing.T) {
	...
	if len(stopped) != 1 {
		t.Fatalf("expected 1 stop attempt, got %d: %v", len(stopped), stopped)
	}
	if len(deleted) != 1 {
		t.Fatalf("expected 1 delete attempt (proceeds after stop failure), got %d: %v", len(deleted), deleted)
	}
}
...
func TestDeployStackMultipleFailuresRollback(t *testing.T) {
	...
	if len(started) != 2 {
		t.Fatalf("expected 2 starts (a, b succeeded), got %d: %v", len(started), started)
	}
	// a and b were created and started; they should both be rolled back
	if len(stopped) != 2 {
		t.Fatalf("expected 2 stops (rollback of a, b), got %d: %v", len(stopped), stopped)
	}
	if len(deleted) != 2 {
		t.Fatalf("expected 2 deletes (rollback of a, b), got %d: %v", len(deleted), deleted)
	}
