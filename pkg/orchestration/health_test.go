...
func TestHealthChecker_OnHealthChange(t *testing.T) {
	hc := NewHealthChecker(nil)
	called := false
	hc.OnHealthChange(func(id string, old, new HealthStatus) {
		called = true
	})
	// Callbacks registered without panicking; called flag can't be tested
	// without actually running a check — just confirm no panic
	_ = called
}
