...
func (c *restClient) clientFor(socketPath string) *http.Client {
	return &http.Client{
		...
		Transport: &http.Transport{
			...
			// One client per request, and nothing ever calls
			// CloseIdleConnections: keeping the connection alive left a socket
			// and its descriptor behind on every state poll.
