...
type HookResult struct {
	...
	// Fatal reports whether this failure should stop the operation. A hook that
	// failed always carries Error; Fatal says what the caller does about it.
...
func (p *JailProvider) runDiscoveredHook(ctx context.Context, hookType HookType, scriptPath string, env HookEnv) HookResult {
	// executeHook reports Type from env, and buildHookEnv exports the same field
	// as HOSPITUS_HOOK_TYPE. The rejection path below reports the argument, so
	// without this the two disagree for the same hook — and the script sees a
	// type that is not the one it was run for.
...
func (p *JailProvider) executeHook(ctx context.Context, scriptPath string, env HookEnv, timeout time.Duration, failOnError bool) HookResult {
	...
	if err != nil {
		...
		// Always recorded. failOnError says whether the *caller* aborts, not
		// whether the failure happened: discarding it here made a failed
		// configured post-hook return a result that looks like a success.
...
// hookIsFatal reports whether a failing hook of this type should stop the
// operation.
//
// A post hook runs after the thing it observes has already happened — the jail
// is created, started, stopped or destroyed — so failing the call would report
// an operation that in fact succeeded. Its error is still recorded in the
// result, which is what makes the failure visible.
func hookIsFatal(hookType HookType) bool {
	switch hookType {
	case HookPostCreate, HookPostStart, HookPostStop, HookPostDestroy:
		return false
	}
	return true
}

// validateHookType accepts only the hook types this provider defines. The value
// becomes a file name, so anything else is a path this code did not choose.
func validateHookType(hookType HookType) error {
	switch hookType {
...
func (p *JailProvider) CreateHookScript(jailName string, hookType HookType, content string) error {
	// Validate hook type
	if err := validateHookType(hookType); err != nil {
		return err
	}

	// Before MkdirAll, not after: the confinement check below refuses the path,
	// but the directory has already been created by then. jailName="../../tmp/x"
	// leaves /tmp/x behind on a call that reports an error.
...
func (p *JailProvider) DeleteHookScript(jailName string, hookType HookType) error {
	...
	// The type becomes a file name. CreateHookScript checks it against the known
	// set; without the same check here a traversing value removes a .sh file
