...
func TestFormatPrometheusMetrics(t *testing.T) {
	...
	if !containsString(output, "# HELP hospitus_jail_info") {
		t.Error("Missing hospitus_jail_info HELP")
	}
	if !containsString(output, "# TYPE hospitus_jail_info gauge") {
		t.Error("Missing hospitus_jail_info TYPE")
	}
	if !containsString(output, "hospitus_jail_info{name=\"web\"") {
		t.Error("Missing web jail info")
	}
	if !containsString(output, "hospitus_jail_info{name=\"db\"") {
		t.Error("Missing db jail info")
	}

	// Check CPU metrics
	if !containsString(output, "# HELP hospitus_jail_cpu_percent") {
		t.Error("Missing CPU percent HELP")
	}
	if !containsString(output, "hospitus_jail_cpu_percent{name=\"web\"} 10.50") {
		t.Error("Missing or incorrect web CPU percent")
	}

	// Check memory metrics
	if !containsString(output, "# HELP hospitus_jail_memory_bytes") {
		t.Error("Missing memory bytes HELP")
	}
	if !containsString(output, "hospitus_jail_memory_bytes{name=\"web\"} 268435456") {
		t.Error("Missing or incorrect web memory bytes")
	}

	// Check memory limit metrics
	if !containsString(output, "hospitus_jail_memory_limit_bytes{name=\"web\"} 536870912") {
		t.Error("Missing or incorrect web memory limit")
	}

	// Check disk metrics
	if !containsString(output, "# HELP hospitus_jail_disk_used_bytes") {
		t.Error("Missing disk used HELP")
	}
	if !containsString(output, "hospitus_jail_disk_used_bytes{name=\"web\"} 5368709120") {
		t.Error("Missing or incorrect web disk used")
	}

	// Check uptime metrics
	if !containsString(output, "# HELP hospitus_jail_uptime_seconds") {
		t.Error("Missing uptime HELP")
	}
	if !containsString(output, "hospitus_jail_uptime_seconds{name=\"web\"} 3600") {
		t.Error("Missing or incorrect web uptime")
	}

	// Check process count
	if !containsString(output, "hospitus_jail_processes{name=\"web\"} 5") {
...
func TestFormatPrometheusMetricsNetwork(t *testing.T) {
	...
	if !containsString(output, "hospitus_jail_network_receive_bytes_total{name=\"web\"} 1073741824") {
		t.Error("Missing or incorrect network rx bytes")
	}
	if !containsString(output, "hospitus_jail_network_transmit_bytes_total{name=\"web\"} 536870912") {
		t.Error("Missing or incorrect network tx bytes")
	}
	if !containsString(output, "hospitus_jail_network_receive_packets_total{name=\"web\"} 1000000") {
		t.Error("Missing or incorrect network rx packets")
	}
	if !containsString(output, "hospitus_jail_network_transmit_packets_total{name=\"web\"} 500000") {
...
func TestFormatPrometheusMetricsIO(t *testing.T) {
	...
	if !containsString(output, "hospitus_jail_io_read_bytes_per_second{name=\"db\"} 104857600") {
		t.Error("Missing or incorrect read BPS")
	}
	if !containsString(output, "hospitus_jail_io_write_bytes_per_second{name=\"db\"} 52428800") {
		t.Error("Missing or incorrect write BPS")
	}
	if !containsString(output, "hospitus_jail_io_read_ops_per_second{name=\"db\"} 10000") {
		t.Error("Missing or incorrect read IOPS")
	}
	if !containsString(output, "hospitus_jail_io_write_ops_per_second{name=\"db\"} 5000") {
		t.Error("Missing or incorrect write IOPS")
	}
	if !containsString(output, "hospitus_jail_open_files{name=\"db\"} 256") {
