...
func TestDetachDiskStopped(t *testing.T) {
	...
	// The effect, not just the absence of an error: a silent no-op passed.
	fstab, err := os.ReadFile(filepath.Join(p.stateDir, "fstab", "web"))
	if err != nil && !os.IsNotExist(err) {
		t.Fatalf("reading the jail fstab: %v", err)
	}
	if strings.Contains(string(fstab), "d1") {
		t.Errorf("the fstab still carries the detached disk:\n%s", fstab)
	}
}
