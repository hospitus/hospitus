...
func (p *JailProvider) collectNetworkMetrics(ctx context.Context, name string, metrics *JailMetrics) {
	// "epair" and "vnet0" are not interface names. This provider puts epairNb in
	// the jail and may rename it to whatever the network was configured with, so
	// ask for every interface and let the parser pick the ones that carry
	// traffic. netstat -I <name> on a name that does not exist simply fails, and
	// the counters were never updated.
	output, err := p.cmd().Output(ctx, "jexec", name, "netstat", "-b", "-n", "-i")
	if err != nil {
		return
	}
	...
	for i, line := range lines {
		...
		// "netstat -i" lists one line per interface plus one per address; the
		// address lines repeat the same counters, so counting them would double
		// every figure. Take the link-layer line only, and skip the loopback.
		if fields[0] == "lo0" || !strings.HasPrefix(fields[2], "<Link") {
			continue
		}
