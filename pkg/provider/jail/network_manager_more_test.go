...
func TestGetGatewayIPFromSpecificPool(t *testing.T) {
	...
	// The exact address, not merely a non-empty one: returning the network
	// address or the broadcast would pass an emptiness check.
	if gw != "10.20.0.1/24" {
		t.Errorf("gateway = %q, want 10.20.0.1/24", gw)
	}
}
