...
// was not.
// foreignArch returns an architecture this machine is not, so a cross-arch
// case is genuinely cross-arch wherever the tests run.
...
func TestRuntimeTypeFollowsTheImage(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			// claims the image. The architecture is chosen at run time: riscv64
			// is not foreign on a riscv64 builder, where determineRuntimeType
			// answers "freebsd" and this case would fail for the wrong reason.
			name: "foreign architecture base set",
			spec: provider.InstanceSpec{Image: "freebsd-14.3-RELEASE-" + foreignArch()},
			want: "crossarch",
		},
	}
