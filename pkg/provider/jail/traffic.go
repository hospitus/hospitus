...
func (p *JailProvider) GetTrafficStats(ctx context.Context, handle provider.InstanceHandle) ([]TrafficStats, error) {
	...
	for _, iface := range interfaces {
		...
		// The name comes from another command's output and becomes an argument
		// to netstat inside the jail.
		if err := validation.ValidateInterfaceName(iface); err != nil {
			p.logWarn(ctx, "skipping an interface whose name is not usable",
				"jail", jailName, "interface", iface, logging.FieldError, err)
			continue
		}
