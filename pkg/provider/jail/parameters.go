...
type JailParameters struct {
	...
	// The three below are not jail(8) parameters — the manual documents
	// mount.fstab, mount.devfs, mount.fdescfs and mount.procfs only, and these
	// exist there as allow.mount.* permissions. They are Hospitus switches:
	// StartInstance prepares and performs the mounts itself when one is set,
	// which is why ToJailArgs does not emit them.
	MountLinprocfs bool `json:"mount.linprocfs"`
	MountLinsysfs  bool `json:"mount.linsysfs"`
	MountTmpfs     bool `json:"mount.tmpfs"`
...
func ApplyJailParameterMap(base JailParameters, m map[string]interface{}) (JailParameters, error) {
	...
	for key, value := range m {
		...
		case "exec.consolelog":
			val := toString(value)
			if val != "" {
				// ToJailArgs checks this field with ValidateFilePath; using a
				// different validator here let a value pass one gate and fail
				// the other, on a field that is a path, not a command.
				if err := validation.ValidateFilePath(val, true); err != nil {
		...
		// These three are Hospitus switches, not jail(8) parameters: the start
		// path performs the mounts rather than passing them to jail(8).
		...
		case "fdescfs":
			// Stored but never read: ToJailArgs does not emit it, and the Linux
			// mount setup uses a fixed /dev/fd target. Setting it did nothing.
			return JailParameters{}, fmt.Errorf("fdescfs is not configurable; " +
				"use mount.fdescfs to mount it, at the fixed /dev/fd target")
...
func toInt(v interface{}) int {
	...
	case float64:
		return int(val)
	case string:
		// Sscanf stops at the first non-digit and reports no error, so "10x"
		// still yielded 10 — a securelevel the caller never asked for, which
		// then validates fine. ParseInt refuses the whole string instead.
