...
func (p *JailProvider) GetConsole(ctx context.Context, handle provider.InstanceHandle) (provider.ConsoleConnection, error) {
	...
	//
	// The environment is set explicitly: exec.Cmd reads a nil Env as "inherit
	// the parent's", which would hand hospitusd's own environment — API keys,
	// credentials — to a shell inside the jail.
...
func (p *JailProvider) ExecCommand(ctx context.Context, handle provider.InstanceHandle, opts provider.ExecOptions) (*provider.ExecResult, error) {
	...
	// Get exit code
	if err != nil {
		// A deadline kills the process, so Run returns an ExitError like any
		// other death by signal. Reporting that as the command's own exit code
		// tells the caller the command ran and failed, when in fact it was cut
		// off and its work may be half-done.
...
func (p *JailProvider) ExecInteractive(ctx context.Context, handle provider.InstanceHandle, opts provider.ExecOptions) error {
	...
	// The timeout is validated above but was never applied: the command ran on
	// the caller's context, so "Timeout: 1" bounded nothing unless that context
	// happened to carry a deadline of its own.
	if opts.Timeout > 0 {
		var cancel context.CancelFunc
...
func (p *JailProvider) ExecCommandStream(ctx context.Context, handle provider.InstanceHandle, opts provider.ExecOptions, stdout, stderr io.Writer) (int, error) {
	...
	exitCode := 0
	if err != nil {
		// Same reason as ExecCommand: a killed process is an ExitError, and
		// passing its code up hides the deadline.
		if ctxErr := execCtx.Err(); errors.Is(ctxErr, context.DeadlineExceeded) {
			return -1, fmt.Errorf("command timed out after %ds", opts.Timeout)
		} else if ctxErr != nil {
			return -1, fmt.Errorf("command canceled: %w", ctxErr)
		}
		var exitError *exec.ExitError
		if errors.As(err, &exitError) {
...
func buildJexecArgs(jailName string, opts provider.ExecOptions, interactive bool) ([]string, error) {
	...
	case opts.WorkingDir != "":
		...
		// Absolute only: jexec starts at the jail root, so a relative path is
		// resolved against a directory the caller did not choose and cannot see.
		if !filepath.IsAbs(opts.WorkingDir) {
			return nil, fmt.Errorf("working directory %q must be absolute: it is resolved inside the jail", opts.WorkingDir)
		}
		...
		if opts.Command == "" && interactive {
			...
			// Quoted like the arguments below: ValidateExecCommand rejects shell
			// operators but allows spaces, "*" and "?", and /bin/sh -c would
			// field-split and glob-expand an unquoted command — changing which
			// executable runs and with what.
			quotedCmd, qErr := shellQuote(opts.Command)
			if qErr != nil {
				return nil, fmt.Errorf("cannot quote command: %w", qErr)
			}
			cmdStr = quotedCmd
...
func applyExecEnv(cmd *exec.Cmd, opts provider.ExecOptions) error {
	...
	//
	// Returning early when the caller sets no variables left cmd.Env nil, and
	// exec.Cmd reads nil as "inherit the parent's environ" — which is exactly
	// what this function exists to prevent, and the common case at that.
