...
func TestGetIPPools(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// A default case with neither the variable nor a setting falls
			// through to LoadConfigWithDefaults, which reads the host's
			// /usr/local/etc/hospitus/hospitusd.conf — so the result would
			// depend on the machine running the tests. Pin it.
			t.Setenv("HOSPITUS_IP_POOL", "")
			os.Unsetenv("HOSPITUS_IP_POOL")

			if tt.envVar != "" {
				// t.Setenv restores the previous value rather than unsetting,
				// and fails a parallel test instead of racing it.
				t.Setenv("HOSPITUS_IP_POOL", tt.envVar)
			}
...
func TestMultiplePoolFallback(t *testing.T) {
	...
	if err := p.saveJailConfig(config1, filepath.Join(tempDir, "test1.json")); err != nil {
	...
	if err := p.saveJailConfig(config2, filepath.Join(tempDir, "test2.json")); err != nil {
		t.Fatalf("saveJailConfig: %v", err)
	}
