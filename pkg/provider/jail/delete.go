...
func (p *JailProvider) DeleteInstance(ctx context.Context, handle provider.InstanceHandle, force bool) (err error) {
	...
	// Post-destroy hooks are non-fatal, so ExecuteHooks answers nil while the
	// individual results still carry their errors. Reading them only on a
	// non-nil error made every one of those failures silent.
	results, hookErr := p.ExecuteHooks(ctx, HookPostDestroy, postHookEnv)
	if hookErr != nil {
		p.logWarn(ctx, "post-destroy hooks reported an error", "jail", jailName, logging.FieldError, hookErr)
	}
