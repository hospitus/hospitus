	...
	return &execx.Fake{Func: func(name string, args []string) ([]byte, error) {
		...
		case "jexec":
			...
			if len(args) >= 2 && args[1] == "netstat" {
				// The shape "netstat -b -n -i" actually prints on FreeBSD: one
				// link line per interface, then one line per address repeating
				// the same counters, plus loopback. Counting the address lines
				// would double every figure.
				return []byte("Name Mtu Network Address Ipkts Ierrs Idrop Ibytes Opkts Oerrs Obytes Coll\n" +
					"epair0b 1500 <Link#4> 02:ff:00:00:00:01 10 0 0 5000 20 0 8000 0\n" +
					"epair0b - 10.0.0.0/24 10.0.0.2 10 - - 5000 20 - 8000 -\n" +
					"lo0 16384 <Link#3> lo0 99 0 0 9999 99 0 9999 0\n"), nil
			}
		case "zfs":
			return []byte("1048576\n"), nil
		}
