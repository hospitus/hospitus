...
func (p *JailProvider) DeleteSnapshot(ctx context.Context, snapshot provider.SnapshotHandle) error {
	...
	// SECURITY: the name is read back from a stored handle, so it is checked
	// against the datasets this provider owns rather than trusted.
...
func (p *JailProvider) RestoreSnapshot(ctx context.Context, handle provider.InstanceHandle, snapshot provider.SnapshotHandle) error {
	...
	// SECURITY: `rollback -r` destroys every later snapshot of the dataset it
	// names, so the name is checked against the datasets we own.
	if err := p.ensureOwnSnapshot(zfsName); err != nil {
		return err
	}
