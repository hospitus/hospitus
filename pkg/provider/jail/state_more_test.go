...
func TestGetInstanceStateNotFound(t *testing.T) {
	...
	_, err := p.GetInstanceState(context.Background(), provider.InstanceHandle{ID: "ghost"})
	if !errors.Is(err, provider.ErrInstanceNotFound) {
		t.Fatalf("GetInstanceState on a missing jail = %v, want ErrInstanceNotFound", err)
	}
}
...
func TestGetInstanceMetricsRunning(t *testing.T) {
	p, _ := runningProvider(t, "web", func(cmd string, args []string) ([]byte, error) {
		...
		if cmd == "netstat" {
			// The shape "netstat -bin" actually prints: a link line carrying the
			// counters, then an address line repeating them.
			return []byte("Name Mtu Network Address Ipkts Ierrs Idrop Ibytes Opkts Oerrs Obytes Coll\n" +
				"epair0a 1500 <Link#4> 02:ff:00:00:00:0a 1 0 0 4096 2 0 8192 0\n" +
				"epair0a - 10.0.0.0/24 10.0.0.2 1 - - 4096 2 - 8192 -\n"), nil
		}
		return nil, nil
	})
