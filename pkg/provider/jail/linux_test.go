...
func TestLinuxReleasesBaseURLsReachable(t *testing.T) {
	// https only: validateMirrorScheme refuses anything else, so accepting
	// http:// here would let an entry through that the provider then rejects.
	for key, release := range linuxReleases {
		if !strings.HasPrefix(release.BaseURL, "https://") {
			t.Errorf("linuxReleases[%q].BaseURL = %q, must start with https://", key, release.BaseURL)
		}
	}
}
