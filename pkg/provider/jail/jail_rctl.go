...
func isValidRctlAction(action string) bool {
	validActions := map[string]bool{
		"deny":   true, // Deny resource allocation
		"log":    true, // Log when limit is reached
		"devctl": true, // Send devctl notification
		"sig":    true, // Send signal (followed by signal name)
	}
...
func (p *JailProvider) SetCPUPriority(ctx context.Context, handle provider.InstanceHandle, priority int) error {
	...
	// Set priority for all processes using renice
	// renice -n <priority> -p <pid1> <pid2> ...
	args := []string{"-n", fmt.Sprintf("%d", priority), "-p"}
	args = append(args, pids...)

	reniceOutput, err := p.cmd().CombinedOutput(ctx, "renice", args...)
	if err != nil {
		return fmt.Errorf("failed to set CPU priority: %w (output: %s)", err, string(reniceOutput))
	}
...
func (p *JailProvider) GetCPUPriority(ctx context.Context, handle provider.InstanceHandle) (int, error) {
	...
	// Get first process in jail (init process)
	output, err := p.cmd().CombinedOutput(ctx, "jexec", jailName, "ps", "-ax", "-o", "pid=", "|", "head", "-1")
	if err != nil {
		return 0, fmt.Errorf("failed to get jail init process: %w", err)
	}
