...
func (p *JailProvider) copyResolvConf(jailPath string) error {
	...
	// 0644, as the other writer already uses: an unprivileged process inside the
	// jail has to read this file, and 0600 owned by root leaves it unable to
	// resolve anything.
	if err := os.WriteFile(dst, data, 0o644); err != nil { //nolint:gosec // G306: jail resolv.conf is intentionally world-readable
...
// debootstrapArch maps a hospitus architecture onto the name debootstrap uses.
func debootstrapArch(arch string) (string, error) {
	switch arch {
	case "", "amd64", "x86_64":
		return "amd64", nil
	case "arm64", "aarch64":
	...
	case "armv7", "arm":
		return "armhf", nil
	default:
		return "", fmt.Errorf("architecture %q is not supported by debootstrap", arch)
	}
}
...
func (p *JailProvider) debootstrapBase(ctx context.Context, jailPath string, osRelease *OSRelease, codenames map[string]string, defaultMirror string, keyrings []string) error {
	...
	// osRelease.Arch carries what the caller asked for; hardcoding amd64 built an
	// amd64 rootfs for an arm64 request while create.go installed the arm64
	// emulator alongside it.
	debArch, err := debootstrapArch(osRelease.Arch)
	if err != nil {
		return err
	}
...
func (p *JailProvider) installRHELBase(ctx context.Context, jailPath string, osRelease *OSRelease) error {
	...
	// Signature checking stays on, as the debootstrap path already insists on an
	// archive keyring: without it the whole base system is installed from the
	// network unverified.
	if osRelease.GPGKey == "" {
		return fmt.Errorf("no package-signing key is known for %s; refusing to install the base system "+
			"without signature verification", osRelease.Version)
	}
...
gpgcheck=1
gpgkey=%s
`, repoName, osRelease.BaseURL, osRelease.GPGKey)
...
func (p *JailProvider) installAlpineBase(ctx context.Context, jailPath string, osRelease *OSRelease) error {
	...
	// Alpine publishes "<tarball>.sha256" beside each release: the digest, two
	// spaces, the file name. Verify before extracting as root, exactly as the
	// FreeBSD base system is verified against its MANIFEST.
	if err := p.verifyAlpineDigest(ctx, tmpDir, tarballPath, downloadURL); err != nil {
		return err
	}
...
// verifyAlpineDigest checks a downloaded minirootfs against the .sha256 file
// the mirror publishes next to it.
func (p *JailProvider) verifyAlpineDigest(ctx context.Context, tmpDir, tarballPath, downloadURL string) error {
	sumPath := filepath.Join(tmpDir, "alpine-minirootfs.tar.gz.sha256")
	if out, err := p.cmd().CombinedOutput(ctx, "fetch", "-o", sumPath, downloadURL+".sha256"); err != nil {
		return fmt.Errorf("cannot fetch %s.sha256 to verify the minirootfs: %w (output: %s)",
			downloadURL, err, strings.TrimSpace(string(out)))
	}
	data, err := os.ReadFile(sumPath) //nolint:gosec // G304: path built from our own temp dir
	if err != nil {
		return fmt.Errorf("cannot read the fetched checksum: %w", err)
	}
	fields := strings.Fields(string(data))
	if len(fields) == 0 {
		return fmt.Errorf("%s.sha256 is empty", downloadURL)
	}
	return verifyArchiveDigest(tarballPath, fields[0])
}
