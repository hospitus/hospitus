...
// firewall returns the firewall manager under the lock that initFirewall
// writes it with. Reading the field directly raced with that write, and could
// observe a manager that was not finished publishing.
func (nm *NetworkManager) firewall() *firewall.Manager {
	nm.firewallMu.Lock()
	defer nm.firewallMu.Unlock()
	return nm.firewallMgr
}
...
func (nm *NetworkManager) initFirewall(ctx context.Context) error {
	...
	// Direct read: the lock is already held here, and firewall() would take it
	// again on a non-reentrant mutex.
...
func (nm *NetworkManager) CleanupFirewallRules(ctx context.Context, jailName string) error {
	...
	return nm.firewall().RemoveAllRules(ctx, jailName)
}
...
func (nm *NetworkManager) EnsureBridge(ctx context.Context, bridgeName, ipPool string) error {
	// Every value below becomes an ifconfig argument.
	if err := validation.ValidateBridgeName(bridgeName); err != nil {
		return fmt.Errorf("invalid bridge name %q: %w", bridgeName, err)
	}
...
func (nm *NetworkManager) ConfigureNAT(ctx context.Context, jailName, ipAddress string) error {
	...
	_, err := fw.SetupNAT(ctx, jailName, "jail", network, extIf)
	if err != nil {
...
func (nm *NetworkManager) ConfigureGateway(ctx context.Context, jailPath, gateway string) error {
	...
	// Read the file directly: the write below already uses os.OpenFile, so
	// spawning test(1) and grep(1) to answer the same question adds two
	// processes and a dependency on host binaries for nothing.
	if existing, err := os.ReadFile(rcConfPath); err == nil { //nolint:gosec // G304: path is built from the jail root
		if strings.Contains(string(existing), "defaultrouter") {
			nm.logger.Debug("gateway already configured; skipping", "gateway", gateway, "rc_conf", rcConfPath)
			return nil
		}
	} else if !errors.Is(err, fs.ErrNotExist) {
		return fmt.Errorf("failed to read rc.conf: %w", err)
	}
...
func (nm *NetworkManager) ConfigureDNS(jailPath, fallbackForLoopback string) error {
	...
	if info, err := os.Lstat(jailResolvConf); err == nil && info.Mode()&os.ModeSymlink != 0 {
		// A warning is not enough here: the write below follows the link, so a
		// rootfs whose /etc/resolv.conf points at an absolute path would have
		// hospitusd overwrite the host's own file.
		if err := os.Remove(jailResolvConf); err != nil {
			return fmt.Errorf("jail resolv.conf %s is a symlink that cannot be removed; "+
				"refusing to write through it: %w", jailResolvConf, err)
		}
	...
	//
	// O_NOFOLLOW closes the window between the check above and this write: a
	// symlink appearing in between is refused rather than followed.
	f, err := os.OpenFile(jailResolvConf, os.O_WRONLY|os.O_CREATE|os.O_TRUNC|syscall.O_NOFOLLOW, 0o644) //nolint:gosec // G302/G304: jail resolv.conf is intentionally world-readable
	if err != nil {
		return fmt.Errorf("failed to write jail resolv.conf: %w", err)
	}
	if _, err := f.WriteString(jailContent); err != nil {
		_ = f.Close()
		return fmt.Errorf("failed to write jail resolv.conf: %w", err)
	}
...
func (nm *NetworkManager) ApplyPortForwards(ctx context.Context, jailName string) error {
	...
	mappings, err := fw.ListExposedPorts(ctx, jailName)
	if err != nil {
	...
	for i := range mappings {
		...
		if err := fw.Backend().AddPortMapping(ctx, *m); err != nil {
...
func (nm *NetworkManager) attachInterfaceToBridge(ctx context.Context, ifname string, spec provider.NetworkSpec) error {
	...
	// Both become ifconfig arguments, and spec arrives from the caller:
	// CreateVNetInterface forwards it without a name check of its own.
	if err := validation.ValidateBridgeName(bridge); err != nil {
		return fmt.Errorf("invalid bridge name %q: %w", bridge, err)
	}
	if err := validation.ValidateInterfaceName(ifname); err != nil {
		return fmt.Errorf("invalid interface name %q: %w", ifname, err)
	}
	if spec.VLAN < 0 || spec.VLAN > 4094 {
		return fmt.Errorf("invalid VLAN %d: expected 0..4094", spec.VLAN)
	}
...
func (nm *NetworkManager) ConfigureVNetJailNetwork(ctx context.Context, jailName, bridgeName, ifname, ipAddress, runtimeType string, withDefaultRoute bool) error {
	...
	// The bridge is optional here — EnableVNET treats it that way — and
	// ValidateBridgeName rejects an empty string, so checking it unconditionally
	// refused before the interface was ever configured.
	if bridgeName != "" {
		if err := validation.ValidateBridgeName(bridgeName); err != nil {
			return fmt.Errorf("invalid bridge name %q: %w", bridgeName, err)
		}
	...
	if ipAddress != "" && ipAddress != "dhcp" {
		addr := ipAddress
		if i := strings.Index(addr, "/"); i >= 0 {
			addr = addr[:i]
		}
		if err := validation.ValidateIPAddress(addr); err != nil {
			return fmt.Errorf("invalid address %q: %w", ipAddress, err)
		}
	}
...
func (nm *NetworkManager) EnsureGatewayOnBridge(ctx context.Context, bridgeName, gateway, jailIP string) error {
	...
	netmask := getMaskFromBits(maskBits)
	if netmask == "" {
		return fmt.Errorf("cannot express a /%d prefix as an IPv4 netmask (jail address %q); "+
			"the bridge gateway alias is IPv4 only", maskBits, jailIP)
	}

	// Add gateway IP as alias on bridge
	output, err := nm.cmd().CombinedOutput(ctx, "ifconfig", bridgeName, "inet", gateway, "netmask", netmask, "alias")
	if err != nil {
...
//
// net.CIDRMask answers nil outside 0..32, and indexing that panics. An IPv6
// jail address reaches here with 64 bits from Mask.Size(), which is exactly the
// case EnableIPv6 makes ordinary.
