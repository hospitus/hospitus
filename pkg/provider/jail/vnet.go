...
func (p *JailProvider) EnableVNET(ctx context.Context, handle provider.InstanceHandle, config VNETConfig) error {
	...
	//
	// "dhcp" is allowed past validation above but is not something ifconfig
	// understands: passing it made the command fail, the epair get destroyed and
	// the whole call return an error. EnableVNET configures a static address;
