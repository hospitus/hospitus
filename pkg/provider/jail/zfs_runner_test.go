...
// TestZFSHelpersRefuseForeignDatasets covers the create side of the guard the
// destroy side already had: "zfs create -p" on an arbitrary path is as
// unwelcome as a destroy, and the name reaches these helpers from handle
// metadata a client can influence.
func TestZFSHelpersRefuseForeignDatasets(t *testing.T) {
	p, _ := runningProvider(t, "web", nil)

	foreign := []string{"zroot/home", "tank/other/thing", "zroot", "-oexec=1", "zroot/hospitus/jails/../../home"}
	for _, ds := range foreign {
		if err := p.createZFSDataset(context.Background(), ds); err == nil {
			t.Errorf("createZFSDataset(%q) was accepted", ds)
		}
		if err := p.ensureZFSDataset(context.Background(), ds); err == nil {
			t.Errorf("ensureZFSDataset(%q) was accepted", ds)
		}
	}

	// The parents themselves stay creatable: that is what ensure is for.
	if err := p.ensureZFSDataset(context.Background(), p.zfsParent); err != nil {
		t.Errorf("ensureZFSDataset on the parent was refused: %v", err)
	}
}

// TestZfsDestroyCleanupRefusesForeignNames covers the cleanup path, which had
// no confinement at all: the name arrives from handle metadata a client can
// influence, and "cleanup" is no reason to destroy someone else's dataset.
func TestZfsDestroyCleanupRefusesForeignNames(t *testing.T) {
	p, fake := runningProvider(t, "web", nil)

	for _, ds := range []string{"zroot/home", "tank/other@snap", "zroot", "-r"} {
		p.zfsDestroyCleanup(context.Background(), ds)
	}
	for _, c := range fake.Snapshot() {
		if c.Name == "zfs" && len(c.Args) > 0 && c.Args[0] == "destroy" {
			t.Errorf("a destroy was issued for a foreign name: %v", c.Args)
		}
	}

	// Our own snapshots stay destroyable: that is what this path is for.
	p.zfsDestroyCleanup(context.Background(), p.zfsParent+"/web@snap")
	found := false
	for _, c := range fake.Snapshot() {
		if c.Name == "zfs" && len(c.Args) > 1 && c.Args[0] == "destroy" {
			found = true
		}
	}
	if !found {
		t.Error("a snapshot under our own parent was refused")
	}
}
