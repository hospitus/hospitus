...
// TestConfigureVNetJailNetworkWritesTheDefaultRoute covers the branch both
// other VNET tests skip: they set DefaultGateway to "none", so the host-side
// route call and the bridge gateway alias were never exercised.
func TestConfigureVNetJailNetworkWritesTheDefaultRoute(t *testing.T) {
	fake := &execx.Fake{Func: func(_ string, _ []string) ([]byte, error) { return nil, nil }}
	nm := newNMWithFake(fake, &config.Config{DefaultGateway: "10.0.0.1", BridgePrefix: "hospitus"})

	if err := nm.ConfigureVNetJailNetwork(context.Background(), "web", "hospitus0",
		"epair0b", "10.0.0.5/24", "freebsd", true); err != nil {
		t.Fatalf("ConfigureVNetJailNetwork err = %v", err)
	}

	var sawRoute bool
	for _, c := range fake.Snapshot() {
		if c.Name == "route" || (c.Name == "jexec" && len(c.Args) > 1 && c.Args[1] == "route") {
			sawRoute = true
		}
	}
	if !sawRoute {
		t.Errorf("no default route was installed: %v", fake.Snapshot())
	}
}
