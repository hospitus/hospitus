...
func (p *JailProvider) CreateVolume(ctx context.Context, name string, opts VolumeCreateOptions) (*Volume, error) {
	...
	if !validation.ValidZFSCompression(compression) {
		return nil, fmt.Errorf("invalid compression %q: expected one of off, on, lz4, zle, lzjb, gzip, gzip-1..9, zstd, zstd-1..19, zstd-fast or zstd-fast-N (N in 1-10, 20, 30 ... 100, 500, 1000)", compression)
	}
