...
// current call chain already holds. The owner is part of the key: two providers
// each holding their own InstanceLocks can lock the same name, and a marker
// keyed on the name alone would make one of them skip serialization entirely.
...
func (l *InstanceLocks) Acquire(ctx context.Context, name string) (context.Context, func(), error) {
	if ctx.Value(heldLocks{l, name}) != nil {
		return ctx, func() {}, nil
	}
	...
	// A select with both cases ready picks at random, so a caller that has
	// already given up would acquire the lock about half the time.
	...
	return context.WithValue(ctx, heldLocks{l, name}, struct{}{}), release, nil
}
