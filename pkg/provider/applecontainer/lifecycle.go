...
func (p *Provider) CreateInstance(ctx context.Context, spec provider.InstanceSpec) (_ provider.InstanceHandle, err error) {
	...
	// Only a genuine absence means "go ahead and create". A list that failed to
	// run, or output that failed to decode, says nothing about whether the
	// container exists — treating it as absent creates a duplicate.
	if _, findErr := p.find(ctx, spec.Name); findErr == nil {
		return provider.InstanceHandle{}, provider.ErrInstanceExists
	} else if !errors.Is(findErr, provider.ErrInstanceNotFound) {
		return provider.InstanceHandle{}, fmt.Errorf("checking for an existing container: %w", findErr)
	}
...
func commandArgs(providerConfig map[string]interface{}) ([]string, error) {
	// []interface{} is what a decoded JSON body carries; []string is what Go
	// code building a spec passes. Accepting only the first silently dropped
	// the command a caller had asked for, and any other type did the same.
	value, present := providerConfig["command"]
	if !present {
		return nil, nil
	}
	var raw []interface{}
	...
	default:
		return nil, fmt.Errorf("command must be a list of strings, got %T", value)
	}
