...
func (p *QEMUProvider) GetInstanceMetrics(ctx context.Context, handle provider.InstanceHandle) (provider.Metrics, error) {
	...
	// Left at zero when the reading fails. Falling back to the configured total
	// reported every stopped VM at a hundred percent of its memory — and
	// getProcessMemory now refuses whenever the recorded PID is no longer this
	// VM's QEMU, which is the ordinary case for a VM that is not running.
	if memUsedMB, err := p.getProcessMemory(ctx, vmName); err == nil && memUsedMB > 0 {
		metrics.MemoryUsedMB = memUsedMB
	} else if err != nil {
		p.logger.Debug("memory usage unavailable", "vm", vmName, logging.FieldError, err)
	}
	...
	if netRx, netTx, err := p.getNetStats(ctx, vmName); err == nil {
		...
		// Left at zero, like the memory path above, but not in silence: a VM
		// on user-mode networking has no host-visible counter at all, and a
		// torn-down tap reads the same way from here.
		p.logger.Debug("network counters unavailable", "vm", vmName, logging.FieldError, err)
	}
...
func (p *QEMUProvider) getNetStats(ctx context.Context, vmName string) (rxBytes, txBytes int64, err error) {
	...
	// Every counter, or none. The totals are a sum across the VM's taps, so a
	// tap that does not answer makes the sum smaller than the last one — which
	// the caller reads as a counter reset, not as the missing reading it is.
	// Tracking "at least one succeeded" was the same bug one interface wider.
	var readErr error
	for _, ifname := range ifaces {
		rx, rerr := readIfaceStat(ifname, "rx_bytes")
		if rerr != nil {
			readErr = errors.Join(readErr, rerr)
			continue
		}
		tx, terr := readIfaceStat(ifname, "tx_bytes")
		if terr != nil {
			readErr = errors.Join(readErr, terr)
			continue
		}
		rxBytes += rx
		txBytes += tx
	...
	if readErr != nil {
		return 0, 0, fmt.Errorf("incomplete interface counters for VM %s: %w", vmName, readErr)
	}
...
func (p *QEMUProvider) getProcessCPU(ctx context.Context, vmName string) (float64, error) {
	...
	// Once QEMU exits the kernel hands its number to whatever starts next, and
	// this would then report an unrelated host process as the VM's usage.
	if !p.pidBelongsToVM(ctx, pid, vmName) {
		return 0, fmt.Errorf("pid %d is no longer the QEMU process for %s", pid, vmName)
	}
...
func (p *QEMUProvider) getProcessMemory(ctx context.Context, vmName string) (int64, error) {
	...
	if !p.pidBelongsToVM(ctx, pid, vmName) { // see getProcessCPU
		return 0, fmt.Errorf("pid %d is no longer the QEMU process for %s", pid, vmName)
	}
