...
func TestQEMUProviderCapabilities(t *testing.T) {
	...
	config := provider.ProviderConfig{
		DataDir:  "/tmp/hospitus-test/qemu/data",
		StateDir: "/tmp/hospitus-test/qemu/state",
	}
...
func TestQEMUProviderInitialize(t *testing.T) {
	// Check if running as root (required for creating temp directories)
	if os.Getuid() != 0 {
...
func TestBuildQEMUConfigWithKVM(t *testing.T) {
	...
	config := p.buildQEMUConfig(spec, "qemu-system-x86_64", "", "", "/tmp/vm", runtime.GOARCH)
...
func TestBuildQEMUConfigCrossArchFallsBackToEmulation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			config := p.buildQEMUConfig(spec, "qemu-system-"+foreign, "", "", "/tmp/vm", foreign)
...
func TestReadPID(t *testing.T) {
	...
	t.Run("valid PID file", func(t *testing.T) {
		os.WriteFile(filepath.Join(tmpDir, "test-vm.pid"), []byte("12345\n"), 0o644)
	...
	t.Run("invalid PID content", func(t *testing.T) {
		os.WriteFile(filepath.Join(tmpDir, "bad-vm.pid"), []byte("notanumber\n"), 0o644)
	...
	t.Run("zero PID", func(t *testing.T) {
		os.WriteFile(filepath.Join(tmpDir, "zero-vm.pid"), []byte("0\n"), 0o644)
	...
	t.Run("negative PID", func(t *testing.T) {
		os.WriteFile(filepath.Join(tmpDir, "neg-vm.pid"), []byte("-1\n"), 0o644)
...
func TestRebuildArgsWithMedia(t *testing.T) {
	...
	t.Run("eject existing CDROM", func(t *testing.T) {
		args := []string{"-m", "2048", "-drive", "file=old.iso,format=raw,media=cdrom,readonly=on", "-boot", "d"}
		result := p.rebuildArgsWithMedia(args, "", false)

		for _, arg := range result {
			if arg == "-drive" || arg == "-boot" {
				t.Errorf("CDROM drive and boot args should be removed, found: %s", arg)
			}
		}
	})
}
