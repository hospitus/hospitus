...
func TestBuildQEMUConfigDiskFormat(t *testing.T) {
	...
	for _, c := range cases {
		...
		// The -drive naming this disk, not any -drive: buildQEMUConfig emits
		// raw UEFI drives before the data drive, so a joined search let one of
		// those satisfy every raw case even when the data drive was wrong.
		var drive string
		for _, arg := range config.Args {
			if strings.Contains(arg, "file="+c.diskPath+",") || strings.HasSuffix(arg, "file="+c.diskPath) {
				drive = arg
				break
			}
		}
		if drive == "" {
			t.Errorf("diskPath %q: no -drive names it, got: %s", c.diskPath, strings.Join(config.Args, " "))
			continue
		}
		if !strings.Contains(drive, c.wantFormat) {
			t.Errorf("diskPath %q: expected %q on its own drive, got: %s", c.diskPath, c.wantFormat, drive)
		}
	}
}
