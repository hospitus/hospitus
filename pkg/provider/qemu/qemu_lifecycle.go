...
func (p *QEMUProvider) StartInstance(ctx context.Context, handle provider.InstanceHandle) (err error) {
	...
	// Build command - QEMU with -daemonize will fork and exit
	cmd := exec.CommandContext(ctx, vmConfig.QEMUBin, vmConfig.Args...)

	// Capture stderr for error messages
	var stderr strings.Builder
	cmd.Stderr = &stderr

	// Start QEMU process (with -daemonize, this will exit quickly after forking)
	if err := cmd.Run(); err != nil {
		errMsg := stderr.String()
		if errMsg != "" {
			return provider.NewProviderError("qemu", "start", vmName,
				fmt.Errorf("failed to start QEMU: %w\nQEMU error: %s", err, errMsg))
		}
		return provider.NewProviderError("qemu", "start", vmName,
...
func (p *QEMUProvider) AttachNetwork(ctx context.Context, handle provider.InstanceHandle, network provider.NetworkAttachment) error {
	...
	if network.Network.Bridge != "" {
		...
		netdevProps["ifname"] = fmt.Sprintf("tap-%s", network.Network.ID[:8])
...
func (p *QEMUProvider) RenameInstance(ctx context.Context, handle provider.InstanceHandle, newName string) error {
	...
	if iso, ok := config.Spec.ProviderConfig["install_iso"].(string); ok && strings.HasPrefix(iso, oldPrefix) {
		config.Spec.ProviderConfig["install_iso"] = newPrefix + strings.TrimPrefix(iso, oldPrefix)
	}
