...
func makeAddr(ipType, ip string, prefix int) GuestIPAddress {
	return GuestIPAddress{IPAddressType: ipType, IPAddress: ip, Prefix: prefix}
}
...
func TestFilterGuestIPs_ExcludesLoopbackInterface(t *testing.T) {
	ifaces := []GuestNetworkInterface{
		{
			Name:        "lo",
			IPAddresses: []GuestIPAddress{makeAddr("ipv4", "127.0.0.1", 8)},
		},
		{
			Name:        "eth0",
			IPAddresses: []GuestIPAddress{makeAddr("ipv4", "192.168.1.100", 24)},
		},
	}
...
func TestFilterGuestIPs_MultipleInterfaces(t *testing.T) {
	ifaces := []GuestNetworkInterface{
		...
		{
			Name:        "eth1",
			IPAddresses: []GuestIPAddress{makeAddr("ipv6", "2001:db8::1", 64)},
		},
	}
