...
func TestSSHPortForKeepsTheVMsOwnPort(t *testing.T) {
	...
	writeVMState(t, stateDir, "ubuntu-server", 2224)

	if got := p.sshPortFor("ubuntu-server"); got != 2224 {
		t.Errorf("sshPortFor = %d, want the recorded 2224", got)
	}
}
...
func TestRefreshSSHForwardRewritesAPortAnotherVMHolds(t *testing.T) {
	...
	if _, ok := storedPort(config, "ssh_port"); !ok {
		t.Error("the new port was not recorded in the spec")
	}
}
