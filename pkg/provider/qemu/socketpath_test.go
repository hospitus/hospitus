...
func TestCheckSocketPathLimit(t *testing.T) {
	tests := []struct {
		...
		{"at the limit", "/" + strings.Repeat("a", maxUnixSocketPath-len("/"+longestSocketName)-2), false},
		// The first length that must fail: checkSocketPathLimit counts the NUL
		// terminator, so a 104-byte path is already one too long. The case used
		// to start one byte further out and never exercised the boundary.
		{"one past the limit", "/" + strings.Repeat("a", maxUnixSocketPath-len("/"+longestSocketName)-1), true},
