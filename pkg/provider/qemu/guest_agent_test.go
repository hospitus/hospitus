...
func TestQEMUOffersAGuestAgentChannel(t *testing.T) {
	// The shared helper, like the rest of the package: a bare literal leaves
	// stateDir, imageDir and the runner at their zero values, so the provider
	// under test is not the one the other tests exercise.
