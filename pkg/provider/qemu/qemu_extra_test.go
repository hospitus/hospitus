...
func TestRebuildArgsWithMedia_InsertNew(t *testing.T) {
	...
	// The order= form, which is what rebuildArgsWithBootOrder writes: the
	// legacy letter alongside it would leave the VM with two -boot options.
	bootFound := false
	for i, arg := range result {
	...
	if !bootFound {
		t.Errorf("a cdrom-first boot order should be added when inserting an ISO: %v", result)
	}
}
...
// TestAllocateVNCPort_Concurrent covers what the callers of allocatePort do,
// not the primitive on its own.
//
// allocatePort reads the state files to learn what is taken, so it is only
// unique when the allocation and the config that records it happen together —
// which is why every caller brackets the pair with hostPortMu. Calling it bare,
// as this test used to, handed the same port to all ten goroutines and proved
// nothing about the invariant the lock exists for.
func TestAllocateVNCPort_Concurrent(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		go func() {
			...
			p.hostPortMu.Lock()
			port := p.allocatePort(name, "vnc_port", vncBasePort)
			cfg := &vmConfig{Spec: provider.InstanceSpec{
				Name:           name,
				ProviderConfig: map[string]interface{}{"vnc_port": port},
			}}
			_ = p.saveVMConfig(cfg, filepath.Join(p.stateDir, name+".json"))
			p.hostPortMu.Unlock()
			ports[i] = port
			done <- struct{}{}
		}()
	...
	// Distinct, not merely valid: an allocator handing the same port to every
	// VM satisfied "at least 5900", and the second QEMU would then fail to bind
	// its display.
	...
	for i, port := range ports {
		...
		if first, dup := seen[port]; dup {
			t.Errorf("vm%d and vm%d were both given port %d", first, i, port)
		}
		seen[port] = i
	}
}
...
// TestEjectKeepsTheRestOfTheBootOrder covers the letter test that was reading
// the word "order".
//
// rebuildArgsWithBootOrder writes "-boot order=c", and a Contains check for "d"
// matched it — the "d" of "order" — so ejecting media stripped the boot order
// from every VM written in the modern form, including one already booting from
// disk.
func TestEjectKeepsTheRestOfTheBootOrder(t *testing.T) {
	p := newTestProvider(t)
	for _, tc := range []struct {
		name string
		in   []string
		want string // the -boot value expected afterwards, "" for none
	}{
		{"disk only, modern form", []string{"-boot", "order=c"}, "order=c"},
		{"cdrom first, modern form", []string{"-boot", "order=dc"}, "order=c"},
		{"cdrom only, modern form", []string{"-boot", "order=d"}, ""},
		{"cdrom only, legacy form", []string{"-boot", "d"}, ""},
		{"network then disk", []string{"-boot", "order=nc"}, "order=nc"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			got := p.rebuildArgsWithMedia(tc.in, "", false)
			value := ""
			for i, a := range got {
				if a == "-boot" && i+1 < len(got) {
					value = got[i+1]
				}
			}
			if value != tc.want {
				t.Errorf("boot value = %q, want %q (args %v)", value, tc.want, got)
			}
		})
	}
}
