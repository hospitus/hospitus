...
func (p *QEMUProvider) AttachDisk(ctx context.Context, handle provider.InstanceHandle, disk provider.DiskAttachment) error {
	...
	// Every type Capabilities advertises, mapped to the format driver qemu
	// knows it by. Sending "raw" for a VHD or a VMDK makes qemu read the
	// container's own metadata as guest sectors, and the guest's writes then
	// corrupt the image.
	driver, err := qemuBlockDriver(disk.Disk.Type)
	if err != nil {
		return err
	}
...
func (p *QEMUProvider) createDiskImage(ctx context.Context, path string, sizeGB int, diskType provider.DiskType, backingFile string) error {
	...
	if backingFile != "" {
		...
		backingFormat, err := p.detectImageFormat(ctx, backingFile)
		if err != nil {
			return err
		}
...
// qemuBlockDriver maps a disk type to the qemu format driver for it.
func qemuBlockDriver(t provider.DiskType) (string, error) {
	switch t {
	case provider.DiskTypeQCOW2:
		return "qcow2", nil
	case provider.DiskTypeVHD:
	...
	case provider.DiskTypeVMDK:
		return "vmdk", nil
	case provider.DiskTypeRaw, provider.DiskTypeZVOL, provider.DiskTypePhysical, "":
		return "raw", nil
	default:
		return "", fmt.Errorf("no qemu format driver for disk type %q", t)
	}
}

// detectImageFormat reports the format qemu-img sees in an image file.
//
// It does not guess. Naming a qcow2 file as raw hands the guest the image's own
// metadata as if it were disk contents, and writes through it corrupt the
// image; "-F" is what stops qemu from probing, so a wrong answer here is worse
// than none.
func (p *QEMUProvider) detectImageFormat(ctx context.Context, imagePath string) (string, error) {
	output, err := p.cmd().Output(ctx, "qemu-img", "info", "--output=json", imagePath)
	if err != nil {
		return "", fmt.Errorf("cannot determine the format of %s: %w", imagePath, err)
	}
	...
	if err := json.Unmarshal(output, &info); err != nil {
		return "", fmt.Errorf("cannot read qemu-img output for %s: %w", imagePath, err)
	}
	if info.Format == "" {
		return "", fmt.Errorf("qemu-img reported no format for %s", imagePath)
	}
