...
func TestRemovePortForwardRejectsBadProtocol(t *testing.T) {
	p := fakeProvider(t, &execx.Fake{})
	// Persisted, so the protocol check is the only thing that can fail: without
	// a config on disk the method also errors on the load, and the assertion
	// could not tell the two apart — it would keep passing if the protocol
	// check were removed.
	persistEmptyVM(t, p, "web")

	err := p.RemovePortForward(context.Background(), provider.InstanceHandle{ID: "web"}, "icmp", 80)
	if err == nil {
