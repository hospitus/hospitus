...
func (p *QEMUProvider) Initialize(ctx context.Context, config provider.ProviderConfig) error {
	...
	case "freebsd":
		...
		p.hasNVMM = p.cmd().Run(ctx, "kldstat", "-q", "-m", "nvmm") == nil
	case "darwin":
		// HVF (Hypervisor.framework) on macOS
		// Check if sysctl returns success for HVF capability
		output, err := p.cmd().Output(ctx, "sysctl", "-n", "kern.hv_support")
...
func (p *QEMUProvider) allocatePort(vmName, key string, base int, reserved ...int) int {
	...
	if err == nil {
		for _, entry := range entries {
			...
			taken[port] = true
			// A host port another VM publishes is taken too, whatever key it
			// was recorded under: scanning only "ssh_port" or "vnc_port" let
			// this hand out a port a port-forward already claims, and both
			// VMs then fail to bind at start.
			for _, pf := range portForwardsFromConfig(config.Spec.ProviderConfig) {
				taken[pf.HostPort] = true
			}
		}
	}
...
func (p *QEMUProvider) saveVMConfig(config *vmConfig, path string) error {
	...
	// The rename gives readers atomicity; it does not give durability. Without
	// this, a crash after the rename can leave the new name pointing at a file
	// whose contents never reached the disk — an empty config for a VM that
	// exists.
...
func (p *QEMUProvider) DeleteSnapshot(ctx context.Context, snapshot provider.SnapshotHandle) error {
	name, diskPath, err := p.snapshotTargetFor(snapshot)
	if err != nil {
		return err
	}
	...
	// Delete the snapshot from the disk the resolver bound to this instance.
...
func (p *QEMUProvider) RestoreSnapshot(ctx context.Context, handle provider.InstanceHandle, snapshot provider.SnapshotHandle) error {
	...
	// Overwrites the disk with the snapshot contents, on the disk the resolver
...
func (p *QEMUProvider) refreshUserForwards(config *vmConfig) bool {
	...
	for i, arg := range config.Args {
		...
		// A forward to guest 22 belongs to refreshSSHForward — but only when
		// there is an ssh_port for it to refresh. A VM created with an explicit
		// network records none, so this function owns that rule instead: it
		// both keeps the existing field and re-adds it below, which put the
		// same forward on the command line twice on every refresh.
		_, hasSSHPort := storedPort(config, "ssh_port")

		fields := strings.Split(arg, ",")
		kept := make([]string, 0, len(fields)+len(forwards))
		for _, field := range fields {
			if strings.HasPrefix(field, "hostfwd=") {
				// Kept only when refreshSSHForward will maintain it; every
				// other forward is rebuilt from the config below.
				if !hasSSHPort || !sshHostForward.MatchString(field) {
					continue
				}
			}
		...
		for _, pf := range forwards {
			if pf.GuestPort == 22 && pf.Protocol == "tcp" && hasSSHPort {
