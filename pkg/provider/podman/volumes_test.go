...
func TestVolumeAllowlistResolvesSymlinks(t *testing.T) {
	// The allowed root is a real directory this test owns, so the symlink below
	// sits *inside* it and only its resolved target puts it out of bounds.
	// Against the production root the link was already outside the allowlist,
	// and the case passed without the resolution ever being exercised.
	root := t.TempDir()
	target := t.TempDir()
	...
	if err := checkPodmanVolumeUnder(inside+":/srv", root); err != nil {
		t.Fatalf("a directory under the allowed root was refused: %v", err)
	}
	...
	if err := checkPodmanVolumeUnder(link+":/host", root); err == nil {
