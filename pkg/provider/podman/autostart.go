...
func (p *PodmanProvider) SetAutoStart(ctx context.Context, handle provider.InstanceHandle, config provider.AutoStartConfig) error {
	...
	// 0 is reserved for "not specified" — it is the zero value an absent JSON
	// field leaves behind — so the usable range starts at 1, as the contract in
	// pkg/provider says.
	if config.Priority < 0 || config.Priority > 100 {
		return fmt.Errorf("priority must be between 1 and 100, got %d", config.Priority)
	}

	// Normalized whether or not auto-start is enabled: a record that keeps 0
	// reads back as the highest priority the day it is enabled.
