...
func TestValidateExportPath(t *testing.T) {
	...
	tests := []struct {
		...
		// The archive is a file: "podman export -o <a directory>" fails anyway,
		// and the old prefix test accepted it only by accident.
		{name: "data dir itself", path: "/var/lib/hospitus", wantErr: true},
