...
func TestGetDetailedMetricsPopulatesInterfaces(t *testing.T) {
	...
	fake := &execx.Fake{Func: func(name string, args []string) ([]byte, error) {
		...
		// The two ps invocations ask for different columns, and
		// commandIsBhyveVM inspects the first field of what follows them. One
		// line cannot satisfy both: the previous fixture matched neither, so
		// the process fields stayed at zero and the assertions below passed on
		// those zeros.
		switch {
		case name == "ps" && slices.Contains(args, "pid,pcpu,command"):
			return []byte("4242 5.0 bhyve: web\n"), nil
		case name == "ps" && slices.Contains(args, "pid,state,time,command"):
			return []byte("4242 R 0:30.00 bhyve: web\n"), nil
		}
		return nil, nil
	}}
	p := metricsProvider(t, fake, 4242)
	...
	// The fields the ps output is there to fill. Left unchecked, a fixture that
	// matches nothing looks exactly like one that matches.
	if info.PID != 4242 {
		t.Errorf("PID = %d, want 4242", info.PID)
	}
	if info.ProcessState != "R" {
		t.Errorf("ProcessState = %q, want R", info.ProcessState)
	}
	if info.CPUTime != 30 {
		t.Errorf("CPUTime = %v, want 30", info.CPUTime)
	}
}
