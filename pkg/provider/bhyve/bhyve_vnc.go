...
func (p *BhyveProvider) EnableVNC(ctx context.Context, handle provider.InstanceHandle, port, width, height int, host string, wait bool) error {
	...
	// read and the write — StartInstance assigning MACs, a resize — had its
	// change overwritten by whichever finished last.
	// The derived context is discarded: what follows is file work that takes none.
