	...
	tests := []struct {
		...
		},
		{
			...
			config: &vmConfig{
				...
				VNCHeight:  1080,
				VNCHost:    "0.0.0.0",
				// The opt-in a bind to every interface requires: without it,
				// validateRuntimeConfig refuses the configuration, which is the
				// policy this fixture would otherwise quietly bypass.
				VNCInsecure: true,
				VNCWait:     false,
			},
			checks: []func(t *testing.T, args []string){
...
func TestBuildBhyveArgsPCISlotAssignment(t *testing.T) {
	...
	if _, err := p.buildBhyveArgs(config); err != nil {
