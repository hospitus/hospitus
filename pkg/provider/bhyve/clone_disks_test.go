...
func cloningProvider(t *testing.T) (*BhyveProvider, *execx.Fake) {
	t.Helper()
	dir := t.TempDir()
	p := &BhyveProvider{dataDir: dir, stateDir: dir, zfsParent: "zroot/hospitus/bhyve"}
...
func TestCloneDisksClonesEveryVolume(t *testing.T) {
	...
	source := []provider.DiskSpec{
		{Type: provider.DiskTypeZVOL, Path: "/dev/zvol/zroot/hospitus/bhyve/web/disk0", SizeGB: 20},
		{Type: provider.DiskTypeZVOL, Path: "/dev/zvol/zroot/hospitus/bhyve/web/disk1", SizeGB: 50},
		{Type: provider.DiskTypeZVOL, Path: "/dev/zvol/zroot/hospitus/bhyve/web/disk2", SizeGB: 10},
	}
	...
	for i := range dest {
		want := "/dev/zvol/zroot/hospitus/bhyve/web2/disk" + string(rune('0'+i))
		if dest[i].Path != want {
	...
	for i := 0; i < 3; i++ {
		target := "zroot/hospitus/bhyve/web2/disk" + string(rune('0'+i))
...
func TestCloneDisksKeepsSnapshotsOnlyForALinkedClone(t *testing.T) {
	source := []provider.DiskSpec{{Type: provider.DiskTypeZVOL, Path: "/dev/zvol/zroot/hospitus/bhyve/web/disk0", SizeGB: 20}}
...
func TestCloneRefusesAPhysicalDisk(t *testing.T) {
	...
	stoppedVM(t, p, "win", []string{"/dev/zvol/zroot/hospitus/bhyve/win/disk0", "/dev/nda0"})
