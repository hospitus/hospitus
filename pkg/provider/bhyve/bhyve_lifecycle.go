...
func (p *BhyveProvider) CreateInstance(ctx context.Context, spec provider.InstanceSpec) (handle provider.InstanceHandle, err error) {
	...
	if spec.Image != "" && spec.Image != "none" {
		// Image format can be "type:reference" or just "reference"
		imageType := ""
	...
	for i, disk := range spec.Disks {
		...
		case provider.DiskTypeZVOL:
			...
			if disk.Path != "" && p.zvolSizeGB(ctx, strings.TrimPrefix(disk.Path, "/dev/zvol/")) > 0 {
				diskPath = disk.Path
...
func (p *BhyveProvider) DeleteInstance(ctx context.Context, handle provider.InstanceHandle, force bool) (err error) {
	...
	cmd := exec.CommandContext(ctx, "bhyvectl", "--destroy", fmt.Sprintf("--vm=%s", vmName))
	...
	if configErr == nil {
		for _, tap := range config.TapDevs {
			...
			if err := destroyTapSafe(ctx, tap); err != nil {
	...
	if err := exec.CommandContext(ctx, "zfs", "destroy", "-r", zvolParent).Run(); err != nil {
...
func destroyVMMObject(vmName string) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	_ = exec.CommandContext(ctx, "bhyvectl", "--destroy", fmt.Sprintf("--vm=%s", vmName)).Run()
}
...
func (p *BhyveProvider) StopInstance(ctx context.Context, handle provider.InstanceHandle, opts provider.StopOptions) (err error) {
	...
	if stateErr == nil {
		...
		case provider.StatePaused:
			...
			if state.PID > 0 && pidIsBhyveVM(ctx, state.PID, vmName) {
	...
	if !opts.Force {
		if !pidIsBhyveVM(ctx, state.PID, vmName) {
...
func (p *BhyveProvider) GetInstanceState(ctx context.Context, handle provider.InstanceHandle) (provider.InstanceState, error) {
	...
	case state.State == provider.StateStopped:
		if pids := bhyvePIDsForVM(ctx, vmName); len(pids) > 0 {
...
func (p *BhyveProvider) GetInstanceInfo(ctx context.Context, handle provider.InstanceHandle) (provider.InstanceInfo, error) {
	...
	for _, tap := range config.TapDevs {
		spec.Networks = append(spec.Networks, provider.NetworkSpec{ID: tap, Type: provider.NetworkTypeBridge})
	}
...
func cleanupZVOLs(ctx context.Context, zvols []string) {
	for _, zvol := range zvols {
		cmd := exec.CommandContext(ctx, "zfs", "destroy", zvol)
		if err := cmd.Run(); err != nil {
...
func cleanupTaps(ctx context.Context, taps []string) {
	for _, tap := range taps {
		_ = destroyTapSafe(ctx, tap) // best-effort cleanup of orphaned taps
	}
}
...
func destroyTapSafe(ctx context.Context, tap string) error {
	...
	if out, err := exec.CommandContext(ctx, "ifconfig", "-a").Output(); err == nil {
		...
		for _, line := range strings.Split(string(out), "\n") {
			...
			if strings.HasPrefix(trimmed, "member: "+tap) {
				if out, err := exec.CommandContext(ctx, "ifconfig", currentBridge, "deletem", tap).CombinedOutput(); err != nil {
	...
	cmd := exec.CommandContext(ctx, "ifconfig", tap, "destroy")
	if err := cmd.Run(); err != nil {
...
func pidIsBhyveVM(ctx context.Context, pid int, vmName string) bool {
	if pid <= 0 {
		return false
	}
...
func bhyvePIDsForVM(ctx context.Context, vmName string) []int {
	// SECURITY: QuoteMeta prevents regex injection via VM names containing regex metacharacters
	out, err := exec.CommandContext(ctx, "pgrep", "-f", fmt.Sprintf("bhyve.*%s", regexp.QuoteMeta(vmName))).Output()
	if err != nil {
		return nil
	}
	var pids []int
	for _, line := range strings.Fields(string(out)) {
		...
		cmdOut, err := exec.CommandContext(ctx, "ps", "-p", strconv.Itoa(pid), "-o", "command=").Output()
		if err != nil {
			continue
		}
...
func killOrphanBhyveProcess(ctx context.Context, vmName string) {
	...
	defer cancel()
	for _, pid := range bhyvePIDsForVM(pgrepCtx, vmName) {
...
func (p *BhyveProvider) startSwtpm(ctx context.Context, vmName, vmDir string, config *vmConfig) error {
	...
	if err != nil || len(entries) == 0 {
		...
		setupCmd := exec.CommandContext(ctx, p.swtpmSetupPath,
			"--tpm2",
			"--tpmstate", tpmStateDir,
			"--createek",
			"--allow-signing",
			"--decryption",
			"--create-ek-cert",
	...
	cmd := exec.CommandContext(ctx, p.swtpmPath,
		"socket",
		"--tpm2",
		"--tpmstate", fmt.Sprintf("dir=%s", tpmStateDir),
		// --server (not --ctrl): bhyve sends raw TPM2 bytes; --ctrl is for QEMU's PTM protocol.
		"--server", fmt.Sprintf("type=unixio,path=%s", sockPath),
		"--pid", fmt.Sprintf("file=%s", pidFile),
		// not-need-init: allow bhyve to send commands without an explicit TPM_Startup first.
		"--flags", "not-need-init",
		"--daemon",
	)
...
func stopSwtpm(vmName, vmDir string) {
	pidFile := filepath.Join(vmDir, "swtpm.pid")
...
func (p *BhyveProvider) SetInstanceResources(ctx context.Context, handle provider.InstanceHandle, resources provider.ResourceSpec) error {
	...
	if resources.MemoryMB > 0 {
		config.MemoryMB = resources.MemoryMB
	}
