...
func (p *BhyveProvider) InsertMedia(ctx context.Context, handle provider.InstanceHandle, spec provider.MediaSpec) error {
	...
	// Verify the media file exists. Only "not exist" means absent; any other
	// stat failure — a permission error, a broken mount — leaves the path
	// uninspected, and continuing would hand the guest something unverified.
	if _, err := os.Stat(spec.Path); err != nil {
		if os.IsNotExist(err) {
			return fmt.Errorf("media file not found: %s", spec.Path)
		}
		return fmt.Errorf("cannot inspect media file %s: %w", spec.Path, err)
	}
	...
	// A failed lookup is not permission to proceed: changing media under a
	// running VM is exactly what this guard exists to prevent.
	state, err := p.GetInstanceState(ctx, handle)
	if err != nil {
		return fmt.Errorf("cannot determine whether %s is running: %w", vmName, err)
	}
	...
	if deviceFound {
		// parseVMConfig fills DiskPaths and DiskDrivers from two independent
		// lines, so a vm.conf with more drivers than disks makes this index
		// out of range and takes the daemon down.
		if deviceIndex >= len(config.DiskPaths) {
			return fmt.Errorf("configuration of %s is inconsistent: %d disk drivers but %d disk paths",
				vmName, len(config.DiskDrivers), len(config.DiskPaths))
		}
		// Update existing CD-ROM device path.
		config.DiskPaths[deviceIndex] = spec.Path
	} else {
		// An empty DeviceID means "add one"; a DeviceID that names nothing is a
		// caller referring to a device that is not there. Appending gave it a
		// different id than the one it asked for and reported success.
		if spec.DeviceID != "" {
			return fmt.Errorf("VM %s has no CD-ROM device %s", vmName, spec.DeviceID)
		}
...
func (p *BhyveProvider) EjectMedia(ctx context.Context, handle provider.InstanceHandle, deviceID string) error {
	...
	// BootOrder holds disk indices, and removing an entry shifts every index
	// above it down by one. Left untouched, the saved order then names the
	// wrong disks — or one that no longer exists.
	if len(config.BootOrder) > 0 {
		adjusted := make([]int, 0, len(config.BootOrder))
		for _, idx := range config.BootOrder {
			switch {
			case idx == ejectedIndex:
				// The device is gone; drop it from the order.
			case idx > ejectedIndex:
...
func (p *BhyveProvider) SetBootOrder(ctx context.Context, handle provider.InstanceHandle, order provider.BootOrder) error {
	...
	// bhyve has no one-time boot: the order it is given is the order it keeps.
	// Accepting the field and writing a permanent order instead would leave the
	// VM booting from that device on every start, silently.
	if order.OnceDevice != "" {
		return fmt.Errorf("bhyve cannot boot once from %s: it has no one-time boot order", order.OnceDevice)
	}

	// On SetBootOrder rather than setBootOrderByIndex: the device names are
	// mapped to indices from a config read here, and a concurrent disk change
	// between that read and the write would make them point elsewhere.
	_, release, err := p.locks.Acquire(ctx, handle.ID)
	if err != nil {
		return err
	}
	...
	// -1 means "this VM has no such device". Falling back to index 0 made a
	// request to boot from CD-ROM persist a boot order pointing at the hard
	// disk, and report success for it.
	...
	for _, dev := range order.Devices {
		idx := findIndex(dev)
		if idx < 0 {
			return fmt.Errorf("VM %s has no %s device to boot from", handle.ID, dev)
		}
		if !seen[idx] {
