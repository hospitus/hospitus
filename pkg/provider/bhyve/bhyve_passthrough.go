...
func (p *BhyveProvider) AttachPassthroughDevice(ctx context.Context, handle provider.InstanceHandle, device PassthroughDevice) error {
	...
	// passthrough.json is read, changed and written back. Two attachments at
	// once both read the old list and the second write dropped the first
	// device.
	...
	// Only "the file is not there" may continue with an empty list: any other
	// read failure leaves the existing devices unknown, and the save below
	// would then erase every one of them.
	...
	case !os.IsNotExist(err):
		return fmt.Errorf("reading passthrough config %s: %w", passthroughPath, err)
	}
...
func pciSlotToBhyve(slot string) (string, error) {
	...
	// "bus.device.function", the other spelling pciSelectorRE accepts.
	if dots := strings.Split(slot, "."); len(dots) == 3 {
		slot = fmt.Sprintf("%s:%s.%s", dots[0], dots[1], dots[2])
	}
...
func (p *BhyveProvider) validatePCIDevice(ctx context.Context, pciSlot string) error {
	...
	output, err := p.cmd().CombinedOutput(ctx, "pciconf", "-l")
	if err != nil {
		return fmt.Errorf("failed to list PCI devices: %w", err)
	}

	// pciconf writes "name@pci<domain>:<bus>:<dev>:<func>:" — colon-separated
	// and domain-prefixed — while pciSlot is bhyve's "bus/dev/func". A substring
	// search across those two spellings can never match, so every device was
	// reported as absent. Both sides are normalized before comparing.
	want, err := pciSlotToBhyve(pciSlot)
	if err != nil {
		return fmt.Errorf("invalid PCI slot %q: %w", pciSlot, err)
	}
	for _, line := range strings.Split(string(output), "\n") {
		_, rest, found := strings.Cut(line, "@pci")
		if !found {
			continue
		}
		addr, _, _ := strings.Cut(rest, ":\t")
		fields := strings.Split(strings.TrimSuffix(addr, ":"), ":")
		if len(fields) != 4 {
			continue
		}
		if got, err := pciSlotToBhyve(fmt.Sprintf("%s:%s:%s.%s",
			fields[0], fields[1], fields[2], fields[3])); err == nil && got == want {
			return nil
		}
	}
...
func (p *BhyveProvider) ListAvailablePCIDevices(ctx context.Context) ([]PCIDeviceInfo, error) {
	...
	output, err := p.cmd().CombinedOutput(ctx, "pciconf", "-l", "-v")
	if err != nil {
	...
	for _, line := range lines {
		...
		if strings.Contains(line, "@pci") {
			...
			// The key, not the line: pciconf -lv also emits subvendor,
			// subdevice and subclass, and a substring match let those overwrite
			// the identifiers with the sub-identifier values.
