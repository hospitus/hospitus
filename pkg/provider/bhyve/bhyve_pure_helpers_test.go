...
func TestCoerceInt64(t *testing.T) {
	cases := []struct {
		...
		{"int", 7, 7, true},
		// JSON has no integers, so a whole number arrives as a float64 and must
		// be accepted; a fractional one is a mistake in the request, and
		// truncating it gave the caller a VM it did not ask for.
		{"whole float64", float64(3), 3, true},
		{"fractional float64", float64(3.9), 0, false},
...
func TestGuestMACForVMMissingConfig(t *testing.T) {
	...
	if got := p.guestMACForVM(context.Background(), "ghost"); got != "" {
