...
func (p *BhyveProvider) ensureNATBridge(ctx context.Context) error {
	...
	cmd := exec.CommandContext(ctx, "ifconfig", natBridge)
	...
	if err == nil {
		...
		cmd = exec.CommandContext(ctx, "ifconfig", natBridge, "inet", natGatewayIP, "netmask", natNetmask)
	...
	cmd = exec.CommandContext(ctx, "ifconfig", "bridge", "create", "name", natBridge)
	if out, err := cmd.CombinedOutput(); err != nil {
	...
	// Assign gateway IP.
	cmd = exec.CommandContext(ctx, "ifconfig", natBridge, "inet", natGatewayIP, "netmask", natNetmask, "up")
	if out, err := cmd.CombinedOutput(); err != nil {
	...
	cmd = exec.CommandContext(ctx, "sysctl", "net.inet.ip.forwarding=1")
	if out, err := cmd.CombinedOutput(); err != nil {
...
func (p *BhyveProvider) setupNATForVM(ctx context.Context, vmName, tapDev string) error {
	...
	extIf, err := bhyveDefaultInterface()
	if err != nil {
...
func (p *BhyveProvider) ensureDNSMasq(ctx context.Context) error {
	...
	if isProcessRunning(dnsmasqPidFile) {
		return nil
	}

	cmd := exec.CommandContext(ctx, dnsmasqBin, dnsmasqArgs()...)
	if out, err := cmd.CombinedOutput(); err != nil {
...
// isProcessRunning checks if a process is running by reading its PID file.
func isProcessRunning(pidFile string) bool {
	...
	// Signal 0 performs error checking only: it reports whether the process
	// exists (and is signalable) without actually sending a signal.
	return syscall.Kill(pid, 0) == nil
}

// bhyveDefaultInterface returns the network interface for the default route.
func bhyveDefaultInterface() (string, error) {
...
func (p *BhyveProvider) lookupIPsByVM(ctx context.Context, vmName string) ([]net.IP, error) {
	...
	guestMAC := p.guestMACForVM(vmName)
	if guestMAC == "" {
		return nil, nil
	}
...
func (p *BhyveProvider) setupIPv6NATForVM(ctx context.Context, vmName, ipv6Prefix string) error {
	...
	if out, err := exec.CommandContext(ctx, "ifconfig", natBridge).Output(); err == nil {
		if !strings.Contains(string(out), ipv6NATGateway) {
			cmd := exec.CommandContext(ctx, "ifconfig", natBridge, "inet6", ipv6Prefix, "up")
	...
	if out, err := exec.CommandContext(ctx, "sysctl", "net.inet6.ip6.forwarding=1").CombinedOutput(); err != nil {
...
func (p *BhyveProvider) ensureVLANBridge(ctx context.Context, vlanID int) (string, error) {
	...
	if _, err := exec.CommandContext(ctx, "ifconfig", bridgeName).Output(); err == nil {
		return bridgeName, nil
	}
	if out, err := exec.CommandContext(ctx, "ifconfig", "bridge", "create", "name", bridgeName).CombinedOutput(); err != nil {
		return "", fmt.Errorf("failed to create VLAN bridge %s: %w (output: %s)", bridgeName, err, out)
	}
	if out, err := exec.CommandContext(ctx, "ifconfig", bridgeName, "up").CombinedOutput(); err != nil {
...
func (p *BhyveProvider) setupVLANForTap(ctx context.Context, tapDev string, vlanID int) error {
	...
	parentIf, err := bhyveDefaultInterface()
	if err != nil {
	...
	if _, checkErr := exec.CommandContext(ctx, "ifconfig", vlanIfName).Output(); checkErr != nil {
		if out, err := exec.CommandContext(ctx, "ifconfig", vlanIfName, "create").CombinedOutput(); err != nil {
			return fmt.Errorf("failed to create VLAN interface %s: %w (output: %s)", vlanIfName, err, out)
		}
	}

	// Configure vlandev and tag.
	if out, err := exec.CommandContext(ctx, "ifconfig", vlanIfName, "vlandev", parentIf, "vlan", fmt.Sprintf("%d", vlanID), "up").CombinedOutput(); err != nil {
...
func (p *BhyveProvider) teardownVLANForTap(ctx context.Context, tapDev string, vlanID int) {
	vlanIfName := fmt.Sprintf("%s.%d", tapDev, vlanID)
	if out, err := exec.CommandContext(ctx, "ifconfig", vlanIfName, "destroy").CombinedOutput(); err != nil {
