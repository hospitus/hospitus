...
func (p *BhyveProvider) ExecuteHooks(ctx context.Context, hookType HookType, env HookEnv) ([]HookResult, error) {
	...
	if err != nil {
		firstError = err
		// executeHooksFromDir already fails closed for "pre-" hooks: it stops at
		// the first refusal. Running the per-VM hooks anyway discarded that
		// decision, so a global pre-start hook that refused an operation still
		// let the rest of the sequence run.
		if strings.HasPrefix(string(hookType), "pre-") {
...
func (p *BhyveProvider) executeHooksFromDir(ctx context.Context, hookType HookType, dir string, env HookEnv) ([]HookResult, error) {
	...
	if dirExistsBhyve(hookSubdir) {
		...
		if err != nil {
			// Skipping the directory turns a "pre-" gate into a no-op: an
			// unreadable pre-start directory would let the VM start without the
			// checks the operator installed.
			return results, fmt.Errorf("reading hook directory %s: %w", hookSubdir, err)
		}
