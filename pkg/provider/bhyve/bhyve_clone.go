...
func (p *BhyveProvider) CloneFromSnapshot(ctx context.Context, snapshot provider.SnapshotHandle, cloneName string, opts provider.CloneOptions) (provider.InstanceHandle, error) {
	...
	// Locked like CloneInstance and RenameInstance: without this, two calls with
	// the same clone name both pass the existence checks and then race on the
	// same "zfs clone" target and the same directory.
	ctx, releaseLock, lockErr := p.locks.AcquireAll(ctx, snapshot.Instance, cloneName)
	if lockErr != nil {
		return provider.InstanceHandle{}, lockErr
	}
	defer releaseLock()

	// Both names are bound to the instance the handle declares. The dataset in
	// particular decides where the clone lands, two directories up.
	zfsSnapshot, err := p.snapshotFor(snapshot)
	if err != nil {
		return provider.InstanceHandle{}, err
	}

	dataset, err := p.datasetFor(snapshot)
	if err != nil {
		return provider.InstanceHandle{}, err
	}
	...
	// Carries the TPM, VNC and passthrough settings. Left aliased, any write
	// CreateInstance made to the clone's config also changed the source's.
	cloneSpec.ProviderConfig = copyAnyMap(originalInfo.Spec.ProviderConfig)
	...
	// CloneInstance checks this before it starts; this path did not, and a
	// source whose persisted config lists no disk panicked here — after the ZFS
	// clone was made, which the panic then left behind.
	if len(cloneSpec.Disks) == 0 {
		p.zfsDestroyCleanup(ctx, cloneDataset)
		return provider.InstanceHandle{}, fmt.Errorf("source instance %q has no disks", snapshot.Instance)
	}
	// CreateSnapshot only ever snapshots the primary disk, so a snapshot of a
	// multi-disk VM does not contain the others. Repointing disk 0 and leaving
	// the rest as they were handed the clone the source's own volumes, which
	// CreateInstance then adopted rather than created — two VMs writing one
	// zvol. CloneInstance copies every disk; this path cannot, so it refuses.
	if len(cloneSpec.Disks) > 1 {
		p.zfsDestroyCleanup(ctx, cloneDataset)
		return provider.InstanceHandle{}, fmt.Errorf(
			"instance %q has %d disks and the snapshot covers only the first; "+
				"clone the instance directly instead of cloning from a snapshot",
			snapshot.Instance, len(cloneSpec.Disks))
	}
	...
	// Create the cloned instance. Every earlier failure in this function
	// destroys the clone dataset; this one did not, so a create that failed
	// left the dataset behind and the next attempt with the same name hit it.
	handle, err := p.CreateInstance(ctx, cloneSpec)
	if err != nil {
		p.zfsDestroyCleanup(ctx, cloneDataset)
		return provider.InstanceHandle{}, err
...
func (p *BhyveProvider) zvolSizeGB(ctx context.Context, dataset string) int {
	out, err := p.cmd().Output(ctx, "zfs", "get", "-Hp", "-o", "value", "volsize", dataset)
	if err != nil {
		return 0
	}
