...
func (p *BhyveProvider) CheckpointInstance(ctx context.Context, handle provider.InstanceHandle, name string) error {
	...
	if !bhyvectlSupportsCheckpoint(ctx) {
	...
	cmd := exec.CommandContext(ctx, "bhyvectl",
		fmt.Sprintf("--suspend=%s", ckptFile),
...
func (p *BhyveProvider) PauseInstance(ctx context.Context, handle provider.InstanceHandle) error {
	...
	if !pidIsBhyveVM(ctx, st.PID, vmName) {
...
func (p *BhyveProvider) RenameInstance(ctx context.Context, handle provider.InstanceHandle, newName string) error {
	...
	zfsExists := exec.CommandContext(ctx, "zfs", "list", "-H", oldZFSParent).Run() == nil
	if zfsExists {
		cmd := exec.CommandContext(ctx, "zfs", "rename", oldZFSParent, newZFSParent)
		if output, err := cmd.CombinedOutput(); err != nil {
	...
	// Update tap device names (convention: tap_<vmName>_<index>).
	for i, tap := range config.TapDevs {
		config.TapDevs[i] = strings.ReplaceAll(tap, oldName, newName)
	}
	...
	if err := os.Rename(oldVMDir, newVMDir); err != nil {
		...
		if zfsExists {
			_ = exec.CommandContext(ctx, "zfs", "rename", newZFSParent, oldZFSParent).Run()
		}
...
func bhyvectlSupportsCheckpoint(ctx context.Context) bool {
	// --help exits non-zero on some versions; the output is what matters.
	out, _ := exec.CommandContext(ctx, "bhyvectl", "--help").CombinedOutput()
