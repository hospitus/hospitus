...
// ResetTrafficStats resets traffic statistics for a VM.
//
// Note: This is not truly a "reset" since we can't reset kernel counters,
// but we can save the current counters as a baseline for delta calculations.
func (p *BhyveProvider) ResetTrafficStats(ctx context.Context, handle provider.InstanceHandle) error {
	// For now, this is a no-op
	// A full implementation would store baseline counters in VM config
	// and calculate deltas in GetTrafficStats
	return nil
}
