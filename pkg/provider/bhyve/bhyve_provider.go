...
type vmConfig struct {
	...
	VNCHost    string // VNC bind address (default: 127.0.0.1)
	...
	// Valid values: virtio-blk (default), ahci-hd, nvme, virtio-scsi.
	DiskDriver string
}
...
func (p *BhyveProvider) HealthCheck(ctx context.Context) error {
	...
	cmd := exec.CommandContext(ctx, "kldstat", "-q", "-m", "vmm")
...
func (p *BhyveProvider) detectVirtualizationSupport(ctx context.Context) error {
	...
	cmd = exec.CommandContext(ctx, "sysctl", "-n", "hw.vmm.vmx.initialized")
	...
	cmd = exec.CommandContext(ctx, "sysctl", "hw.vmm.vmx.cap.halt_exit")
	if err := cmd.Run(); err == nil {
		p.hasVMX = true
		return nil
	}
	...
	cmd = exec.CommandContext(ctx, "sysctl", "-n", "hw.vmm.svm.num_asids")
	output, err = cmd.Output()
	if err == nil {
	...
	cmd = exec.CommandContext(ctx, "bhyve", "-h")
	if err := cmd.Run(); err == nil {
...
func (p *BhyveProvider) ensureNMDMModule(ctx context.Context) bool {
	...
	cmd := exec.CommandContext(ctx, "kldstat", "-q", "-n", "nmdm")
	if cmd.Run() == nil {
		return true
	}

	// Try to load the module
	cmd = exec.CommandContext(ctx, "kldload", "nmdm")
	if err := cmd.Run(); err != nil {
...
func (p *BhyveProvider) ensureZFSDataset(ctx context.Context, datasetName string) error {
	...
	cmd := exec.CommandContext(ctx, "zfs", "list", "-H", datasetName)
	if err := cmd.Run(); err == nil {
		// Dataset exists
		return nil
	}
	...
	cmd = exec.CommandContext(ctx, "zfs", "create", "-p", datasetName)
	if err := cmd.Run(); err != nil {
...
func (p *BhyveProvider) createRawDisk(ctx context.Context, path string, sizeBytes int64) error {
	cmd := exec.CommandContext(ctx, "truncate", "-s", strconv.FormatInt(sizeBytes, 10), path)
	if err := cmd.Run(); err != nil {
...
func (p *BhyveProvider) createTapDevice(ctx context.Context, vmName string, index int) (string, error) {
	...
	if exec.CommandContext(ctx, "ifconfig", tapName).Run() == nil {
		if bridge := findBridgeForMember(ctx, tapName); bridge != "" {
			if out, err := exec.CommandContext(ctx, "ifconfig", bridge, "deletem", tapName).CombinedOutput(); err != nil {
		...
		if out, err := exec.CommandContext(ctx, "ifconfig", tapName, "destroy").CombinedOutput(); err != nil {
	...
	// Create tap device
	cmd := exec.CommandContext(ctx, "ifconfig", "tap", "create", "name", tapName)
	if output, err := cmd.CombinedOutput(); err != nil {
		return "", fmt.Errorf("failed to create tap device %s: %w (output: %s)", tapName, err, string(output))
	}

	// Bring up the interface
	cmd = exec.CommandContext(ctx, "ifconfig", tapName, "up")
	if err := cmd.Run(); err != nil {
...
func findBridgeForMember(ctx context.Context, member string) string {
	out, err := exec.CommandContext(ctx, "ifconfig", "-g", "bridge").Output()
	if err != nil {
		return ""
	}
	for _, bridge := range strings.Fields(string(out)) {
		bridgeOut, err := exec.CommandContext(ctx, "ifconfig", bridge).Output()
		if err != nil {
			continue
		}
