...
func (p *BhyveProvider) DeleteSnapshot(ctx context.Context, snapshot provider.SnapshotHandle) error {
	// The handle comes from the caller, not from CreateSnapshot: resolve the
	// name through the guard that confines it and binds it to the instance the
	// handle declares.
...
func (p *BhyveProvider) RestoreSnapshot(ctx context.Context, handle provider.InstanceHandle, snapshot provider.SnapshotHandle) error {
	...
	// "-r" destroys every snapshot taken after this one, so the name has to be
	// bound to the instance the handle declares, not merely well-formed.
	zfsName, err := p.snapshotFor(snapshot)
	if err != nil {
		return err
	}
