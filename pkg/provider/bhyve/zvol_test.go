...
func TestCreateZVOLDelegatesToStorage(t *testing.T) {
	backend := &fakeStorage{}
	p := &BhyveProvider{
		zfsParent:      "zroot/hospitus/bhyve",
	...
	const zvol = "zroot/hospitus/bhyve/vm1/disk0"
...
func TestCreateZVOLReusesExistingVolume(t *testing.T) {
	...
	if err := p.createZVOL(context.Background(), "zroot/hospitus/bhyve/vm1/disk0", 20); err != nil {
		t.Fatalf("createZVOL: %v", err)
	}
...
func TestZVOLExistsQueriesVolumeType(t *testing.T) {
	...
	if !p.zvolExists(context.Background(), "zroot/hospitus/bhyve/vm1/disk0") {
	...
	if got != "list -t volume -H zroot/hospitus/bhyve/vm1/disk0" {
