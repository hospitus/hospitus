...
func TestAllowedPhysicalDisksParsing(t *testing.T) {
	...
	// The third shape allowedPhysicalDisks handles: a []string, which is what
	// Go code building a config passes, as opposed to a decoded JSON body.
	p2b := &BhyveProvider{config: provider.ProviderConfig{Settings: map[string]interface{}{
		"allowed_physical_disks": []string{"/dev/ada2", "/dev/da5"},
	}}}
	if got := p2b.allowedPhysicalDisks(); len(got) != 2 || got[0] != "/dev/ada2" || got[1] != "/dev/da5" {
		t.Errorf("[]string parse = %v", got)
	}
