...
func (p *PFBackend) checkAnchorNoLock(ctx context.Context) error {
	...
	// Check if our nat-anchor is declared in the active main ruleset.
	// nat-anchor lines only appear in "pfctl -sn" (NAT rules), not "-sr" (filter rules).
	natCmd := exec.CommandContext(ctx, "pfctl", "-sn")
	natOut, _ := natCmd.CombinedOutput()
	if natAnchorDeclared(string(natOut)) {
		p.logger.Debug("PF nat-anchor already present in active ruleset", "anchor", pfAnchorName)
		return nil
	}

	return fmt.Errorf("hospitus anchor %q is not declared in pf.conf", pfAnchorName)
}

// natAnchorDeclared reports whether the hospitus nat-anchor is present in the
// output of "pfctl -sn" (the active NAT ruleset). It matches the exact bare
// anchor name Hospitus loads rules into (pfctl -a hospitus); the "hospitus/*" glob form
// matches only sub-anchors and therefore does not count.
func natAnchorDeclared(natRuleset string) bool {
	return strings.Contains(natRuleset, fmt.Sprintf("nat-anchor %q", pfAnchorName))
}
...
func (p *PFBackend) reloadRulesNoLock(ctx context.Context) error {
	...
	if natContent, err := os.ReadFile(natFile); err == nil {
	...
	if rdrContent, err := os.ReadFile(rdrFile); err == nil {
	...
	if filterContent, err := os.ReadFile(filterFile); err == nil {
...
func (p *PFBackend) appendRule(filename, id, instance, rule string) error {
	...
	// SECURITY: Reject multi-line injection in instance name and rule.
	// PF rules are stored as text files; newlines in these fields would
	// allow injecting arbitrary PF rules outside the intended rule.
	...
	// Rule may contain a single trailing newline (formatted PF rules
	// often end with \n), but embedded newlines are invalid — they
	// would split one rule into multiple independent rules.
