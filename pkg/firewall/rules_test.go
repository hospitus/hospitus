...
func TestPortMappingGenerateID(t *testing.T) {
	...
	expected := "web/jail/tcp/any/80/any/8080"
	if id != expected {
		t.Errorf("GenerateID() = %v, want %v", id, expected)
	}
}

// TestPortMappingGenerateIDSeparatesRules covers what the identifier has to
// keep apart: PF reads a re-add under a known ID as a rule it already has and
// writes nothing, so two mappings sharing an ID leave the second one absent.
func TestPortMappingGenerateIDSeparatesRules(t *testing.T) {
	base := PortMapping{
		Instance: "web", Provider: "jail", Protocol: ProtocolTCP,
		HostInterface: "em0", HostPort: 80, TargetIP: "10.0.0.2", TargetPort: 8080,
	}
	otherIface := base
	otherIface.HostInterface = "em1"
	otherTarget := base
	otherTarget.TargetIP = "10.0.0.3"

	if base.GenerateID() == otherIface.GenerateID() {
		t.Error("two mappings on different host interfaces share an ID")
	}
	if base.GenerateID() == otherTarget.GenerateID() {
		t.Error("two mappings to different targets share an ID")
	}

	// Adjacent fields that both allow "-": a "-" separator would merge them.
	left := PortMapping{
		Instance: "web-jail", Provider: "bhyve", Protocol: ProtocolTCP,
		HostInterface: "em0", HostPort: 80, TargetIP: "10.0.0.2", TargetPort: 8080,
	}
	right := left
	right.Instance, right.Provider = "web", "jail-bhyve"
	if left.GenerateID() == right.GenerateID() {
		t.Errorf("hyphenated neighbors collide: %s", left.GenerateID())
	}
}
