...
// TestAppendRulesRejectsInjection covers the multi-rule writer, which the port
// mapping path uses. Its single-rule sibling has always validated these fields;
// this one wrote them straight into the anchor's rule file.
func TestAppendRulesRejectsInjection(t *testing.T) {
	dir := t.TempDir()
	file := filepath.Join(dir, "rdr.conf")
	p := &PFBackend{}
	...
	cases := []struct {
		...
		{"newline in the ID", "web-jail\npass in all", "web", []string{"rdr on em0 proto tcp"}},
		{"newline in the instance", "web-jail", "web\npass in all", []string{"rdr on em0 proto tcp"}},
		{"newline in a rule", "web-jail", "web", []string{"rdr on em0 proto tcp\npass in all"}},
		{"newline in a later rule", "web-jail", "web", []string{"rdr on em0", "ok\nblock all"}},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if err := p.appendRules(file, tc.id, tc.instance, tc.rules); err == nil {
				t.Error("accepted")
			}
		})
	}
	...
	if _, err := os.Stat(file); !os.IsNotExist(err) {
		t.Errorf("a refused rule still created %s", file)
	}
}
