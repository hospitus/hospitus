...
func (j *Job) UpdateProgress(progress float64, message string) {
	j.mu.Lock()
	defer j.mu.Unlock()
	// Every comparison with NaN is false, so it would slip past both clamps and
	// then fail encoding/json on the way out. total == 0 is enough to produce it.
...
func (j *Job) Snapshot() JobRecord {
	...
	return JobRecord{
		...
		Progress:    j.Progress,
		Message:     j.Message,
		Result:      maps.Clone(j.Result),
		Error:       j.Error,
		CreatedAt:   j.CreatedAt,
		StartedAt:   j.StartedAt,
		CompletedAt: j.CompletedAt,
		Metadata:    maps.Clone(j.Metadata),
	}
}
...
var (
	...
	ErrJobActive         = errors.New("job is active")
)
...
func (jm *JobManager) DeleteJob(id string) error {
	...
	if st := job.snapshotStatus(); st == JobStatusRunning || st == JobStatusPending {
		return fmt.Errorf("cannot delete active job: %w: status is %s", ErrJobActive, st)
	}
...
func (jm *JobManager) Shutdown(timeout time.Duration) {
	...
	// Only now. Every job context derives from jm.ctx, so canceling before the
	// wait would kill the very jobs the timeout is meant to let finish, and hand
	// an already-canceled context to anything the drain picks up.
	jm.cancelAll()
}
...
func (jm *JobManager) executeJob(job *Job, fn JobFunc) {
	...
	if job.Status == JobStatusCancelled {
		...
		// CancelJob already fired the hook for this transition; firing it again
		// here would record the same terminal state twice.
		return
	}
