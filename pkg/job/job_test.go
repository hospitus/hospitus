...
// TestSubmit_AfterShutdown verifies that submitting a job after Shutdown returns
// an error containing "shutting down".
func TestSubmit_AfterShutdown(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "shutting down") {
		t.Errorf("expected 'shutting down' in error, got: %v", err)
	}
}
...
func TestJobIDUniqueness(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		go func() {
			...
			if err != nil {
				// Queue may be briefly full under heavy concurrency — acceptable
				return
			}
			mu.Lock()
			ids[j.ID] = struct{}{}
	...
	if len(ids) != n {
		t.Errorf("expected %d unique IDs, got %d (duplicates present)", n, len(ids))
	}
}
...
func TestJob_SetError(t *testing.T) {
	...
	j, err := jm.Submit("error.test", "set error", nil, func(ctx context.Context, job *Job) error {
		job.SetError(errors.New("something went wrong"))
		return fmt.Errorf("job failed intentionally")
	})
	if err != nil {
		t.Fatalf("Submit: %v", err)
	}
	...
	if !strings.Contains(errMsg, "job failed intentionally") {
		t.Errorf("expected error message to contain job error, got: %q", errMsg)
	}
}
...
func TestCancelJob_AlreadyCompleted(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "cannot be canceled") {
		t.Errorf("expected 'cannot be cancelled' in error, got: %v", err)
	}
}
