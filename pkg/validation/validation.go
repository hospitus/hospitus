...
//
// Use this for logging user input to prevent log injection.
func SanitizeString(s string) string {
	// Keep printable characters (excluding DEL) plus tab and newline; drop all
	// other control characters (including carriage return) to prevent log
	// injection. Parentheses make the precedence explicit.
...
var (
	...
	sshHostRegex = regexp.MustCompile(`^([a-zA-Z0-9._-]+@)?[a-zA-Z0-9._-]+$`)
