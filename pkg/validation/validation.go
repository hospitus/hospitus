...
func ValidateProviderConfigEntry(key string, value interface{}) error {
	...
	if strings.ContainsAny(key, commandInjectionChars+"/") {
...
// validateConfigKey checks a map key the way a value is checked. A null byte is
// valid UTF-8, so testing only the encoding lets one through in the one position
// nothing else looks at.
func validateConfigKey(k string) error {
	if strings.ContainsRune(k, 0) {
		return fmt.Errorf("provider config holds a key containing a null byte")
	}
	...
	case reflect.Map:
		for _, k := range rv.MapKeys() {
			if k.Kind() == reflect.String {
				if err := validateConfigKey(k.String()); err != nil {
					return err
				}
			}
...
func ValidateExecOptions(opts provider.ExecOptions) error {
	...
	// Every argument gets the same rule, the shell-code argument of `sh -c`
	// included: reject null bytes, a bare CR and invalid UTF-8, and leave shell
	// metacharacters alone. Nothing here splits a command line, so a metacharacter
	// is only ever interpreted by a shell the caller asked for explicitly.
...
func ValidateExecCommand(cmd string) error {
	...
	if strings.ContainsAny(cmd, commandInjectionChars) {
...
func ValidateDiskSpec(disk provider.DiskSpec) error {
	if disk.ID != "" {
		...
		if strings.ContainsAny(disk.ID, commandInjectionChars) {
	...
	if disk.DeviceName != "" {
		...
		if strings.ContainsAny(disk.DeviceName, commandInjectionChars) {
...
func ValidateNetworkSpec(network provider.NetworkSpec) error {
	if network.ID != "" {
		...
		if strings.ContainsAny(network.ID, commandInjectionChars) {
	...
	if network.IPv6 != "" {
		// No "dhcp" sentinel here, unlike IPv4: nothing downstream interprets one,
		// so it would be written out as a literal address. Say so rather than let
		// the generic parse error suggest a typo.
		if network.IPv6 == "dhcp" || network.IPv6 == "auto" || network.IPv6 == "slaac" {
			return fmt.Errorf("network IPv6 takes a static address; %q is not a supported value", network.IPv6)
		}
