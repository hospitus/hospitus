...
//
// Every probe that shells out takes the caller's context. DefaultChecks runs
// dozens of them one after another, so without it a hung pfctl, zpool or sysctl
// holds the caller for the sum of every timeout and no canceled request or
// Ctrl-C can stop it.
type System interface {
	...
	KernelModuleLoaded(ctx context.Context, name string) bool
	// ServiceEnabled reports whether an rc.d service is enabled (rcvar=YES).
	ServiceEnabled(ctx context.Context, rcvar string) bool
	...
	ZFSPoolExists(ctx context.Context, pool string) bool
	...
	BinmiscActivators(ctx context.Context) []string
...
func (realSystem) Sysctl(ctx context.Context, name string) (string, error) {
...
func (realSystem) KernelModuleLoaded(ctx context.Context, name string) bool {
	ctx, cancel := context.WithTimeout(ctx, 5*time.Second)
	defer cancel()
...
func (realSystem) ServiceEnabled(ctx context.Context, rcvar string) bool {
	ctx, cancel := context.WithTimeout(ctx, 5*time.Second)
	defer cancel()
...
func (realSystem) DirWritable(path string) bool {
	...
	// Does not exist yet: the nearest existing parent decides. Probe it the same
	// way rather than reading its mode bits — the daemon runs as root, for which
	// the mode says "writable" even on a read-only mount or a full disk.
...
func (realSystem) ZFSPoolExists(ctx context.Context, pool string) bool {
	ctx, cancel := context.WithTimeout(ctx, 5*time.Second)
	defer cancel()
...
// deliberate act.
func (realSystem) BinmiscActivators(ctx context.Context) []string {
	ctx, cancel := context.WithTimeout(ctx, 5*time.Second)
	defer cancel()
...
func (realSystem) PFFilterRules(ctx context.Context) []string {
	ctx, cancel := context.WithTimeout(ctx, 5*time.Second)
	defer cancel()
...
func (realSystem) PFAnchorDeclared(ctx context.Context, anchor string) bool {
	ctx, cancel := context.WithTimeout(ctx, 5*time.Second)
	defer cancel()
...
func (realSystem) Run(ctx context.Context, name string, args ...string) error {
	// A fix that fails reports "exit status 1" and nothing else unless the
	// command and what it printed travel with the error; that message is the
	// only thing the operator gets back from `doctor --fix`.
...
func unixDirWritable(path string) bool {
	...
	// Write something rather than read the mode bits: the daemon runs as root,
	// for which the mode says "writable" on a read-only mount and on a full
	// disk alike.
	f, err := os.CreateTemp(path, ".hospitus-doctor-*")
	if err != nil {
		return false
