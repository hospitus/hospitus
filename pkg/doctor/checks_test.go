...
func TestZFSPoolCheck(t *testing.T) {
	...
	// HOSPITUS_ZFS_PARENT override to a different pool. Set the real variable,
	// not the fake one: the check resolves the parent through dataset.Parent(),
	// which is the single source the daemon itself uses.
	t.Setenv("HOSPITUS_ZFS_PARENT", "tank/hospitus")
	if got := c.Run(context.Background(), sys).Status; got != StatusFail {
