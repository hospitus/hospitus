...
func (f *fakeSystem) Sysctl(n string) (string, error) {
	v, ok := f.sysctls[n]
	if !ok {
...
func (f *fakeSystem) KernelModuleLoaded(n string) bool { return f.modules[n] }
func (f *fakeSystem) ServiceEnabled(n string) bool     { return f.services[n] }
...
func (f *fakeSystem) ZFSPoolExists(p string) bool      { return f.pools[p] }
func (f *fakeSystem) PFAnchorDeclared(a string) bool   { return f.pfAnchors[a] }
func (f *fakeSystem) PFFilterRules() []string          { return f.pfRules }
func (f *fakeSystem) BinmiscActivators() []string      { return f.binmisc }
