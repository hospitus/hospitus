	...
	return []Check{
		...
		},
		{
			...
			Run: func(ctx context.Context, s System) Result {
				...
				// One source only: dataset.Parent() is what the daemon itself
				// resolves, so reporting on anything else would describe a layout
				// nothing uses.
				parent := dataset.Parent()
				pool := poolOf(parent)
				if s.ZFSPoolExists(ctx, pool) {
...
func moduleCheck(id, module, desc string, required bool, providers []string) Check {
	...
	return Check{
		...
		Run: func(ctx context.Context, s System) Result {
			...
			if s.KernelModuleLoaded(ctx, module) {
...
func crossArchJailCheck(providers []string) Check {
	return Check{
		...
		Run: func(ctx context.Context, s System) Result {
			...
			if !s.KernelModuleLoaded(ctx, "imgact_binmisc") {
				return Result{
					Status: StatusWarn,
			...
			activators := s.BinmiscActivators(ctx)
...
func podmanPFCheck(providers []string) Check {
	return Check{
		...
		Run: func(ctx context.Context, s System) Result {
			...
			if !s.HasCommand("pfctl") || !s.ServiceEnabled(ctx, "pf_enable") {
	...
	return []Check{
		...
		commandCheck("cmd-pfctl", "pfctl", "PF NAT and port forwarding", false, pf),
		{
			...
			Run: func(ctx context.Context, s System) Result {
				...
				if s.ServiceEnabled(ctx, "pf_enable") {
		...
		},
		{
			...
			Run: func(ctx context.Context, s System) Result {
				...
				if s.PFAnchorDeclared(ctx, "hospitus") {
