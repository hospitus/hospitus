...
func (bm *BackupManager) Configure(instanceID string, config *BackupConfig) error {
	...
	// Reject a schedule we cannot parse instead of silently starting a
	// goroutine that logs an error and exits, leaving backups disabled with no
	...
	if err := bm.persistConfig(instanceID, config); err != nil {
		return err
	}
...
func (bm *BackupManager) Restore(ctx context.Context, backupID, targetInstanceID string) error {
	...
	case BackupTypeFull, BackupTypeIncr:
		...
		if output, err := bm.runRestorePipeline(ctx, recvCmd, source); err != nil {
...
func (bm *BackupManager) removeBackupData(ctx context.Context, b *BackupInfo) error {
	...
	if b.Dataset != "" && b.SnapshotName != "" {
		dataset := fmt.Sprintf("%s@%s", b.Dataset, b.SnapshotName)
		cmd := exec.CommandContext(ctx, "zfs", "destroy", "-r", dataset)
		if err := cmd.Run(); err != nil {
			return fmt.Errorf("failed to delete snapshot %s: %w", dataset, err)
		}
	}
...
func (bm *BackupManager) validateHookPath(hook string) (string, error) {
	...
	if info.Mode()&os.ModeSymlink != 0 {
		// Resolve and re-validate the target directly (avoids a TOCTOU on the link).
		...
		if err != nil || !ti.Mode().IsRegular() {
			return "", fmt.Errorf("hook symlink target is not a regular file: %s", cleaned)
		}
		if !within(target) {
...
func (bm *BackupManager) findPreviousSnapshot(instanceID string) string {
	...
	for i := len(backups) - 1; i >= 0; i-- {
		...
		return fmt.Sprintf("%s@%s", b.Dataset, b.SnapshotName)
	}
...
// instead of shell strings to prevent command injection (CWE-78).
func (bm *BackupManager) runRestorePipeline(ctx context.Context, recvCmd *exec.Cmd, source string) ([]byte, error) {
	...
	sshCmd := exec.CommandContext(ctx, "ssh", "--", sshHost, "zfs", "send", remotePath)
