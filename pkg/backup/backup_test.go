...
func TestParseSchedule(t *testing.T) {
	tests := []struct {
		...
		{"24h", 24 * time.Hour},
		{"30m", 30 * time.Minute},
		// ParseDuration accepts a sign and zero; both must map to 0, or
		// runSchedule hands a non-positive interval to time.NewTicker.
		{"-1h", 0},
		{"-30m", 0},
		{"0s", 0},
		{"invalid", 0},
...
// TestRetentionMinAgeSurvivesARestart covers the only guard that protects
// recent backups from pruning: dropped on persistence, retention silently got
// harsher after every restart.
func TestRetentionMinAgeSurvivesARestart(t *testing.T) {
	cfg := &BackupConfig{
		Enabled:  true,
		Schedule: "daily",
		Retention: RetentionPolicy{
	...
	stored := storedFromConfig(cfg)
	if stored.MinAge == "" {
		t.Fatal("MinAge was not persisted")
	}
	back := configFromStored(stored)
	if back.Retention.MinAge != 6*time.Hour {
		t.Errorf("MinAge came back as %v, want 6h", back.Retention.MinAge)
	}
	if back.Retention.KeepLast != 3 {
		t.Errorf("KeepLast came back as %d, want 3", back.Retention.KeepLast)
	}
}
