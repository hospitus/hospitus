...
func TestVerifyStreamCatchesADamagedFile(t *testing.T) {
	...
	// The .zst cases below run zstd. Without this guard a host that has no zstd
	// still "passes": the missing binary is itself an error, so the assertion
	// holds while nothing under test ran.
	if _, err := exec.LookPath("zstd"); err != nil {
		t.Skip("zstd is not installed; the stream-integrity cases need it")
	}
