...
var waivedFields = map[string]string{
	...
	// No "instances." twin for either: canonical strips that prefix before
	// the lookup, so those entries were never reachable.
	"image.registry": "no example pulls from a private registry",

	// Deliberately absent. Examples are copied verbatim, so an example that
	// sets a password publishes it — and nuageinit has no expiry field for
	// the FreeBSD path, so nothing would force a change at first login. The
	// field is supported; demonstrating it is what is refused.
	"cloud_init.users.plain_text_passwd": "an example password is a published password",
}
...
func definedKeys(t *testing.T, templatePath string) []string {
	...
	rendered, err := RenderTemplate(data, exampleVars("coverage"), &mockSecretStore{}, "workload")
	if err != nil {
		t.Fatalf("render %s: %v", templatePath, err)
	}

	var target any
	// Through the parser's own detection, not a substring search: "[stack]"
	// in a workload's comment or inside a cloud-init block is not a section
	// header, and this test would have decoded that manifest as the wrong kind.
	kind, err := (&Parser{}).detectManifestType(rendered)
	if err != nil {
		t.Fatalf("detect %s: %v", templatePath, err)
	}
	if kind == "stack" {
