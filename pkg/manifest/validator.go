...
func (v *Validator) validateStack(m *StackManifest) ValidationErrors {
	...
	for i := range m.Instances {
		...
		errs = append(errs, v.validateImage(inst.Image, inst.Provider, "")...)
		...
		errs = append(errs, v.validateLifecycle(inst.Lifecycle)...)
	}
...
func (v *Validator) validateImage(img ImageSpec, providerType, osType string) ValidationErrors {
	...
	if img.Arch != "" && !contains(validArchs, img.Arch) {
		errs = append(errs, ValidationError{
			Field:   "image.arch",
...
func (v *Validator) validateNetwork(n NetworkSpec, prefix string) ValidationErrors {
	...
	if n.Type != "" && !contains(validTypes, n.Type) {
		errs = append(errs, ValidationError{
			Field:   prefix + ".type",
	...
	if n.IP != nil {
		...
		if n.IP.Mode != "" && !contains(validModes, n.IP.Mode) {
			errs = append(errs, ValidationError{
...
func (v *Validator) validateStorage(s StorageSpec) ValidationErrors {
	...
	if s.RootDisk != nil {
		...
		if s.RootDisk.Type != "" && !contains(validTypes, s.RootDisk.Type) {
			errs = append(errs, ValidationError{
	...
	for i, vol := range s.Volumes {
		...
		if vol.ZFS != nil {
			...
			if vol.ZFS.Compression != "" && !contains(validCompressions, vol.ZFS.Compression) {
				errs = append(errs, ValidationError{
...
func (v *Validator) validateLifecycle(l LifecycleSpec) ValidationErrors {
	var errs ValidationErrors

	if l.Autostart != nil {
		if l.Autostart.Priority < 0 || l.Autostart.Priority > 100 {
			errs = append(errs, ValidationError{Field: "lifecycle.autostart.priority", Message: "priority must be 0-100"})
		}

		if l.Autostart.Delay != "" {
			_, err := ParseDuration(l.Autostart.Delay)
			if err != nil {
				errs = append(errs, ValidationError{Field: "lifecycle.autostart.delay", Message: err.Error()})
			}
		}
	}
...
func (v *Validator) isValidProvider(provider string) bool {
	return contains(v.SupportedProviders, provider)
}
