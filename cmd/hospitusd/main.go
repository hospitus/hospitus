...
func main() {
	...
	if *migrateOnly || *migrationStatus {
		...
		// --migrate rewrites the schema a running daemon is reading, so it
		// claims the database the same way the daemon does. --migration-status
		// only reads, and takes the lock too rather than report a schema
		// version that is being changed as it prints. This used to run before
		// any lock at all.
		unlockDB, err := lockOne(*dbPath+".lock", *dbPath)
		if err != nil {
			slog.Error("Cannot run migrations", logging.FieldError, err)
			os.Exit(1)
		}
	...
	run := func() error {
		...
		unlockDataDir, err := lockDataDir(*dataDir, *dbPath, *stateDir)
		if err != nil {
...
func providerSettings() map[string]interface{} {
	...
	if err != nil {
		// Said, not swallowed: without this the daemon starts with no
		// allow-list at all, and nothing explains why a disk it should accept
		// is refused.
		slog.Warn("could not load the configuration; provider settings use defaults", logging.FieldError, err)
		return settings
	}
	if cfg == nil {
		return settings
