...
func unresolved(t *testing.T, words []string) string {
	...
	// Deliberately not requiring the final command to be runnable. This
	// resolves command *paths*, which the reference page writes as headings
	// ("### `hospitus jail snapshot`") and the installation guide writes as a
	// completion demo ("hospitus jail <TAB>"). Twenty such lines are correct
	// documentation, and none of them is an instruction anyone types; an
	// incomplete instruction is caught by TestDocumentedCommandsParse, which
	// puts whole lines through cobra.
	return ""
}
		...
		for scanner.Scan() {
			...
			for _, w := range fields[1:] {
				if branches := strings.Split(w, "|"); len(branches) > 1 {
					// An empty branch and a trailing "..." are punctuation, not
					// command names: "create|list|info|..." was rejected whole
					// because of its last branch, which is exactly the dense
					// summary this check exists for.
					var named []string
					allNames := true
