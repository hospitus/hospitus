...
func run() int {
	...
	if err := rootCmd.ExecuteContext(ctx); err != nil {
		// A command run inside an instance reports the guest's own status.
		// That is the outcome a script is waiting on, and replacing it with a
		// blanket 1 threw away the only thing the caller asked for. No
		// message either: the guest command has already written its own, and
		// "Run 'hospitus --help' for usage" is nonsense for a program that
		// merely exited non-zero.
		if code, ok := cmdutil.ExitCodeFrom(err); ok {
			return code
		}
...
// isLocalOnly reports whether a command does its work without the daemon.
//
// "context" edits the config file, and "man" and "completion" write text: none
// of them needs a client, and building one made them fail on a machine with a
// bad context or an unreachable daemon — "hospitus completion bash" refusing to
// print a completion script is a shell that will not start.
func isLocalOnly(target *cobra.Command) bool {
	for c := target; c != nil; c = c.Parent() {
		switch c.Name() {
		case "context", "man", "completion":
			return true
		}
	}
	return false
...
Then update the man database:
  ...
  doas makewhatis /usr/local/man`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// Every command in the tree, not just this one: GenManTree walks from
		// the root, and DisableAutoGenTag is read per command. Set only here,
		// each of the other 100-odd pages carried a generation date, so two
		// builds of the same commit produced different man pages.
...
// disableAutoGenTag clears the generation-date footer on a command and every
// command below it.
