...
func runCreate(cmd *cobra.Command, args []string) error {
	...
	// "native" is resolved to the host architecture, and only that. An empty
	// --arch stays empty: with an explicit --image the architecture belongs to
	// the image name, and filling in the host's contradicted a jail created
	// from an arm64 image on an amd64 host. The image branch below supplies
	// the host value where it needs one.
	effectiveArch := arch
	if effectiveArch == "native" {
		effectiveArch = nativeArch()
	}
	...
	if image == "" && version != "" {
		...
		// A name needs an architecture, so this is where an omitted --arch
		// becomes the host's.
		imageArch := effectiveArch
		if imageArch == "" {
			imageArch = nativeArch()
		}
		image = fmt.Sprintf("freebsd-%s-%s", version, imageArch)
		if effectiveArch == "" {
			effectiveArch = imageArch
		}
	}
	...
	if err != nil {
		...
		if cmdutil.IsImageNotFoundError(err) && image != "" {
			// Whether the image can be fetched is the daemon's answer, not
			// ours: this used to open the catalog under the CLI's own
			// HOSPITUS_IMAGE_DIR, so against a remote daemon — where that
			// directory is empty or absent — a perfectly fetchable image was
			// reported as "not in the available catalog". FetchImage says so
			// itself when the name really is unknown.
			fmt.Fprintf(cmd.OutOrStdout(), "\nImage '%s' is not downloaded.\n", image)
			...
			if fetchErr := cmdutil.APIClient.FetchImage(ctx, image, cmd.OutOrStdout()); fetchErr != nil {
				return fmt.Errorf("failed to download image %q: %w\n  → 'hospitus image available' lists what the daemon can fetch", image, fetchErr)
			}
