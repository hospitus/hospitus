...
func runServiceAction(cmd *cobra.Command, jailName, serviceName, action string) error {
	...
	if !result.Success {
		// Printing "failed" and exiting 0 told every script calling this that
		// the service had started.
		if result.Message != "" {
			return fmt.Errorf("%s of service %s failed: %s", action, serviceName, result.Message)
		}
		return fmt.Errorf("%s of service %s failed", action, serviceName)
	}
...
func runServiceList(cmd *cobra.Command, jailName string, showAll, showEnabled, showRunning bool) error {
	...
	// An explicit filter wins over --all, which defaults to true: gating on
	// !showAll meant "--running" changed nothing unless the caller also passed
	// "--all=false", and the command silently listed everything.
	var filter string
	switch {
	case showRunning:
		filter = "running"
