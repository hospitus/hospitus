...
Note: Some changes require a jail restart to take effect.`,
	...
	cmd.Flags().IntVar(&bootPriority, cmdutil.FlagBootAutoStartPriority, -1, "Boot priority (1-100, lower starts first)")

	return cmd
}

// autostartFailure names what did land when auto-start is the half that fails.
//
// The spec update has already been applied and reported by then; an error that
// only said "failed to set auto-start" left the reader to guess whether the
// rest had gone through.
func autostartFailure(specUpdated bool, err error) error {
	if specUpdated {
		return fmt.Errorf("%w (the other settings were updated)", err)
	}
	return err
}
...
func runSet(cmd *cobra.Command, args []string,
	...
	// On the flag being given, not on its value: --description "" is how a
	// description gets cleared, and testing the string made that a no-op.
	...
	// The daemon accepts 1-100; zero is AutoStartConfig's "not specified"
	// value, which the provider replaces with the default 50. Bounded on the
	// value the caller actually supplied: bootPriority defaults to -1 as the
	// "not given" sentinel, so an unguarded test would reject every
	// invocation that leaves it alone.
	if cmd.Flags().Changed(cmdutil.FlagBootAutoStartPriority) && (bootPriority < 1 || bootPriority > 100) {
		return fmt.Errorf("--%s must be between 1 and 100, got %d", cmdutil.FlagBootAutoStartPriority, bootPriority)
	}

	if !hasChanges && !autostartRequested {
		fmt.Fprintln(cmd.OutOrStdout(), "No changes specified. Use --help to see available options.")
		return nil
	}

	// The spec update goes first. These are two API calls and nothing makes
	// them one transaction, so the order decides what a failure leaves behind:
	// this way a failing update changes nothing at all, where setting
	// auto-start first left it applied under an error that said the jail had
	// not been updated.
	if hasChanges {
		if _, err = cmdutil.APIClient.UpdateInstance(ctx, jailName, req); err != nil {
	...
	if autostartRequested {
		...
		if err != nil {
			...
			// default.
			return autostartFailure(hasChanges, fmt.Errorf("failed to read the current auto-start configuration: %w", err))
		}
		if current != nil {
		...
		if _, err := cmdutil.APIClient.SetAutoStart(ctx, "jail", jailName, cfg); err != nil {
			return autostartFailure(hasChanges, fmt.Errorf("failed to set auto-start: %w", err))
		}
	...
	if hasChanges && (setAllowRawSockets || setAllowSysvipc || setAllowMlock || setAllowReservedPorts || setAllowMount || setPersist) {
