...
func runTmuxAttach(cmd *cobra.Command, args []string) error {
	...
	sessions, err := listTmuxSessions(jailName)
	if err != nil {
		return err
	}
	sessionExists := false
...
// runInJail runs a command inside the jail, returning its stdout and stderr.
//
// Every tmux helper needs the two apart: the exit status alone cannot tell
// "tmux said no" from "jexec could not enter the jail", and treating the
// second as the first is what made a stopped jail report that tmux was not
// installed.
func runInJail(jailName string, args ...string) (stdout, stderr string, err error) {
	jexecPath, lookErr := exec.LookPath("jexec")
	if lookErr != nil {
		return "", "", lookErr
	}
	var outBuf, errBuf bytes.Buffer
	c := exec.Command(jexecPath, append([]string{jailName}, args...)...)
	c.Stdout = &outBuf
	c.Stderr = &errBuf
	err = c.Run()
...
// jexecFailed reports whether jexec itself refused, rather than the command it
// was asked to run. jexec prefixes its own diagnostics with "jexec:".
func jexecFailed(stderr string) bool {
	return strings.Contains(stderr, "jexec:")
}

// checkTmuxInstalled checks if tmux is installed in the jail
func checkTmuxInstalled(jailName string) (bool, error) {
	_, stderr, err := runInJail(jailName, "which", "tmux")
	if err != nil {
		if jexecFailed(stderr) {
			return false, fmt.Errorf("cannot enter jail %s: %s", jailName, strings.TrimSpace(stderr))
		}
		// "which" found nothing: tmux really is absent.
		return false, nil
	}
	return true, nil
}
...
func listTmuxSessions(jailName string) ([]string, error) {
	output, stderr, err := runInJail(jailName, "tmux", "list-sessions", "-F", "#{session_name}")
	if err != nil {
		if jexecFailed(stderr) {
			return nil, fmt.Errorf("cannot enter jail %s: %s", jailName, strings.TrimSpace(stderr))
		}
		return []string{}, nil // No server running: no sessions
	}
...
func listTmuxSessionsFull(jailName string) ([]tmuxSessionInfo, error) {
	...
	output, stderr, err := runInJail(jailName, "tmux", "list-sessions", "-F", "#{session_name}:#{session_windows}:#{session_attached}:#{session_created}")
	if err != nil {
		if jexecFailed(stderr) {
			return nil, fmt.Errorf("cannot enter jail %s: %s", jailName, strings.TrimSpace(stderr))
		}
		return []tmuxSessionInfo{}, nil // No server running: no sessions
	}
