...
Examples:
  ...
  hospitus jail snapshot list --pattern "web*"      # List snapshots for jails matching "web*"`,
	...
	// The shared flag, like every other listing command here: a local
	// declaration is a second definition of --output to keep in step.
...
func runSnapshotList(cmd *cobra.Command, args []string, outputFormat string, all bool, pattern string) error {
	...
	case all || pattern != "":
		// Before the listing, not inside the loop: with no jails to iterate,
		// filepath.Match never ran and "--pattern [" reported "No jails
		// matching" and exited 0 rather than naming the broken pattern.
		if pattern != "" {
	...
	for _, jailName := range jailNames {
		...
		if err != nil {
			...
			if multipleJails {
				// Silently, this reported "No snapshots found" when in fact
				// nothing had been listed at all.
				fmt.Fprintf(cmd.ErrOrStderr(), "warning: could not list snapshots for %s: %v\n", jailName, err)
				failed = append(failed, jailName)
				continue
			}
	...
	if len(failed) == len(jailNames) {
		return fmt.Errorf("could not list snapshots for any of the %d jails", len(jailNames))
	}
