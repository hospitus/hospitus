...
Examples:
  ...
  hospitus qemu media insert myvm --path /path/to/image.iso --readonly`,
	...
	// Read back from the command rather than bound to package variables: those
	// outlive one invocation, so a second command in the same process — and
	// every test after the first — inherited whatever the previous one set.
	cmd.Flags().String("path", "", "Path to the ISO or media file (required)")
	cmd.Flags().Bool("readonly", true, "Mount media as read-only")
	cmd.Flags().Bool("bootable", false, "Mark media as bootable")
...
func runMediaInsert(cmd *cobra.Command, args []string) error {
	...
	mediaPath, _ := cmd.Flags().GetString("path")
	...
	fmt.Fprintf(cmd.OutOrStdout(), "Media inserted successfully into %s\n", instanceName)
	if deviceID != "" {
...
func runMediaEject(cmd *cobra.Command, args []string) error {
	...
	fmt.Fprintf(cmd.OutOrStdout(), "Media ejected from device %s on %s\n", deviceID, instanceName)
