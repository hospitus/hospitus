...
func newBootOrderCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "boot-order",
		Short: "Manage boot device order for a QEMU VM",
		Long: `View or change the firmware boot device order for a QEMU VM.
...
func newBootOrderGetCommand() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// ResolveInstanceNameStrict checks the provider itself, so the
			// RequireInstanceOf that stood here asked the daemon the same
			// question twice on every invocation.
			resolvedName, err := cmdutil.ResolveInstanceNameStrict(ctx, cmdutil.APIClient, vmName, "qemu")
			if err != nil {
			...
			if order == nil || len(order.Devices) == 0 {
				// A one-time override with no standing order is the case an
				// installation boot leaves behind, and reporting only
				// "firmware default" hid the very thing the next boot will do.
				if order != nil && order.OnceDevice != "" {
					fmt.Fprintf(cmd.OutOrStdout(), "Boot order: (firmware default)\nNext boot only: %s\n", order.OnceDevice)
					return nil
				}
...
func newBootOrderSetCommand() *cobra.Command {
	return &cobra.Command{
		...
		Long: `Set the boot device priority order for a QEMU VM.

Devices are tried in the order listed. Valid device types:
  disk     - Primary hard disk / virtual disk image
