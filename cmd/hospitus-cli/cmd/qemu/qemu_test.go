...
func (m *mockQemuClient) GetInstance(_ context.Context, id string) (*datastore.Instance, error) {
	if m.getInstance == nil && m.getInstanceErr == nil {
		// A real daemon always names the provider. Returning nil made the
		// resolver report none, and a qemu command refuses a target it
		// cannot confirm — so every test that did not set this field was
		// asserting on a path the CLI no longer takes.
		return &datastore.Instance{ID: id, Name: id, Provider: "qemu"}, nil
	}
...
func TestExposeListCommand_PrintsNoRulesMessage(t *testing.T) {
	...
	// Case-insensitive: the message opens the sentence, so its first word is
	// capitalised, and asserting on the lowercase form tied this test to a
	// wording rather than to what it means.
	out := buf.String()
	if !strings.Contains(strings.ToLower(out), "no port forwarding rules") {
		t.Errorf("expected a 'no port forwarding rules' message, got: %s", out)
	}
	// And no table header above it.
	if strings.Contains(out, "PROTOCOL") {
		t.Errorf("an empty list printed a table header: %s", out)
	}
}
