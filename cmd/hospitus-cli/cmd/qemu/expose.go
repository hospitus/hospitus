...
func runQemuExposeList(cmd *cobra.Command, args []string) error {
	...
	// printed the table. Before the empty case, so a caller parsing the
	// output gets [] rather than an English sentence.
	...
	// The empty case first: printing a heading and an empty table before
	// saying there is nothing gave the reader two contradictory impressions
	// in a row.
	if len(rules) == 0 {
