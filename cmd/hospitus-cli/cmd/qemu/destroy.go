...
func runDestroy(cmd *cobra.Command, args []string) error {
	...
	case pattern != "":
		...
		for _, inst := range instances {
			// The name as well as the id, and the help says "VM names": a
			// pattern tested only against ids matched nothing where the two
			// differ, and the command reported no match at all.
