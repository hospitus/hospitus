...
func printScopes(out, errOut io.Writer, store *manifest.FileSecretStore, scopes []string) error {
	listed := false
	failed := 0
	for _, scope := range scopes {
		secrets, err := store.List(scope)
		if err != nil {
			// stderr, and counted: printing the failure on stdout mixed it
			// into the listing, and exiting 0 told a script every scope had
			// been read.
			fmt.Fprintf(errOut, "Error listing scope %s: %v\n", scope, err)
			failed++
			continue
		}
		if len(secrets) > 0 {
	...
	if failed > 0 {
		return fmt.Errorf("%d of %d scopes could not be listed", failed, len(scopes))
	}
	if !listed {
...
Common scopes are workload or stack names.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return printScopes(out, cmd.ErrOrStderr(), store, scopes)
		},
	}
