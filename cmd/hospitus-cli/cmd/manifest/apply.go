...
func buildDependencyOrder(instances []manifest.InstanceConfig) ([]manifest.InstanceConfig, error) {
	...
	for i := range instances {
		inst := &instances[i]
		// A repeated name overwrote the first instance in byName and merged
		// their dependencies, so one of the two was silently dropped from the
		// order — and the loss showed up later as a missing instance, or as a
		// cycle that is not in the manifest.
		if _, seen := byName[inst.Name]; seen {
			return nil, fmt.Errorf("two instances are both named %q", inst.Name)
		}
