...
func deleteWorkload(ctx context.Context, workload *manifest.WorkloadManifest, force, yes, keepVolumes bool, namespace string) error {
	...
	// The instance's ID, not its name: applyManagedVolumes scopes every volume
	// it creates to instance.ID, so computing the same names from the name
	// deleted nothing when the two differ — leaving full-size datasets behind
	// — and, worse, named whatever volume another instance happened to own
	// under that prefix. Falling back to the name keeps the old behavior for
	// an instance already gone, where there is no ID left to ask for.
	volumePrefix := name
	if instance, instErr := apiClient.GetInstance(ctx, name); instErr == nil && instance != nil && instance.ID != "" {
		volumePrefix = instance.ID
	}

	// A volume holds the workload's data and outlives its instance, so say what
	// is going before asking rather than after.
	volumes := managedVolumes(workload, volumePrefix)
	if keepVolumes {
		volumes = nil
	}
...
func deleteStack(ctx context.Context, stack *manifest.StackManifest, force, yes, keepVolumes bool, namespace string) error {
	...
	for i := range order {
		...
		// The storage travels with it: managedVolumes reads Storage.Volumes,
		// and a synthetic manifest without it reported no volumes at all, so a
		// stack's declared volumes were left behind as full-size datasets.
