...
func runDestroy(cmd *cobra.Command, args []string) error {
	...
	if len(containersToDestroy) == 1 {
		fmt.Printf("WARNING: This will permanently destroy container '%s' and all its data.\n", containersToDestroy[0])
	} else {
		fmt.Printf("WARNING: This will permanently destroy %d containers and all their data:\n", len(containersToDestroy))
