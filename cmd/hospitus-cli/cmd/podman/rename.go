...
Examples:
  hospitus podman rename old-web new-web`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			ctx := cmd.Context()
			oldName, err := cmdutil.ResolveInstanceNameStrict(ctx, cmdutil.APIClient, args[0], providerName)
			if err != nil {
				return fmt.Errorf("container not found: %w", err)
			}
			newName := args[1]

			result, err := cmdutil.APIClient.RenameInstance(ctx, oldName, newName)
			if err != nil {
