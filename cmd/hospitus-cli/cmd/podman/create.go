...
func runPodmanCreate(cmd *cobra.Command, args []string) error {
	...
	for _, p := range ports {
		...
		if len(containerParts) == 2 {
			...
			// Checked here like the ports are: anything else reached the
			// daemon and came back as an opaque failure, or was silently
			// treated as TCP.
			if protocol != "tcp" && protocol != "udp" {
				return fmt.Errorf("invalid port mapping %q: protocol must be tcp or udp", p)
			}
		}
