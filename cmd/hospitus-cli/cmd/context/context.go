...
func newContextListCommand() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := cmdutil.LoadConfig(configPathOf(cmd))
			if err != nil {
				return err
			}
...
func newContextUseCommand() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := cmdutil.SaveConfig(configPathOf(cmd), cfg); err != nil {
				return err
			}
...
// readAPIKey prompts for a key, without echoing it at a terminal.
//
// The key was read through a plain reader, so it appeared on screen as it was
// typed and stayed in the scrollback of whatever terminal was used. A pipe has
// no terminal to disable echo on, and is read as before, which is what
// "hospitus context add … --api-key - < keyfile" relies on.
...
// configPathOf returns the --config value the caller gave.
//
// The flag is declared persistently on the root command and every context
// subcommand passed "" instead, so "hospitus --config other.yaml context add"
// read and wrote the default file — silently editing a configuration the
// caller had explicitly steered away from.
