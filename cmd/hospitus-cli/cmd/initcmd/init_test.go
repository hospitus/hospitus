...
func TestSelectedProviders(t *testing.T) {
	...
	got, err := selectedProviders([]string{"Jail", " bhyve "})
	if err != nil {
		t.Fatalf("selectedProviders: %v", err)
	}
	if !got["jail"] || !got["bhyve"] || len(got) != 2 {
		t.Errorf("provider set = %v", got)
	}

	// A name that is not a provider matches no check, so it has to be refused
	// rather than quietly diagnosing nothing.
	if _, err := selectedProviders([]string{"jial"}); err == nil {
