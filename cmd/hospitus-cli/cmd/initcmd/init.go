...
anchor lines to add and manages its own anchors.`,
	...
	// --check reports and stops; --auto applies fixes. Asking for both is a
	// contradiction, and --check silently won.
	cmd.MarkFlagsMutuallyExclusive("check", "auto")
	return cmd
}
...
func runInitWithRunner(ctx context.Context, out io.Writer, in io.Reader, runner *doctor.Runner, checkOnly, auto bool, providers []string, requireRoot func() error) error {
	...
	selected, err := selectedProviders(providers)
	if err != nil {
		return err
	}
	...
	// What is fixable first, root second. Asked in the other order, a host
	// with nothing left to fix refused to say so — "this command requires
	// root privileges" for work it was not going to do.
...
// knownProviders is what --provider accepts, in the order the flag's help
// lists them.
var knownProviders = []string{"jail", "bhyve", "qemu", "podman", "vfkit", "container"}

// selectedProviders turns the --provider flag into a set. Empty means all.
//
// A name that is not a provider is refused rather than added to the set: an
// unknown key matches no check, so "--provider jial" silently diagnosed
// nothing and reported everything as fine.
func selectedProviders(providers []string) (map[string]bool, error) {
	if len(providers) == 0 {
		return nil, nil
	}
	...
	for _, p := range providers {
		...
		if !slices.Contains(knownProviders, name) {
			return nil, fmt.Errorf("unknown provider %q: expected one of %s",
