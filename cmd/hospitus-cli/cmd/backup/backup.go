...
func runBackupConfig(cmd *cobra.Command, args []string) error {
	...
	// Start from what is configured, then overlay only the flags the caller
	// actually passed. SetBackupConfig replaces the whole configuration, so
	// building the request from flag defaults meant "--keep-daily 7" also blanked
	// the destination, the schedule, the compression and every other retention
	// count the operator had set.
	req, err := existingBackupConfig(ctx, instanceID)
	if err != nil {
		return err
	}

	// Read at the call site rather than through a helper: the dead-flag check
	// looks for Flags().Get…("name") in the source, and a helper taking the
	// name as an argument hides every one of these from it.
	...
	case cmd.Flags().Changed("disable"):
		...
		req.Enabled = !disable
	case cmd.Flags().Changed("schedule"):
		// A schedule is what makes a backup happen; setting one enables it and
		// clearing one turns it off.
		req.Enabled = req.Schedule != ""
	}
...
// existingBackupConfig reads the instance's current configuration as a request
// the caller can amend.
//
// The daemon answers with a JSON object; round-tripping it through the request
// type reuses the field tags rather than repeating the mapping here. An
// instance with nothing configured yet answers with an empty object, which is
// the zero request and exactly right.
func existingBackupConfig(ctx context.Context, instanceID string) (client.BackupConfigRequest, error) {
	var req client.BackupConfigRequest

	config, err := cmdutil.APIClient.GetBackupConfig(ctx, instanceID)
	if err != nil {
		return req, fmt.Errorf("failed to read the current backup configuration: %w", err)
	}
