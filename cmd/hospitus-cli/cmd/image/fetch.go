...
func imageProviders(name string) []string {
	...
	// Filtered by platform: jails and bhyve are FreeBSD-only, and offering
	// them anywhere else sends the reader after a provider the daemon does not
	// register (see registerProviders in cmd/hospitusd). Darwin was special-
	// cased and every other non-FreeBSD platform — Linux — still got them.
	switch runtime.GOOS {
	case "freebsd":
		return []string{"jail", "bhyve", "qemu", "podman"}
	case "darwin":
		return []string{"qemu", "podman", "vfkit", "container"}
	default:
		return []string{"qemu", "podman"}
	}
}
