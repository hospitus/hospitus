...
func runAvailable(cmd *cobra.Command, args []string) error {
	...
	// JSON before the empty case: "--output json" with no match printed an
	// English sentence, which a caller parsing the output has to special-case.
	// An empty result encodes as [].
	if format == cmdutil.OutputFormatJSON {
		if profiles == nil {
			profiles = []image.ImageProfile{}
		}
		return outputAvailableJSON(cmd.OutOrStdout(), profiles)
	}

	if len(profiles) == 0 {
		fmt.Fprintln(cmd.OutOrStdout(), "No images found matching the criteria.")
		return nil
	}

	return outputAvailableTable(cmd.OutOrStdout(), profiles)
}

func outputAvailableTable(out io.Writer, profiles []image.ImageProfile) error {
...
func outputAvailableJSON(out io.Writer, profiles []image.ImageProfile) error {
