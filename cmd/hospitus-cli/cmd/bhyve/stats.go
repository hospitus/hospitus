...
func newStatsCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short:   "Show resource usage statistics for a bhyve VM",
		Long: `Show resource usage statistics for a bhyve VM.

Displays CPU usage, memory, disk I/O, and network I/O statistics
from the hospitus daemon.

Examples:
  hospitus bhyve stats myvm
  hospitus bhyve stats myvm --watch
  hospitus bhyve stats myvm --watch --interval 5`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			return runStats(cmd, args, watch, interval)
		},
	}
...
func runStats(cmd *cobra.Command, args []string, watch bool, interval int) error {
	vmName := args[0]
	...
	if watch {
		if interval < 1 {
			return fmt.Errorf("--interval must be at least 1 second, got %d", interval)
		}
		...
		for {
			if err := displayStats(cmd, vmName); err != nil {
				return err
			}
			select {
			case <-ctx.Done():
	...
	return displayStats(cmd, vmName)
}

func displayStats(cmd *cobra.Command, vmName string) error {
	ctx := cmd.Context()
