...
func runVNC(cmd *cobra.Command, args []string, showInfo bool, viewerPath string, enable, disable bool, port, width, height int, host string, wait bool) error {
	...
	// The provider names each VM's directory by the instance id, while the
	// operator types whatever the listing shows. Where the two differ, the
	// literal name found no directory and the command reported the VM as
	// absent from a host it was sitting on.
	//
	// Resolved only when the daemon answers: this is the one bhyve command
	// that works entirely off the local data directory, and making it require
	// a running daemon would take "vnc --info" away exactly when a stopped VM
	// is being looked at.
	if resolved, err := cmdutil.ResolveInstanceNameStrict(cmd.Context(), cmdutil.APIClient, vmName, "bhyve"); err == nil {
		vmName = resolved
	}
