...
func runConsole(cmd *cobra.Command, args []string, showDevice bool) error {
	// Resolved before it becomes a path: the argument is interpolated into
	// /dev/nmdm-<name>B and then opened, so a name carrying a slash or ".."
	// names a different device entirely.
	vmName, err := cmdutil.ResolveInstanceNameStrict(cmd.Context(), cmdutil.APIClient, args[0], "bhyve")
	if err != nil {
	...
	if err != nil {
		// The wrapped error says whether cu is absent or merely unusable — a
		// permission problem on the binary reads identically otherwise.
		return fmt.Errorf("'cu' command not found - install it with: pkg install cu: %w", err)
	}
