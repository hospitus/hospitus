...
func runBhyveExposeAdd(cmd *cobra.Command, args []string, portSpec, targetIP string) error {
	...
	if err := cmdutil.RequireInstanceOf(ctx, api, vmName, "bhyve"); err != nil {
		return err
	}
	...
	result, err := api.ExposePort(ctx, vmName, req)
	if err != nil {
...
func runBhyveExposeRemove(cmd *cobra.Command, args []string, hostPort int, protocol string) error {
	...
	if err := api.UnexposePort(ctx, vmName, hostPort, string(proto)); err != nil {
...
func runBhyveExposeList(cmd *cobra.Command, args []string) error {
	...
	ports, err := api.ListExposedPorts(ctx, vmName)
	if err != nil {
