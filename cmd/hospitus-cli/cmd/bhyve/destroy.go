...
func newDestroyCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short:   "Destroy one or more bhyve VMs",
...
func runDestroy(cmd *cobra.Command, args []string) error {
	...
	case pattern != "":
		...
		for _, inst := range instances {
			// The name as well as the id: an operator types what the listing
			// shows, and a pattern tested only against ids silently matched
			// nothing where the two differ.
			matched, err := filepath.Match(pattern, inst.ID)
			if err != nil {
				return fmt.Errorf("invalid pattern '%s': %w", pattern, err)
			}
			if !matched && inst.Name != "" {
	...
	case len(args) > 0:
		for _, arg := range args {
			resolved, err := cmdutil.ResolveInstanceNameStrict(ctx, cmdutil.APIClient, arg, "bhyve")
			if err != nil {
	...
	if !yes {
		// Through the command's own streams: os.Stdin and fmt.Print bypass
		// whatever a caller — or a test — set on the command, so this prompt
		// could not be driven and its output went somewhere else than the
		// rest.
