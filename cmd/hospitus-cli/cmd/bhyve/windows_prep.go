...
Examples:
  ...
  hospitus bhyve windows-prep win11 --windows-iso ~/Win11.iso --output /tmp/bypass.iso`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			vmName := args[0]
			// Checked here as well as resolved seventy lines below: this
			// command generates an ISO before it touches the API, and failing
			// after that work rather than before it wastes the operator's
			// time on a VM that does not exist.
			//
			// Not with --skip-attach, though: that flag exists to prepare the
			// ISO for a VM the operator will attach by hand — often one that
			// does not exist yet — and this command then never reaches the
			// API at all.
			if !skipAttach {
			...
			if outputISO == "" {
				...
				// Removed only if the ISO is never produced. On success this
				// directory holds the deliverable — its path is printed below
				// and the VM boots from it — so cleaning up unconditionally
				// would delete what the command was asked to make. Nothing
				// removed it before, which left a directory behind on every
				// failed run.
				defer func() {
					if isoProduced {
						return
