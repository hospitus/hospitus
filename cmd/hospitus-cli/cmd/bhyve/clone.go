...
Examples:
  ...
  hospitus bhyve clone base-template web-server-1 --linked`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// A nil result with no error is a daemon answering something this
			// client cannot read; printing from it panics.
			if result == nil {
				return fmt.Errorf("the daemon reported no clone for %s", args[0])
			}
