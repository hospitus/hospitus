...
// withMockClient points the package-level API client at mock for the duration
// of one test.
//
// Nine tests wrote the save, the assignment and the restoring defer out by
// hand; one that forgets the defer leaks its mock into every test that runs
// after it.
func withMockClient(t *testing.T, mock cmdutil.APIClientInterface) {
	t.Helper()

	original := cmdutil.APIClient
	cmdutil.APIClient = mock
	t.Cleanup(func() { cmdutil.APIClient = original })
}
