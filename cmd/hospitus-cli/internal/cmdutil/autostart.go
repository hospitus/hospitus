...
Priority (1-100, default 50): lower numbers start first.
Delay: milliseconds to wait before starting this %s.`, noun, noun),
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			// 1, not 0: zero is AutoStartConfig's "not specified" value, which
			// the provider replaces with the default 50 — so --priority 0 was
			// accepted and then quietly ignored.
			if priority < 1 || priority > 100 {
				return fmt.Errorf("priority must be between 1 and 100, got %d", priority)
			}
			if delay < 0 {
	...
	cmd.Flags().IntVar(&priority, "priority", 50, "Start priority (1-100, lower starts first)")
