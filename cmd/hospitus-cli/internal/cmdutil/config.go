...
func SaveConfig(path string, cfg *Config) error {
	...
	// truncated config the temp-and-rename dance exists to prevent.
	...
	// The directory entry too: tmp.Sync above persists the contents, and the
	// rename that gives them their final name lives here. Its failure is
	// returned rather than ignored: this function promises the config is on
	// disk when it returns.
	dir, err := os.Open(filepath.Dir(path))
	if err != nil {
		return fmt.Errorf("failed to open the config directory: %w", err)
	}
	if err := dir.Sync(); err != nil {
		dir.Close()
		return fmt.Errorf("failed to flush the config directory: %w", err)
	}
	if err := dir.Close(); err != nil {
		return fmt.Errorf("failed to close the config directory: %w", err)
	}
	return nil
}
