...
// nothing.
// resolveCommand walks as far down the command tree as the words go.
//
// The longest prefix that names commands, not all-or-nothing: a documented
// line carries its arguments too, and "jail create myjail" used to resolve to
// nil at "myjail" — so every example with a positional argument, which is most
// of them, had its flags checked against nothing at all.
func resolveCommand(words []string) *cobra.Command {
	...
	if cmd == rootCmd {
		// Not a hospitus command at all; checking flags against the root would
...
func commandPathOf(line string) string {
	...
	for len(fields) > 0 && strings.HasPrefix(fields[0], "-") {
		...
		declared := rootCmd.PersistentFlags().Lookup(name)
		fields = fields[1:]
		// An undeclared leading flag means the line is not one this helper can
		// read: stepping over it, and possibly over its value, left that value
		// being taken for the command name. Reported by
		// TestDocumentedCommandsParse, which puts the whole line through
		// cobra; here it only has to stop guessing.
		if declared == nil {
			return ""
		}
		// A global flag that takes a value swallows the next word too, unless
		// it was written as --flag=value.
		if !strings.Contains(flag, "=") && len(fields) > 0 && declared.Value.Type() != "bool" {
			fields = fields[1:]
		}
	}
