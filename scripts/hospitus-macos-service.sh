...
# The agent serves plain HTTP and prints an http:// URL for the CLI. Off
# loopback that puts the API key on the wire in the clear, so this installer
# stays local-only; a listener the network can reach needs TLS, which means
# configuring the daemon rather than running this script.
case "${ADDR%:*}" in
127.0.0.1 | localhost | "[::1]" | ::1) ;;
*) die "refusing to bind ${ADDR}: this installer serves plain HTTP and only supports a loopback address" ;;
esac
...
install)
	...
	# mkdir applies the caller's umask, so these land 0755 and every local user
	# can list the database, the runtime state and the VM disks. Not $AGENT_DIR:
	# launchd has to traverse it.
	chmod 0700 "$DATA_DIR" "$STATE_DIR" "$LOG_DIR"
	# Marks the directory as ours for hospitus-purge.sh.
	: > "$DATA_DIR/.hospitus-data-dir"
	...
	# A plist is XML: a path holding "&" or "<" has to be escaped for the
	# document before it is escaped for sed, or plutil rejects the result.
	xml_escape() { printf '%s' "$1" | sed -e 's/&/\&amp;/g' -e 's/</\&lt;/g' -e 's/>/\&gt;/g'; }
	sed_escape() { printf '%s' "$(xml_escape "$1")" | sed -e 's/[\\|&]/\\&/g'; }
