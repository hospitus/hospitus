...
#
# Usage:
#   doas sh scripts/hospitus-purge.sh [-y] [-f] [-k] [-p parent-dataset]
#
#   -y   assume yes; do not prompt before destroying data
#   -f   force: skip the checks that the targets belong to Hospitus
...
# flag or the environment, and a check on the *shape* of a name is not enough:
# "zroot/home" has a slash and "/etc/ssh" is two levels deep, so both would pass
# one. So look at what the target actually holds, and refuse anything that does
# not carry Hospitus's own furniture. -f overrides, for a half-created install.
#
# HOSPITUS_MARKER is also written into the data directory at install time, so a
# directory Hospitus made is recognisable even when it is otherwise empty.
HOSPITUS_MARKER=".hospitus-data-dir"

# A data directory of ours holds at least one of these.
dir_is_ours() {
	_d=$1
	[ -e "$_d/$HOSPITUS_MARKER" ] && return 0
	for _mark in hospitus.db nexus.db state instances api.key; do
		[ -e "$_d/$_mark" ] && return 0
	done
	return 1
}

# A parent dataset of ours has the property, or holds the per-provider children
# Hospitus creates under it.
dataset_is_ours() {
	_ds=$1
	zfs list -H -o name "$_ds" >/dev/null 2>&1 || return 1
	case "$(zfs get -H -o value hospitus:managed "$_ds" 2>/dev/null)" in
	1 | true | yes) return 0 ;;
	esac
	for _child in jails bhyve qemu podman volumes images; do
		zfs list -H -o name "$_ds/$_child" >/dev/null 2>&1 && return 0
	done
	return 1
}
...
guard_targets() {
	if [ "$FORCE" = "1" ]; then
		echo "warning: -f given; destroying $PARENT and $DATA_DIR without checking they are ours" >&2
		return
	fi
	...
	*)
	...
	if [ "$KEEP_DATA" != "1" ] && ! dataset_is_ours "$PARENT"; then
		echo "refusing to destroy $PARENT: no hospitus:managed property and none of the" >&2
		echo "datasets Hospitus creates under it. Use -f if you are certain." >&2
		exit 1
	fi

	for _dir in "$DATA_DIR" "$LOG_DIR"; do
		case "$_dir" in
		*..*)
			echo "refusing to remove $_dir: it contains a parent reference" >&2
			exit 1
			;;
		/*/*) ;;
		*)
			echo "refusing to remove $_dir: expected a path at least two levels deep" >&2
			exit 1
			;;
		esac
	done
	# The log directory is ours by construction (/var/log/hospitus); the data
	# directory is whatever the caller or the environment named, so check it.
	if [ -d "$DATA_DIR" ] && ! dir_is_ours "$DATA_DIR"; then
		echo "refusing to remove $DATA_DIR: it holds nothing Hospitus put there." >&2
		echo "Use -f if you are certain." >&2
		exit 1
	fi
}
