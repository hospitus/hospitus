...
go test ./pkg/validation/ -run TestValidateInstanceName   # single test
go test ./pkg/job/ -run TestManagerErrors -v -count=1     # one test, verbose
```
...
tests run in CI too, but only when `test/integration` is present — the workflow
skips the job on a branch that does not carry the suite yet. It runs
`./test/integration/...` as root in a FreeBSD 15.1 VM against a file-backed ZFS
pool (`HOSPITUS_ZFS_PARENT=hospitusci/hospitus`); only bhyve stays out, for lack
of hardware virtualization in the VM.
