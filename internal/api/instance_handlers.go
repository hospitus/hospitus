...
func (s *Server) handleListInstances(w http.ResponseWriter, r *http.Request) {
	...
	// Concurrently, and each with its own deadline. Serially, the listing cost
	// the sum of every provider query — each one a command against the host —
	// so a single unresponsive provider held up the whole page, and an
	// operator with thirty instances waited on the slowest of them thirty
	// times over.
	// The whole sweep is bounded as well, the way the metrics scrape is: a
	// hundred instances each taking their per-instance budget still outlasts
	// any client's patience, and the stored state is what the rest report.
	sweepCtx, cancelSweep := context.WithTimeout(ctx, listStateSweepTimeout)
	defer cancelSweep()

	var wg sync.WaitGroup
	slots := make(chan struct{}, listStateConcurrency)
	for i := range instances {
		...
		// The slot is taken here, not inside the goroutine: acquiring it after
		// the launch bounds the concurrent calls but not the goroutines, so a
		// host with thousands of instances started thousands of them just to
		// have them block on the channel.
		slots <- struct{}{}
		wg.Add(1)
		go func(i int, prov provider.Provider) {
			defer wg.Done()
			defer func() { <-slots }()
			// net/http recovers a panic on the goroutine serving the request;
			// this one is not that goroutine, so a provider that panics here
			// would take the whole daemon down. Before this loop ran
			// concurrently, the same panic was contained.
			defer func() {
				if p := recover(); p != nil {
					s.logger.Error("Provider panicked while reporting instance state",
						logging.FieldInstance, instances[i].Name,
						logging.FieldProvider, instances[i].Provider,
						"panic", p)
				}
			}()
			// Bounded per instance: without it one provider that never
			// answers still holds the response until the client gives up.
			queryCtx, cancel := context.WithTimeout(sweepCtx, listStateTimeout)
			defer cancel()

			state, err := prov.GetInstanceState(queryCtx, instances[i].Handle)
			if err != nil {
				// Leave the stored state rather than inventing one. A provider
				// that cannot answer says nothing about whether the instance
				// runs. Say so: silence here makes a stale "running"
				// impossible to account for, since the list looks the same
				// either way.
			...
			s.askProviderForAddress(queryCtx, prov, instances[i])
		}(i, prov)
	}
	wg.Wait()
...
// How the instance listing queries providers: in parallel, a few at a time,
// and never waiting on one of them for long.
const (
	listStateConcurrency  = 8
	listStateTimeout      = 5 * time.Second
	listStateSweepTimeout = 15 * time.Second
)
...
func privilegedProviderConfig(config map[string]interface{}) string {
	...
	for _, mount := range mountEntries(configValue(config, "mounts")) {
...
func privilegedProviderConfigKey(key string) bool {
	// Lowercased first. ParseJailParametersFromMap lowercases every key before
	// handing it to jail(8), so "EXEC.START" reached the host as exec.start
	// while this gate — comparing the caller's spelling — saw a key it did not
	// recognize and let a non-admin through.
	key = strings.ToLower(key)

	switch key {
	case "exec.clean", "exec.timeout", "exec.fib", "exec.jail_user":
		return false
	case "path", "devfs_ruleset", "securelevel", "enforce_statfs", "allow.vmm":
		return true
	}
	// The whole mount. namespace, not "mount" and "mount.fstab" alone:
	// mount.devfs, mount.procfs and mount.zfs each hand the jail a filesystem,
	// and naming two of them left the rest unguarded.
	return strings.HasPrefix(key, "exec.") ||
		strings.HasPrefix(key, "allow.mount") ||
		key == "mount" || strings.HasPrefix(key, "mount.")
}
...
// createdInstanceRecord builds the row a freshly created instance is persisted
// as.
//
// One builder for both the streaming and the JSON branch: they wrote the same
// record from the same inputs, and a field added to one of them would have gone
// missing from the other.
func createdInstanceRecord(handle provider.InstanceHandle, spec provider.InstanceSpec, providerName string) *datastore.Instance {
...
func (s *Server) discardCreatedInstance(ctx context.Context, prov provider.Provider, handle provider.InstanceHandle, cause error) error {
	...
	//
	// Callers may therefore pass r.Context() even when it is already canceled:
	// WithoutCancel drops the parent's cancellation AND its deadline, so the
	// minute below is the whole budget either way. Passing an already-detached
	// context here would change nothing.
...
func (s *Server) handleCreateInstance(w http.ResponseWriter, r *http.Request) {
	...
	if existing != nil {
		...
		if stateErr != nil {
			// A provider that cannot answer is not a provider saying "gone":
			// a transient failure took this branch and deleted the record and
			// firewall rules of an instance that was still there.
			s.writeLoggedError(w, http.StatusServiceUnavailable,
				fmt.Sprintf("Cannot verify whether instance %s still exists", spec.Name), stateErr)
			return
		}
		if state == provider.StateUnknown {
			// Ghost instance - clean it up
			s.logger.Warn("Cleaning up ghost instance from datastore", logging.FieldInstance, spec.Name, "state", state)
	...
	wantsStream := acceptsMedia(acceptHeader, "text/plain", "text/event-stream")
	...
	if wantsStream {
		...
		// The flusher first: WriteHeader(200) commits the status, so the 500
		// below it was discarded and net/http logged a superfluous
		// WriteHeader. A client that cannot be streamed to was told 200 and
		// handed an ERROR line to parse out of the body.
		...
		// Store in datastore
		instance := createdInstanceRecord(handle, spec, providerName)

		// Detached: the provider has already created the instance, and a
		// client that hung up cancels this write — whereupon the error path
		// destroys a perfectly good instance as unrecorded.
		persist, cancelPersist := context.WithTimeout(context.WithoutCancel(ctx), time.Minute)
		defer cancelPersist()
		if err := s.datastore.CreateInstance(persist, instance); err != nil {
		...
		// Detached, as in the streaming branch above.
		persist, cancelPersist := context.WithTimeout(context.WithoutCancel(ctx), time.Minute)
		defer cancelPersist()
		if err := s.datastore.CreateInstance(persist, instance); err != nil {
			err = s.discardCreatedInstance(ctx, prov, handle, err)
...
func (s *Server) handleInstanceDetail(w http.ResponseWriter, r *http.Request) {
	...
	case action == "freebsd":
		...
		//
		// extractRouteParts, not the suffix-only parts this switch runs on:
		// handleFreeBSD indexes parts[5] and length-checks against 6, the
		// shape the explicit routes give it. The catch-all handed it
		// ["freebsd","rctl"] and every request through here answered 404.
		s.handleFreeBSD(w, r, instanceID, s.extractRouteParts(r, instanceID))
	case action == "export":
	...
	case action == "metrics":
		// A known resource reached with the wrong method: this fell through to
		// the default and answered 404 for a route that exists.
	...
	case action == "services":
		...
		//
		// extractRouteParts, like the freebsd branch above: handleServices
		// indexes parts[5] and parts[6], the shape its explicit route hands
		// it. The suffix-only parts this switch runs on are three elements
		// long, so a request naming one service listed them all instead.
		s.handleServices(w, r, instanceID, s.extractRouteParts(r, instanceID))
	case action == "volumes":
		// Handle volume attachment operations
		s.handleInstanceVolumes(w, r, instanceID, parts)
	case action == "interfaces":
		// Handle network interface operations. Same convention as services:
		// handleInterfaces reads parts[5].
		s.handleInterfaces(w, r, instanceID, s.extractRouteParts(r, instanceID))
	case action == "backups":
	...
	case action == "":
		// The instance path exists and the method does not: 405 with an Allow
		// header, not 404. Reporting "not found" for a PUT to an instance that
		// is right there sent the caller looking for the wrong problem.
...
func (s *Server) handleInstanceHealthRoute(w http.ResponseWriter, r *http.Request) {
	// Registered without a method prefix, unlike the metrics route, so every
	// method lands here: a POST went straight to the capability check and was
	// answered 501 rather than 405.
...
func (s *Server) handleUpdateInstance(w http.ResponseWriter, r *http.Request, instanceID string) {
	...
	// Checked before the writes below, not after: a provider that cannot
	// reconfigure would take the new provider config no further than the
	// datastore, and the handler answered 200 for a change the instance never
	// received.
	if len(req.ProviderConfig) > 0 {
		prov, err := s.registry.Get(instance.Provider)
		if err != nil {
			s.writeError(w, http.StatusInternalServerError,
				fmt.Sprintf("Provider not found: %s", instance.Provider))
			return
		}
		if _, ok := prov.(provider.ReconfigureProvider); !ok {
			s.writeError(w, http.StatusNotImplemented,
				fmt.Sprintf("Provider %s cannot reconfigure an instance", instance.Provider))
			return
		}
	}
	...
	if len(req.ProviderConfig) > 0 {
		...
		// Written back, not only handed to the provider: the spec that gets
		// persisted below still held the creation-time map, so the next PATCH
		// recomputed "merged" without the entries this one applied and sent
		// the provider a configuration missing them.
		instance.Spec.ProviderConfig = merged
	}
	// Resolved once, and a failure is fatal: the error used to be discarded, so
	// an unregistered provider skipped the whole apply block and the handler
	// then persisted the merged spec and answered 200 — telling the caller a
	// CPU or memory change had been applied that no provider ever received.
	prov, err := s.registry.Get(instance.Provider)
	if err != nil {
		s.writeLoggedError(w, http.StatusInternalServerError,
			fmt.Sprintf("Provider not found: %s", instance.Provider), err)
		return
	}
	if reconfigurable, ok := prov.(provider.ReconfigureProvider); ok {
		if err := reconfigurable.Reconfigure(ctx, instance.Handle, applied); err != nil {
			s.writeLoggedError(w, http.StatusInternalServerError,
				"The provider could not apply the change; nothing was recorded", err)
			return
		}
	...
	// Persisted only once the provider has taken the change. The other order
	// recorded the new spec and handle first, so a Reconfigure that failed left
	// the row describing a configuration the instance never received while the
	// caller was told the update failed.
	//
	// lookupInstance resolves by ID or name, so these writes must use the
	// resolved ID, never the raw path value.
	// Detached, like every other post-provider write in this file: moving these
	// after the provider call is what made the request context wrong for them.
	// A client that disconnects during Reconfigure would otherwise leave the row
	// describing the configuration the instance no longer has.
	postApply, cancelPostApply := context.WithTimeout(context.WithoutCancel(ctx), time.Minute)
	defer cancelPostApply()

	if err := s.datastore.UpdateInstanceSpec(postApply, instance.ID, instance.Spec); err != nil {
		s.writeLoggedError(w, http.StatusInternalServerError,
			"The provider applied the change but it could not be recorded", err)
		return
	}

	if req.ProviderConfig != nil {
		if err := s.datastore.UpdateInstanceHandle(postApply, instance.ID, instance.Handle); err != nil {
			s.writeLoggedError(w, http.StatusInternalServerError,
				"The provider applied the change but the handle could not be recorded", err)
			return
		}
	}
...
func (s *Server) handleDeleteInstance(w http.ResponseWriter, r *http.Request, instanceID string) {
	...
	// Through lookupInstance like every other instance route: this one read
	// the datastore by id alone, so "hospitus delete web" answered 404 for an
	// instance the same CLI could start and stop by that name.
	instance, err := s.lookupInstance(ctx, instanceID)
	if err != nil {
		s.writeLoggedError(w, http.StatusNotFound, "Instance not found", err)
		return
	...
	// The instance is gone from the provider, so neither of these two may ride
	// on a context the client can cancel: a caller that hung up between the
	// provider delete and here left the firewall rules in place and the row
	// behind, describing an instance that no longer exists.
	postDelete, cancelPostDelete := context.WithTimeout(context.WithoutCancel(ctx), time.Minute)
	defer cancelPostDelete()

	// Clean up firewall rules for this instance
	if err := s.cleanupFirewallRules(postDelete, instance.Name); err != nil {
	...
	// By the resolved id, not the path value: deleting by name reached the
	// provider and then left the record behind.
	if err := s.datastore.DeleteInstance(postDelete, instance.ID); err != nil {
...
func (s *Server) handleStartInstance(w http.ResponseWriter, r *http.Request, instanceID string) {
	...
	acceptHeader := r.Header.Get("Accept")
	wantsStream := acceptsMedia(acceptHeader, "text/plain")
	...
	if wantsStream {
		...
		// A context of its own for the writes that follow. The provider can
		// return with the instance running even when the request context is
		// already canceled — a client that hung up mid-start — and both writes
		// then failed, leaving a running instance recorded as stopped with its
		// allocated addresses unsaved.
		postStart, cancelPostStart := context.WithTimeout(context.WithoutCancel(ctx), time.Minute)
		defer cancelPostStart()

		if err := s.datastore.UpdateInstanceState(postStart, instance.ID, provider.StateRunning); err != nil {
		...
		s.syncInstanceSpec(postStart, instance.ID, prov, instance.Handle)
		...
		// Same as the streaming branch: the instance is running, so these two
		// writes must not ride on a context the client may have canceled.
		postStart, cancelPostStart := context.WithTimeout(context.WithoutCancel(ctx), time.Minute)
		defer cancelPostStart()
...
func (s *Server) handleStopInstance(w http.ResponseWriter, r *http.Request, instanceID string) {
	...
	// Detached, like the start paths: the instance is already stopped, and a
	// client that hung up during the stop canceled the context this write
	// would have ridden on — leaving the row saying "running".
	postStop, cancelPostStop := context.WithTimeout(context.WithoutCancel(ctx), time.Minute)
	defer cancelPostStop()

	if err := s.datastore.UpdateInstanceState(postStop, instance.ID, provider.StateStopped); err != nil {
...
func (s *Server) handleRestartInstance(w http.ResponseWriter, r *http.Request, instanceID string) {
	...
	// Recorded, as start and stop both do: restarting a stopped instance left
	// the row saying "stopped" while it ran, until some later sync corrected
	// it. On a context of its own, for the same reason they use one.
	postRestart, cancelPostRestart := context.WithTimeout(context.WithoutCancel(ctx), time.Minute)
	defer cancelPostRestart()

	if err := s.datastore.UpdateInstanceState(postRestart, instance.ID, provider.StateRunning); err != nil {
		s.logger.Warn("Failed to update instance state after restart", "instance", instance.ID, "error", err)
	}
...
// configValue reads one provider-config key regardless of how the caller
// capitalised it, for the same reason privilegedProviderConfigKey lowercases:
// the provider reads these case-insensitively, so the gate has to as well.
func configValue(config map[string]interface{}, want string) interface{} {
	if v, ok := config[want]; ok {
		return v
	}
