...
func TestHandleListAutoStart_Empty(t *testing.T) {
	...
	// The values, not just the key: checking presence alone passed for any
	// list at all, including one this server could not have produced.
	var resp struct {
		Instances []AutoStartInstanceInfo `json:"instances"`
	...
	if len(resp.Instances) != 0 {
		t.Errorf("instances = %+v, want none: no provider here implements auto-start", resp.Instances)
	}
	if resp.Count != 0 {
...
func TestHandleTriggerAutoStart_NoProviders(t *testing.T) {
	...
	// 202 with a job id: the trigger is queued rather than awaited, because
	// StartAutoStartInstances brings instances up with delays between them and
	// held the request open for as long as that took.
	if w.Code != http.StatusAccepted {
		t.Fatalf("POST /api/v1/autostart: expected 202, got %d body=%s", w.Code, w.Body.String())
	}
	...
	if got.JobID == "" {
		t.Error("the response names no job to poll")
	}
}
