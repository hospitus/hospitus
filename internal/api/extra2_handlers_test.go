...
func TestHandleTriggerAutoStart_NoProviders(t *testing.T) {
	...
	if w.Code != http.StatusOK && w.Code != http.StatusPartialContent {
		t.Fatalf("POST /api/v1/autostart: expected 200 or 206, got %d body=%s", w.Code, w.Body.String())
	}
}

func TestHandleAutoStartInstance_InvalidPath(t *testing.T) {
	s, ds := setupTestServer(t)
	defer ds.Close()
...
func TestHandleDeleteBackup_NoManager(t *testing.T) {
	s, ds := setupTestServer(t)
	defer ds.Close()
	...
	// No backup manager in test server → may get 503, 404, 500 etc.
	if w.Code < 400 {
		t.Fatalf("DELETE /api/v1/backups/someid: expected 4xx/5xx, got %d", w.Code)
	}
}
...
func TestHandleListExposedPorts_NoFirewall(t *testing.T) {
	...
	if w.Code != http.StatusServiceUnavailable && w.Code != http.StatusBadRequest {
		t.Fatalf("GET /api/v1/firewall/expose/myjail: expected 503 or 400, got %d body=%s", w.Code, w.Body.String())
	}
}
...
func TestHandleRemoveNAT_NoFirewall(t *testing.T) {
	...
	if w.Code != http.StatusServiceUnavailable && w.Code != http.StatusBadRequest {
		t.Fatalf("DELETE /api/v1/firewall/nat: expected 503 or 400, got %d body=%s", w.Code, w.Body.String())
	}
}
...
func TestHandleCloneFromSnapshot_NotFound(t *testing.T) {
	...
	// Should be 404 (instance not found) or 400/405
	if w.Code != http.StatusNotFound && w.Code != http.StatusBadRequest && w.Code != http.StatusMethodNotAllowed {
		t.Fatalf("POST clone-snapshot: expected 4xx, got %d body=%s", w.Code, w.Body.String())
	}
}
