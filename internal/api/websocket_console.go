...
func (s *Server) handleWebSocketConsole(w http.ResponseWriter, r *http.Request, instanceID string) {
	...
	if open := s.countConsoleSessions(); open >= maxConsoleSessions {
		console.logger.Warn("Refusing console session: too many are open",
			"open", open, "limit", maxConsoleSessions)
...
// countConsoleSessions reports how many consoles are currently open.
func (s *Server) countConsoleSessions() int {
	n := 0
	s.activeConsoleSessions.Range(func(_, _ any) bool {
		n++
		return true
	})
	return n
