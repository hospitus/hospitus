...
// TestLoggingMiddleware_AuthRequired verifies what a non-health path with auth
// enabled and a key present records.
//
// KEY_PRESENT, not AUTHENTICATED: this runs before the key is verified, so
// every 401 for a wrong key was logged as an authenticated request. Checking
// only the 200 would have passed after a regression to the old value.
func TestLoggingMiddleware_AuthRequired(t *testing.T) {
	...
	if !strings.Contains(logged.String(), "auth_status=KEY_PRESENT") {
		t.Errorf("auth status not recorded as KEY_PRESENT; log was: %s", logged.String())
	}
}
...
// TestHijackSurvivesTheWholeMiddlewareChain covers the WebSocket console
// upgrade through every wrapper, not just loggingResponseWriter.
//
// recoveryMiddleware wraps the writer in committedWriter, which sits UNDER
// loggingResponseWriter. Without a Hijack of its own, the logging wrapper
// delegated to a writer that is not an http.Hijacker, and every console
// upgrade failed in production while the single-wrapper test stayed green.
func TestHijackSurvivesTheWholeMiddlewareChain(t *testing.T) {
	// Both conditional wrappers enabled: withMiddleware adds authMiddleware and
	// rateLimitMiddleware only behind their flags, so an empty config left the
	// test covering a shorter chain than the one production serves. The limits
	// are set explicitly because this helper does not apply NewServer's
	// defaults.
	s := newMiddlewareServer(&ServerConfig{
		EnableAuth:        true,
	...
	handler := s.withMiddleware(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
	...
	// /health, which authMiddleware forwards without credentials.
	req := httptest.NewRequest(http.MethodGet, "/health", nil)
	handler.ServeHTTP(&hijackRecorder{ResponseRecorder: httptest.NewRecorder()}, req)

	if !gotHijacker {
		t.Fatal("the handler's ResponseWriter is not an http.Hijacker: the console cannot upgrade")
	}
	if hijackErr != nil {
		t.Fatalf("Hijack through the chain: %v", hijackErr)
	}
}

// hijackRecorder is an httptest.ResponseRecorder that can be hijacked, which
// the real server's writer can and the recorder alone cannot.
