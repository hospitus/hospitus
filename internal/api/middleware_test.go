...
// TestExtractClientIP_TrustedProxy_XForwardedFor verifies that the first IP in
// X-Forwarded-For is returned when the request comes from a trusted proxy.
func TestExtractClientIP_TrustedProxy_XForwardedFor(t *testing.T) {
	...
	ip := s.extractClientIP(req)
	if ip != "203.0.113.10" {
		t.Errorf("expected 203.0.113.10 from X-Forwarded-For, got %q", ip)
	}
}
