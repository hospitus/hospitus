...
func (s *Server) handleExec(w http.ResponseWriter, r *http.Request, instanceID string) {
	...
	// A timeout, always. Zero means "no timeout" to the providers, and zero is
	// what an omitted field decodes to — so a command that never returns held
	// a jexec process and this request open for the life of the daemon. Both
	// the streaming and the non-streaming path go through here.
	// Clamped, not reset: asking for one second past the ceiling used to drop
	// the run to the 300-second default — a twelvefold cut, unannounced.
	if req.Timeout <= 0 {
		req.Timeout = defaultExecTimeoutSeconds
	}
	if req.Timeout > maxExecTimeoutSeconds {
		req.Timeout = maxExecTimeoutSeconds
	}

	// Check if client wants streaming (Accept: text/plain)
	acceptHeader := r.Header.Get("Accept")
	wantsStreaming := acceptsMedia(acceptHeader, "text/plain")
	...
	security.GetGlobalAuditLogger().LogResourceAccess(clientIP, s.auditIdentity(r), "exec", fmt.Sprintf("instance:%s command:%s", instance.Name, req.Command))
...
func (s *Server) handleExecStreaming(w http.ResponseWriter, r *http.Request, instance *datastore.Instance, streamProv provider.ExecStreamingProvider, req provider.ExecOptions) {
	...
	// no-store, not just no-cache: no-cache lets a shared cache keep the body
	// and revalidate, and the body here is the output of a command run inside
	// someone's instance.
	...
	if !ok {
		// The marker goes out too: the client reads the stream until it sees
		// an EXIT_CODE line, and returning without one left it waiting on a
		// response that had already ended.
...
// Exec timeouts, in seconds. The default is generous enough for a package
// install; the ceiling stops a caller asking for a command that outlives the
// daemon.
const (
