...
func TestValidateUserFilePathConfinement(t *testing.T) {
	...
	// The sibling has to exist: validateUserFilePath resolves symlinks, so
	// against a missing directory the case above fails on resolution rather
	// than on the confinement check it is named for.
	if err := os.MkdirAll(base+"-evil", 0o755); err != nil {
		t.Fatal(err)
	}
	// t.TempDir removes base, not its sibling: every run left one behind.
	t.Cleanup(func() { _ = os.RemoveAll(base + "-evil") })
