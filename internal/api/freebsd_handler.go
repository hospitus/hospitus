...
func (s *Server) handleFreeBSD(w http.ResponseWriter, r *http.Request, instanceID string, parts []string) {
	...
	// lookupInstance, not GetInstance: this was the one instance route that
	// resolved by id alone, so "hospitus ... web" worked for start, stop and
	// media and answered 404 here.
	...
	// Exactly six segments, and only the two features above. "< 6" accepted
	// anything deeper: /freebsd/rctl/extra dispatched as rctl, and
	// /freebsd/vnet/disable reached handleVnet, which routes on the method
	// alone — so a POST to it ENABLED VNET. The verb in the path was never
	// read; there are no seven-part routes here.
	if len(parts) != 6 || (parts[5] != "rctl" && parts[5] != "vnet") {
...
func (s *Server) handleGetRctl(w http.ResponseWriter, r *http.Request, instanceID string, instance *datastore.Instance, rctlProv provider.RctlProvider) {
	...
	security.GetGlobalAuditLogger().LogResourceAccess(clientIP, s.auditIdentity(r), "GET", fmt.Sprintf("/api/v1/instances/%s/freebsd/rctl", instanceID))
...
func (s *Server) handleSetRctl(w http.ResponseWriter, r *http.Request, instanceID string, instance *datastore.Instance, rctlProv provider.RctlProvider) {
	...
	if err := rctlProv.SetResourceLimits(r.Context(), instance.Handle, req.Limits); err != nil {
		// A limit the caller got wrong is a 400: every rejected resource,
		// action and amount came back as a server failure.
		if errors.Is(err, provider.ErrInvalidResourceLimit) {
			s.writeLoggedError(w, http.StatusBadRequest, "Invalid resource limit", err)
			return
		}
...
func (s *Server) handleGetVnet(w http.ResponseWriter, r *http.Request, instanceID string, instance *datastore.Instance, vnetProv provider.VNETProvider) {
	...
	// Log access
	clientIP := s.extractClientIP(r)
	security.GetGlobalAuditLogger().LogResourceAccess(clientIP, s.auditIdentity(r), "GET", fmt.Sprintf("/api/v1/instances/%s/freebsd/vnet", instanceID))
...
func (s *Server) handleEnableVnet(w http.ResponseWriter, r *http.Request, instanceID string, instance *datastore.Instance, vnetProv provider.VNETProvider) {
	...
	// This endpoint enables; EnableVNET returns nil without doing anything when
	// Enabled is false, so a body that omits the field — or sets it false — was
	// answered with a success and an ENABLE_VNET audit entry for a jail whose
	// VNET was untouched.
	config.Enabled = true

	if err := vnetProv.EnableVNET(r.Context(), instance.Handle, config); err != nil {
		// A malformed bridge, address or route is the caller's mistake.
		if errors.Is(err, provider.ErrInvalidVNETConfig) {
			s.writeLoggedError(w, http.StatusBadRequest, "Invalid VNET configuration", err)
			return
		}
