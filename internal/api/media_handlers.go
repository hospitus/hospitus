...
func (s *Server) handleMedia(w http.ResponseWriter, r *http.Request, instanceID string, parts []string) {
	...
	// Parse path: /api/v1/instances/{id}/media[/{device}]
	//
	// The device segment belongs to DELETE, and to exactly one segment: the
	// eject branch tested "len(parts) < 6" and then read parts[5], so a DELETE
	// to .../media/cd0/extra ejected cd0 — a device the caller never named.
	deviceID := ""
	if len(parts) > 5 {
		if r.Method != http.MethodDelete || len(parts) != 6 {
