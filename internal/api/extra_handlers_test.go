...
func TestJobNotFound(t *testing.T) {
	...
	// No ID-format validation anywhere in job_handlers.go: an unknown id is
	// 404 whatever it looks like.
	if w.Code != http.StatusNotFound {
		t.Errorf("GET /api/v1/jobs/nonexistent: expected 404, got %d", w.Code)
	}
}
...
func TestInstanceMetricsEndpoint(t *testing.T) {
	...
	// The mock provider answers metrics, so 200 is the one reachable outcome
	// here. Accepting 501 as well pinned nothing.
	if w.Code != http.StatusOK {
		t.Errorf("GET /api/v1/instances/{id}/metrics: expected 200, got %d body=%s", w.Code, w.Body.String())
	}
}
...
func TestNetworkBridgesEndpoint(t *testing.T) {
	...
	// The bridge lister answers on every platform, empty where there is no
	// ZFS or no bridges. 200 is the only outcome this can take.
	if w.Code != http.StatusOK {
		t.Errorf("GET /api/v1/network/bridges: expected 200, got %d body=%s", w.Code, w.Body.String())
	}
}
...
func TestDeleteJobEndpoint(t *testing.T) {
	...
	// A job neither the manager nor the datastore knows: 404.
	req := httptest.NewRequest(http.MethodDelete, "/api/v1/jobs/nonexistent-job-id", nil)
	w := httptest.NewRecorder()
	s.mux.ServeHTTP(w, req)

	if w.Code != http.StatusNotFound {
		t.Errorf("DELETE /api/v1/jobs/nonexistent: expected 404, got %d", w.Code)
	}
}
