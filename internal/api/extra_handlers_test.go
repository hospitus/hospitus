...
func TestExecEndpointMissingCommand(t *testing.T) {
	s, ds := setupTestServer(t)
	defer ds.Close()
	...
	// Mock provider doesn't support exec, so 501 expected
	if w.Code != http.StatusNotImplemented && w.Code != http.StatusBadRequest {
		t.Errorf("exec missing command: expected 400 or 501, got %d body=%s", w.Code, w.Body.String())
	}
}
...
func TestAutoStartListEndpoint(t *testing.T) {
	s, ds := setupTestServer(t)
	defer ds.Close()
	...
	if w.Code != http.StatusOK {
		t.Errorf("GET /api/v1/autostart: expected 200, got %d body=%s", w.Code, w.Body.String())
	}
}
