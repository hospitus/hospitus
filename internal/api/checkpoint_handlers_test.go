...
func setupTestServerWithProvider(t *testing.T, prov provider.Provider) (*Server, *datastore.Datastore) {
	...
	// NewServer starts job-manager workers. The one captured here, not
	// server.jobManager read at cleanup time: some callers swap in their own
	// manager and shut down only the replacement, which left this one running
	// for the rest of the package.
	created := server.jobManager
	t.Cleanup(func() { created.Shutdown(2 * time.Second) })

	// Confine export/import paths to a temp dir so tests may use /tmp targets.
	// t.TempDir, not os.TempDir: the latter is one process-wide directory that
	// every test built by this helper shared, nothing cleaned up what the
	// export and import handlers wrote there, and two tests picking the same
	// target name interfered in an order-dependent way.
...
func TestHandleCheckpoint_Create_OK(t *testing.T) {
	cp := &mockCheckpointProvider{mockProvider: newMockProvider()}
	server, ds := setupTestServerWithProvider(t, cp)
	createTestInstanceInDS(t, ds, "vm1", "mock")
...
func TestHandlePauseInstance_OK(t *testing.T) {
	pp := &mockPauseProvider{mockProvider: newMockProvider()}
...
func TestHandleRenameInstance_OK(t *testing.T) {
	rp := &mockRenameProvider{mockProvider: newMockProvider()}
	...
	// The status is not the point of this handler: the row has to carry the
	// new id AND a handle that names the new instance. A handle left naming
	// the old one makes every later handle operation miss, and delete destroys
	// the dataset it names.
	renamed, err := ds.GetInstance(context.Background(), "vm2")
	if err != nil {
		t.Fatalf("GetInstance(vm2): %v", err)
	}
	if renamed == nil {
		t.Fatal("instance vm2 not found after rename")
	}
	if renamed.Name != "vm2" {
		t.Errorf("name = %q, want vm2", renamed.Name)
	}
	if renamed.Handle.ID != "vm2" {
		t.Errorf("handle id = %q, want vm2 — the handle still names the old instance", renamed.Handle.ID)
	}

	old, err := ds.GetInstance(context.Background(), "vm1")
	if err == nil && old != nil {
		t.Error("the old id still resolves after rename")
	}
}
...
func TestHandleAutoStartInstance_SetConfig(t *testing.T) {
	ap := &mockAutoStartProvider{mockProvider: newMockProvider()}
	server, _ := setupTestServerWithProvider(t, ap)
