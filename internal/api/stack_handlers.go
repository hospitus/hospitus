...
func (s *Server) handleDeployStack(w http.ResponseWriter, r *http.Request) {
	...
	parsed, err := parser.ParseInScope([]byte(req.Manifest), "api-upload", apiUploadSecretScope, req.Vars)
	if err != nil {
...
// apiUploadSecretScope is the secret scope every manifest posted to the API is
// rendered in, whatever it calls itself.
const apiUploadSecretScope = "api-upload"
