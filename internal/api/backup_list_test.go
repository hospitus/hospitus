...
func TestListAllBackupsReturnsWhatExists(t *testing.T) {
	...
	// Which ones, not only how many: the endpoint aggregates across instances,
	// so listing one backup twice is the regression to catch, and a count of
	// two would not.
	got := make(map[string]string, len(listed))
	for _, b := range listed {
		got[b.ID] = b.InstanceID
	}
	want := map[string]string{"web-1": "web", "db-1": "db"}
	if len(listed) != len(want) {
		t.Fatalf("listed %d backups, want %d: %v", len(listed), len(want), got)
	}
	for id, instance := range want {
		if got[id] != instance {
			t.Errorf("backup %s listed for instance %q, want %q", id, got[id], instance)
		}
	}
}
