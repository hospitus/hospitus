...
func TestRefusePrivilegedConfigCoversTheManifestShape(t *testing.T) {
	srv := &Server{config: &ServerConfig{}, logger: slog.Default()}
...
// TestStackDeployCallsThePrivilegedConfigGate drives the deploy endpoint.
//
// The test above exercises refusePrivilegedConfig directly, which cannot show
// that the deploy path calls it: removing that call would leave it green. This
// one posts a stack whose instance asks for a host hook and requires the
// endpoint itself to refuse. The gate runs before anything is deployed, so no
// provider is touched.
func TestStackDeployCallsThePrivilegedConfigGate(t *testing.T) {
...
# A nullfs mount of the host root: the converter turns this into the
# "mounts" entry refusePrivilegedConfig refuses to a write-scoped key.
[[instances.storage.volumes]]
name = "host"
host_path = "/"
mount_path = "/host"
`
	body, err := json.Marshal(map[string]string{"manifest": stack})
	if err != nil {
		t.Fatal(err)
	}
	...
	req = req.WithContext(context.WithValue(req.Context(), contextKeyPermissions, []string{"write"}))
