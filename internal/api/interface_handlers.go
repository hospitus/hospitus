...
func (s *Server) handleInterfaces(w http.ResponseWriter, r *http.Request, instanceID string, parts []string) {
	...
	//
	// Exactly six segments for a named interface. ">= 6" accepted a deeper
	// path, so a DELETE to .../interfaces/epair0b/extra removed epair0b — an
	// interface the caller never named.
	interfaceName := ""
	switch {
	case len(parts) == 6:
...
func (s *Server) handleListInterfaces(w http.ResponseWriter, r *http.Request, jailProv *jail.JailProvider, handle provider.InstanceHandle) {
	...
	if err != nil {
		// Logged whole, answered generically — not writeLoggedError, which
		// forwards clientErrorDetail. The error here quotes ifconfig output:
		// host interface names, bridge members, addresses on interfaces the
		// jail is not on. The operator reads that in the daemon log.
		s.logger.Error("failed to list interfaces", logging.FieldError, err)
		s.writeError(w, http.StatusInternalServerError, "Failed to list interfaces")
		return
	}
...
func (s *Server) handleAddInterface(w http.ResponseWriter, r *http.Request, jailProv *jail.JailProvider, handle provider.InstanceHandle) {
	...
	if err != nil {
		// Logged, not returned, like the listing above: the error quotes
		// ifconfig and names host interfaces.
		s.logger.Error("failed to add interface", logging.FieldError, err)
		s.writeError(w, http.StatusInternalServerError, "Failed to add interface")
		return
	}
...
func (s *Server) handleRemoveInterface(w http.ResponseWriter, r *http.Request, jailProv *jail.JailProvider, handle provider.InstanceHandle, interfaceName string) {
	...
	if err := jailProv.RemoveNetworkInterface(ctx, handle, interfaceName); err != nil {
		s.logger.Error("failed to remove interface", logging.FieldError, err)
		s.writeError(w, http.StatusInternalServerError, "Failed to remove interface")
		return
	}
