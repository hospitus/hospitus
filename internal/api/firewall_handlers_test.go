...
// The name used to say "InvalidBody", but the body never gets read: the nil
// firewall manager refuses first, which is what this actually covers.
//
// Not a duplicate of TestHandleUnexposePort_NoFirewall in
// extra2_handlers_test.go, which sends valid JSON: this one sends a body that
// could not be decoded, so a pass proves the guard runs *before* the decode.
func TestHandleUnexposePort_NoFirewallManager(t *testing.T) {
	srv, ds := setupTestServer(t)
	defer ds.Close()
