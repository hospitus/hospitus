...
func (s *Server) handleSnapshots(w http.ResponseWriter, r *http.Request, instanceID string, parts []string) {
	...
	// lookupInstance, not GetInstance: every sibling instance route resolves
	// this same path segment by id or name, and a name answered 404 here.
	...
	case len(parts) == 1:
		...
		switch r.Method {
		case http.MethodGet:
			s.handleListSnapshots(w, r, instance, snapProvider)
		case http.MethodPost:
			s.handleCreateSnapshot(w, r, instance, snapProvider)
		default:
	...
	case len(parts) == 2:
		...
		if r.Method == http.MethodDelete {
			s.handleDeleteSnapshot(w, r, instance, snapshotName, snapProvider)
		} else {
	...
	case len(parts) == 3 && parts[2] == "restore":
		...
		s.handleRestoreSnapshot(w, r, instance, parts[1], snapProvider)
...
// findSnapshot resolves a caller-supplied snapshot name against the provider's
// own list, answering the request when it cannot.
//
// One copy of what delete, restore and clone-from-snapshot each spelled out:
// list every snapshot, scan for the name, 404 when it is not there.
func (s *Server) findSnapshot(w http.ResponseWriter, r *http.Request, instance *datastore.Instance, snapshotName string, snapProvider provider.SnapshotProvider) (*provider.SnapshotInfo, bool) {
...
func (s *Server) handleListSnapshots(w http.ResponseWriter, r *http.Request, instance *datastore.Instance, snapProvider provider.SnapshotProvider) {
	...
	security.GetGlobalAuditLogger().LogResourceAccess(clientIP, s.auditIdentity(r), "LIST", fmt.Sprintf("/api/v1/instances/%s/snapshots", instanceID))
...
func (s *Server) handleCreateSnapshot(w http.ResponseWriter, r *http.Request, instance *datastore.Instance, snapProvider provider.SnapshotProvider) {
	instanceID := instance.ID
	// Parse request body
	var req struct {
...
func (s *Server) handleDeleteSnapshot(w http.ResponseWriter, r *http.Request, instance *datastore.Instance, snapshotName string, snapProvider provider.SnapshotProvider) {
	instanceID := instance.ID
	// SECURITY: Validate snapshot name
...
func (s *Server) handleRestoreSnapshot(w http.ResponseWriter, r *http.Request, instance *datastore.Instance, snapshotName string, snapProvider provider.SnapshotProvider) {
	instanceID := instance.ID
	// SECURITY: Validate snapshot name
	if err := validation.ValidateSnapshotName(snapshotName); err != nil {
		s.writeLoggedError(w, http.StatusBadRequest, "Invalid snapshot name", err)
