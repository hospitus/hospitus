...
// decodeInstanceList reads the instance array a list response carries.
//
// One shape, not two: handleListInstances answers with a bare array, and the
// object fallback that stood here would have quietly accepted a response shape
// the API does not produce.
func decodeInstanceList(t *testing.T, w *httptest.ResponseRecorder) []datastore.Instance {
	t.Helper()

	var listed []datastore.Instance
...
func TestInstancesPostCreatesInstance(t *testing.T) {
	...
	// The one reachable outcome: this body takes the non-streaming branch,
	// which answers 201 after the row is written.
	if w.Code != http.StatusCreated {
		t.Fatalf("POST /api/v1/instances: expected 201, got %d body=%s", w.Code, w.Body.String())
	}

	// And it has to be there: 201 alone would pass for a handler that created
	// the instance on the provider and stored nothing.
	stored, err := ds.GetInstanceByName(context.Background(), "post-test-vm")
	if err != nil {
		t.Fatalf("GetInstanceByName: %v", err)
	}
	if stored == nil {
		t.Fatal("the created instance was not persisted")
	}
	if stored.Provider != "mock" {
		t.Errorf("stored provider = %q, want mock", stored.Provider)
	}
}
