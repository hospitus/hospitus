...
func (s *Server) handleListJobs(w http.ResponseWriter, r *http.Request) {
	...
	// Try datastore first for persisted jobs
	jobs, err := s.datastore.ListJobs(ctx, statusFilter)
	if err != nil {
		s.logger.Warn("Failed to list jobs from datastore, falling back to in-memory", "error", err)
		jobs = s.jobManager.ListJobs(statusFilter)
	}

	// If datastore returned nothing, fall back to in-memory
	if len(jobs) == 0 {
		jobs = s.jobManager.ListJobs(statusFilter)
	}
	...
	records := make([]job.JobRecord, 0, len(jobs))
	for _, j := range jobs {
