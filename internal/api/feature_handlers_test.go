...
func TestGetAutoStartInstance(t *testing.T) {
	...
	// mockProvider implements no AutoStartProvider, so 501 is the one outcome
	// this route can reach. Rejecting only 500 passed on any answer at all.
	if w.Code != http.StatusNotImplemented {
		t.Errorf("autostart get = %d, want 501; body=%s", w.Code, w.Body.String())
	}
}
...
// TestInstanceMetricsResolvedByID covers the metrics route for a record whose
// ID is its bare name, which is the other half of what lookupInstance resolves:
// TestInstanceMetricsEndpoint uses makeTestInstance, whose ID is "name@provider"
// and which therefore reaches the instance by name.
func TestInstanceMetricsResolvedByID(t *testing.T) {
	s, ds := setupTestServer(t)
	defer ds.Close()
	...
	// mockProvider answers metrics, so 200 is the one reachable outcome; the
	// old name said "NotImplemented" while the assertion accepted anything
	// but 500.
	if w.Code != http.StatusOK {
		t.Errorf("metrics = %d, want 200; body=%s", w.Code, w.Body.String())
	}
}
