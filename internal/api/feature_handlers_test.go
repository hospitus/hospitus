...
func TestPauseInstanceNotImplemented(t *testing.T) {
	...
	if w.Code != http.StatusNotImplemented && w.Code != http.StatusOK {
		t.Errorf("pause: expected 501 or 200, got %d body=%s", w.Code, w.Body.String())
	}
}
...
func TestRenameInstanceNotImplemented(t *testing.T) {
	...
	if w.Code != http.StatusNotImplemented && w.Code != http.StatusOK {
		t.Errorf("rename: expected 501 or 200, got %d body=%s", w.Code, w.Body.String())
	}
}
...
func TestCloneNotImplemented(t *testing.T) {
	...
	if w.Code != http.StatusNotImplemented && w.Code != http.StatusAccepted {
		t.Errorf("clone: expected 501 or 202, got %d body=%s", w.Code, w.Body.String())
	}
}
...
func TestExportNotImplemented(t *testing.T) {
	...
	// Export requires POST with an export_path
	body, _ := json.Marshal(map[string]interface{}{
		"export_path": "/tmp/export-test.tar",
		"compress":    false,
	})
	...
	if w.Code != http.StatusNotImplemented && w.Code != http.StatusAccepted && w.Code != http.StatusBadRequest {
		t.Errorf("export: got %d body=%s", w.Code, w.Body.String())
	}
}
...
func TestExportInstanceNotFound(t *testing.T) {
	...
	if w.Code != http.StatusNotFound && w.Code != http.StatusBadRequest {
		t.Errorf("export ghost: expected 404 or 400, got %d body=%s", w.Code, w.Body.String())
	}
}
...
func TestInstanceBackupsNoManager(t *testing.T) {
	...
	// Without a backup manager the handler must still return a well-formed
	// response (either an error or a 200 with an empty array), never a crash.
	if w.Code == 0 {
		t.Fatalf("expected a response status code, got none")
	}
}
...
func TestSnapshotNotImplemented(t *testing.T) {
	...
	if w.Code != http.StatusNotImplemented && w.Code != http.StatusCreated {
		t.Errorf("snapshot create: expected 501/201, got %d body=%s", w.Code, w.Body.String())
	}
}
...
func TestSnapshotListNotImplemented(t *testing.T) {
	...
	if w.Code != http.StatusNotImplemented && w.Code != http.StatusOK {
		t.Errorf("snapshot list: got %d body=%s", w.Code, w.Body.String())
	}
}
...
func TestExposeListInstanceNotFound(t *testing.T) {
	...
	// Ghost instance should not return 200
	if w.Code == http.StatusOK {
		t.Errorf("expose ghost: should not return 200, got 200")
	}
}
...
func TestMetricsInstanceNotFound(t *testing.T) {
	...
	if w.Code != http.StatusNotFound && w.Code != http.StatusOK {
		t.Errorf("metrics ghost: got %d body=%s", w.Code, w.Body.String())
	}
}
