...
// Tests for stack handlers. Most paths exercise the nil-stackManager guard
// (setup via setupTestServer which does not init a stackManager). This gives
// us coverage of every early-return branch cheaply without needing a real
// orchestrator.
...
func TestHandleStacks_Dispatch(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:   "delete stack (not found returns 500)",
		...
		{
			name:   "start stack (not found returns 500)",
			method: http.MethodPost,
		...
		{
			name:   "stop stack (not found returns 500)",
			method: http.MethodPost,
