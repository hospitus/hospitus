...
func TestHandleStacks_Dispatch(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// 404, like every other named-stack route: 200 with an empty list
			// made an unknown stack indistinguishable from one that registers
			// no service.
			name:   "services on an unknown stack",
			method: http.MethodGet,
