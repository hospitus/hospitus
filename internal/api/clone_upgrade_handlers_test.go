...
func TestHandleCloneFromSnapshot_InvalidSnapshotName(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/instances/snapsrc3/snapshots/../etc/passwd/clone", bytes.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
	srv.mux.ServeHTTP(w, req)

	// A path-traversal snapshot name must never result in a successful clone;
	// reject any 2xx (not just 201), which is what the tautological "!= 201"
	// check previously allowed to slip through.
	if w.Code >= 200 && w.Code < 300 {
		t.Errorf("path-traversal snapshot name must not succeed, got %d: %s", w.Code, w.Body.String())
	}
}
