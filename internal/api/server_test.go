...
type mockProvider struct {
	// Guarded: setupTestServer registers one shared provider, and a handler
	// under test can create, list and delete concurrently — which is a
	// concurrent map write, and a panic rather than a failed assertion.
	mu        sync.Mutex
...
func (m *mockProvider) ListInstances(ctx context.Context, filter provider.InstanceFilter) ([]provider.InstanceHandle, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	handles := make([]provider.InstanceHandle, 0, len(m.instances))
...
func setupTestServer(t *testing.T) (*Server, *datastore.Datastore) {
	...
	// Closed here, not by each caller: most of them never did, and NewServer
	// also starts a backup manager, so every such test left an open SQLite
	// handle and a running manager for the rest of the package run. Close is
	// safe to call twice, so the callers that do defer it are unaffected.
	...
	// NewServer starts a job manager with worker goroutines. Callers that only
	// defer ds.Close() leaked them for the rest of the package run; Shutdown is
	// idempotent, so the callers that shut one down themselves are unaffected.
	t.Cleanup(func() { server.jobManager.Shutdown(2 * time.Second) })

	return server, ds
}

// setupTestServerWithConfig is setupTestServer for the tests that need a
// specific ServerConfig. They each built the datastore and the server inline
// and closed neither, so every one of them left an open SQLite handle and a
// running job manager behind for the rest of the package run.
func setupTestServerWithConfig(t *testing.T, cfg *ServerConfig) *Server {
	t.Helper()
	...
	registry := provider.NewRegistry()
	if err := registry.Register(newMockProvider()); err != nil {
		t.Fatalf("Failed to register mock provider: %v", err)
	}

	server, err := NewServer(":8080", ds, registry, cfg)
	if err != nil {
...
func TestCORSHeaders(t *testing.T) {
	...
	server := setupTestServerWithConfig(t, &ServerConfig{
		EnableAuth:     false,
		AllowNoAuth:    true, // unauthenticated on purpose: this exercises CORS/rate limiting, not auth
		AllowedOrigins: []string{"*"},
	})

	// A real preflight: the browser always sends the method it intends to use.
...
// TestClientErrorDetail covers the cause a 4xx carries. A 5xx never reaches
// this helper — see TestWriteLoggedErrorWithholdsInternalCauses.
...
// TestWriteLoggedErrorWithholdsInternalCauses covers whose fault the status
// says it is.
//
// A 4xx describes the request, and its cause is what lets the caller fix it. A
// 5xx describes the daemon, and its cause used to go out too: "sql: database
// is locked" and provider command lines reached an authenticated but
// narrowly-scoped caller, telling them how the server is built (CWE-209).
func TestWriteLoggedErrorWithholdsInternalCauses(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{
			name:       "a bad request keeps its cause",
			status:     http.StatusBadRequest,
			err:        errors.New("instance name contains a slash"),
			wantDetail: "instance name contains a slash",
		},
		{
			name:       "a conflict keeps its cause",
			status:     http.StatusConflict,
			err:        errors.New("the job is still running"),
			wantDetail: "the job is still running",
		},
		{
			name:       "an internal failure does not",
			status:     http.StatusInternalServerError,
			err:        errors.New("sql: no such table: instances"),
			wantDetail: "the daemon log has the cause",
		},
		{
			name:       "nor does a provider's command line",
			status:     http.StatusInternalServerError,
			err:        errors.New("exit status 125: --message is incompatible with --format oci"),
			wantDetail: "the daemon log has the cause",
		},
		{
			name:       "nor a gateway failure",
			status:     http.StatusServiceUnavailable,
			err:        errors.New("zfs: pool zroot is faulted"),
			wantDetail: "the daemon log has the cause",
		},
	} {
		t.Run(tt.name, func(t *testing.T) {
			w := httptest.NewRecorder()
			s.writeLoggedError(w, tt.status, "Operation failed", tt.err)
			...
			if tt.status >= http.StatusInternalServerError && strings.Contains(w.Body.String(), tt.err.Error()) {
				t.Errorf("the internal cause reached the client: %s", w.Body.String())
			}
		})
	}
}

// TestOptionsWithoutPreflightHeadersIsRouted covers an OPTIONS that is not a
// CORS preflight.
//
// corsMiddleware short-circuited every OPTIONS, so with no configured origin —
// or from a client that sends none, which is every non-browser — the answer was
// 403 and the routed OPTIONS handling was unreachable.
func TestOptionsWithoutPreflightHeadersIsRouted(t *testing.T) {
	server := setupTestServerWithConfig(t, &ServerConfig{
		EnableAuth:  false,
		AllowNoAuth: true,
	})
	...
	for _, tt := range []struct {
		...
		{"no Origin at all, as curl sends it", ""},
		{"an Origin but no requested method", "http://example.com"},
	} {
		t.Run(tt.name, func(t *testing.T) {
			req := httptest.NewRequest(http.MethodOptions, "/api/v1/providers", nil)
			if tt.origin != "" {
			...
			// It reaches the router, which serves GET here and says so.
			if w.Code != http.StatusMethodNotAllowed {
				t.Fatalf("status = %d, want 405 from the router; body=%s", w.Code, w.Body.String())
			}
...
// TestAcceptsMedia covers how the streaming handlers read Accept.
//
// strings.Contains answered yes to "text/plain;q=0, application/json", where
// the client is refusing text/plain outright, and missed "TEXT/PLAIN" because
// media ranges are case-insensitive.
func TestAcceptsMedia(t *testing.T) {
	for _, tt := range []struct {
		...
		{"application/json, text/plain;q=0.9", true},
		{"TEXT/PLAIN", true},
		{"  text/plain ; charset=utf-8 ", true},

		// The case that prompted this: the client says "not text/plain".
		{"text/plain;q=0, application/json", false},
		{"text/plain;q=0.0", false},
		{"application/json, text/plain;q=0", false},

		// A q it cannot read is not a q it may assume.
		{"text/plain;q=bogus", false},

		// A longer type that merely contains the wanted one must not match.
		{"text/plain-ish", false},
		{"application/text/plain", false},

		// A subtype wildcard asks for a text representation.
		{"text/*", true},
		{"text/*;q=0.5", true},
		{"text/*;q=0", false},
		{"application/*", false},

		// "*/*" is what curl sends and what no Accept header means: no
		// preference. Both must pick the same representation, so neither
		// selects streaming.
		{"*/*", false},
		{"application/json, */*", false},
	} {
		if got := acceptsMedia(tt.accept, "text/plain"); got != tt.want {
			t.Errorf("acceptsMedia(%q) = %v, want %v", tt.accept, got, tt.want)
		}
	}

	// The create handler accepts either of two types.
	if !acceptsMedia("text/event-stream", "text/plain", "text/event-stream") {
		t.Error("text/event-stream should be accepted when asked for")
	}
	if acceptsMedia("text/event-stream;q=0", "text/plain", "text/event-stream") {
		t.Error("a refused text/event-stream should not select streaming")
	}
}
