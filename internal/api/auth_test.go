...
//
// The per-IP auth rate limiter is configured with authRateLimiterBurst
// (see middleware.go). Those requests consume all tokens and receive 401;
// the next one gets 429.
func TestRateLimitAuthBurst(t *testing.T) {
	s := newAuthServer("valid-key")
	// Pre-exhaust the burst by injecting a limiter for the test IP. Taking the
	// burst from the constant keeps the test covering the real limit rather
	// than a literal that silently goes stale when the constant changes.
	// httptest.NewRequest sets RemoteAddr to "192.0.2.1:1234" by default.
	testIP := "192.0.2.1"
	limiter := rate.NewLimiter(rate.Limit(5.0/60.0), authRateLimiterBurst) // 5 req/min
	...
	// Exhaust every burst token with wrong-key requests (expect 401 each time).
	for i := 0; i < authRateLimiterBurst; i++ {
		req := httptest.NewRequest(http.MethodGet, "/api/v1/instances", nil)
		req.Header.Set("X-API-Key", "wrong-key")
		w := httptest.NewRecorder()
		handler.ServeHTTP(w, req)
	...
	// One more: no tokens remaining — must get 429 (rate limited).
	req := httptest.NewRequest(http.MethodGet, "/api/v1/instances", nil)
	req.Header.Set("X-API-Key", "wrong-key")
	w := httptest.NewRecorder()
	handler.ServeHTTP(w, req)
	...
	if w.Code != http.StatusTooManyRequests {
		t.Errorf("request %d: expected 429, got %d — body: %s", authRateLimiterBurst+1, w.Code, w.Body.String())
	}
}
...
func TestNoTLSRequiresFlag(t *testing.T) {
	...
	// NewServer starts job-manager workers; every accepted call leaks them
	// for the rest of the package run otherwise.
...
// TestAPIKeysWithoutEnableAuthIsRefused pins the footgun that shipped as a real
// auth bypass: API keys configured, EnableAuth left false, and the server
// served every request unauthenticated while looking configured.
//
// It is now a configuration error rather than a silent mode. Running without
// authentication takes AllowNoAuth, which someone has to write down.
func TestAPIKeysWithoutEnableAuthIsRefused(t *testing.T) {
	ds, err := datastore.NewDatastore(":memory:", nil)
	if err != nil {
		t.Fatalf("NewDatastore: %v", err)
	}
	...
	if _, err := NewServer(":0", ds, registry, &ServerConfig{
		EnableAuth:       false,
		AllowInsecureTLS: true,
		APIKeys:          []string{"present-but-ignored"},
	}); err == nil {
		t.Fatal("a config with keys and EnableAuth=false was accepted; it serves everything unauthenticated")
	}

	// Unauthenticated on purpose is still allowed, when it is said out loud.
	noAuth, err := NewServer(":0", ds, registry, &ServerConfig{
		EnableAuth:       false,
		AllowNoAuth:      true,
		AllowInsecureTLS: true,
	})
	if err != nil {
		t.Fatalf("explicit AllowNoAuth was refused: %v", err)
	}
	t.Cleanup(func() { noAuth.jobManager.Shutdown(2 * time.Second) })
	...
	t.Cleanup(func() { secure.jobManager.Shutdown(2 * time.Second) })
