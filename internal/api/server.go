...
type Datastore interface {
	...
	RenameInstance(ctx context.Context, oldName, newName string, handle provider.InstanceHandle) error
...
type Server struct {
	...
	cancelBackups    context.CancelFunc // Cancel function for the backup manager
	...
	// consoleSessions counts the slots claimed against maxConsoleSessions. It
	// tracks activeConsoleSessions, but as one atomic a caller can reserve
	// against — a count taken from the map is stale the moment it is read.
	consoleSessions atomic.Int32
}
...
func instanceDatasetResolver(ds Datastore) backup.DatasetResolver {
	return func(ctx context.Context, instanceID string) (string, error) {
		// NewServer accepts a nil Datastore. A request that panicked here would
		// be caught by the recovery middleware, but the backup manager runs its
		// schedule in a goroutine of its own, where the panic would take the
		// daemon with it.
		if ds == nil {
			return "", fmt.Errorf("no datastore is configured, so the storage of instance %s cannot be located", instanceID)
		}
		...
		for _, disk := range instance.Spec.Disks {
			...
			if i := strings.LastIndex(zvol, "/"); i > 0 {
				// Checked like the recorded handle above: ValidateDiskSpec
				// refuses "..", but nothing kept Path inside the pool this
				// daemon owns, so /dev/zvol/otherpool/critical/disk0 handed
				// "otherpool/critical" to zfs snapshot -r and zfs destroy.
				parent := zvol[:i]
				if err := ensureHospitusDataset(parent); err != nil {
					return "", err
				}
				return parent, nil
			}
...
//
// The rate limiter's defaults, named once so the nil-config branch above and
// the zero-value defaulting below cannot drift apart.
const (
...
func NewServer(addr string, ds Datastore, registry *provider.Registry, config *ServerConfig) (*Server, error) {
	...
	// The rate limits too, not only the timeouts. A caller passing a non-nil
	// ServerConfig with EnableRateLimit set and these left at zero built a
	// rate.NewLimiter(0, 0), which refuses every request with 429 — the daemon
	...
	// Running unauthenticated has to be asked for. EnableAuth is a bool, so
	// its zero value — a config that simply never mentions it — served every
	// endpoint to anyone who could reach the port. AllowNoAuth's own contract
	// already says it "must be explicitly set to true to run without
	// authentication"; this is what makes that true on both paths.
	//
	// Checked here, before the storage backend is built: Initialize runs
	// "zfs create -p" when the parent dataset is missing, so a configuration
	// this refuses had already created datasets on the host by the time the
	// error came back.
	if !config.EnableAuth && !config.AllowNoAuth {
		return nil, fmt.Errorf("EnableAuth is false: set AllowNoAuth to run without authentication, and mean it")
	}
	...
	s := &Server{
		...
		// The interface value, not concreteDS: a datastore that is not a
		// *datastore.Datastore leaves concreteDS nil, and a typed nil stored in
		// an interface is not nil — the resolver then called GetInstance on it
		// and dereferenced ds.db.
		backupManager: backup.NewBackupManager(config.DataDir, instanceDatasetResolver(ds), concreteDS),
	...
	// Start backup manager on a context this server owns, not
	// context.Background(): Shutdown had no way to end the watcher waiting on
	// it, so scheduled backups outlived the server that started them.
	backupCtx, cancelBackups := context.WithCancel(context.Background())
	s.cancelBackups = cancelBackups
	if s.backupManager != nil {
		if err := s.backupManager.Start(backupCtx); err != nil {
...
func (s *Server) registerRoutes() {
	...
	// No /vnet/enable or /vnet/disable: they were registered here, but
	// handleVnet routes on the method and never read the verb from the path,
	// so a POST to either of them enabled VNET. POST /vnet enables, DELETE
	// /vnet disables.
...
func (s *Server) Shutdown(ctx context.Context) error {
	...
	// Stop the backup manager: Stop() ends the schedules and aborts a running
	// backup, and canceling its context ends the watcher that Start left
	// waiting. Both are needed — neither alone stops everything.
	if s.backupManager != nil {
		s.backupManager.Stop()
	}
	if s.cancelBackups != nil {
		s.cancelBackups()
	}
...
func (s *Server) handleHealth(w http.ResponseWriter, r *http.Request) {
	...
	//
	// NewServer accepts a nil Datastore, and /health is registered either way:
	// dereferencing it here let the recovery middleware answer 500 for the one
	// endpoint whose job is to report unhealthy as 503.
	ctx := r.Context()
	if s.datastore == nil {
...
func (s *Server) SyncInstanceStates() {
	// Start calls this before the listener opens, so a nil datastore panicked
	// here — before /health existed to report the trouble as 503.
	if s.datastore == nil {
		s.logger.Warn("Skipping instance state sync: no datastore configured")
		return
	}
...
// acceptsMedia reports whether an Accept header asks for one of these media
// types and has not refused it.
//
// strings.Contains answered yes to "text/plain;q=0, application/json", where
// the client is saying precisely the opposite: RFC 9110 gives q=0 the meaning
// "not acceptable". Media ranges are also case-insensitive, which a substring
// search on the lowercase spelling missed.
func acceptsMedia(accept string, wanted ...string) bool {
	for _, part := range strings.Split(accept, ",") {
		...
		for _, w := range wanted {
			...
			// "text/*" asks for a text representation, so it selects one.
			//
			// "*/*" does not: it says the client has no preference, which is
			// what curl sends by default and what an absent Accept header
			// means. Those two must not diverge — an absent header picks the
			// JSON representation, so "*/*" picks it too.
			if kind, _, ok := strings.Cut(w, "/"); ok && media == kind+"/*" {
				matched = true
				break
			}
		}
...
// mediaQuality reads the q parameter of one media range. Absent means 1;
// unparseable means the range is unusable, so 0.
func mediaQuality(params []string) float64 {
	for _, p := range params {
		p = strings.TrimSpace(p)
		if !strings.HasPrefix(strings.ToLower(p), "q=") {
			continue
		}
		q, err := strconv.ParseFloat(strings.TrimSpace(p[len("q="):]), 64)
		if err != nil {
			return 0
		}
		return q
...
// writeMethodNotAllowed answers 405 and names the methods the resource serves.
//
// RFC 9110 requires the Allow header on every 405; most of this package sent
// the status bare, so a client was told its method was wrong and never which
// one to use.
...
// maxRequestBody bounds a decoded JSON body. Every request this daemon takes
// is a small document; nothing legitimate approaches this.
const maxRequestBody = 4 << 20 // 4 MiB

// validPort reports whether a port number is one a host can actually bind.
func validPort(port int) bool {
	return port >= 1 && port <= 65535
}

// writeLoggedError logs the error server-side and answers the client with a
// stable message plus, for a client error, its cause.
//
// The two fields serve different readers. "error" is a fixed sentence a client
// can match on; "detail" carries the cause a caller can act on: an image that
// does not exist, an argument QEMU rejected, a name that is not valid.
//
// What the detail may say depends on whose fault the status says it is. A 4xx
// describes the request, and the cause is the whole point — a bare "Invalid
// request body" only sends the caller guessing. A 5xx describes the daemon,
// and its cause is the daemon's internals: "no such table: instances" or a
// provider's command line told an authenticated but narrowly-scoped caller how
// the server is built, for no benefit to them (CWE-209). Those go to the log,
// which keeps them whole, and the client is told where to look.
//
// The client gets one bounded line, because the JSON field and the streaming
// protocol are both line-oriented.
func (s *Server) writeLoggedError(w http.ResponseWriter, statusCode int, clientMsg string, err error) {
	s.logger.Error(clientMsg, logging.FieldError, err, "status", statusCode)
	...
	if statusCode >= http.StatusInternalServerError {
		...
		s.writeJSON(w, statusCode, map[string]string{
			"error":  clientMsg,
			"detail": "the daemon log has the cause",
		})
		return
	}
...
// clientErrorDetail returns the cause of err as it may be shown to the caller
// that provoked it, or an empty string when there is nothing to add.
//
// Only reached for a 4xx: writeLoggedError answers a 5xx without it.
...
func (s *Server) decodeJSONBody(w http.ResponseWriter, r *http.Request, v interface{}) error {
	...
	// Bounded: json.Decoder reads until EOF, so an unauthenticated caller
	// could stream an arbitrarily large body and hold the memory for it.
	// The writer, not nil: MaxBytesReader uses it to mark the connection so
	// the server stops reading and closes it after an oversized body, instead
	// of leaving the rest of the request in flight.
	...
	// And nothing after it. Decode stops at the end of the first value, so
	// a body of two concatenated objects was accepted and the second silently
	// ignored — which makes what the server acted on differ from what the
	// client believes it sent.
	if err := decoder.Decode(&struct{}{}); !errors.Is(err, io.EOF) {
		return fmt.Errorf("unexpected content after the JSON body")
	}
	return nil
}
