...
func (s *Server) handleExport(w http.ResponseWriter, r *http.Request, instanceID string) {
	...
	// lookupInstance, not GetInstance: every sibling route resolves this same
	// path segment by id or name, and a name answered 404 here.
...
func (s *Server) handleImport(w http.ResponseWriter, r *http.Request) {
	...
	// 400, not 500: this provider name comes from the request body, unlike
	// handleExport where it comes from the stored instance.
	prov, err := s.registry.Get(req.Provider)
	if err != nil {
		s.writeLoggedError(w, http.StatusBadRequest, "Unknown provider", err)
		return
	}
	...
	// Validated here, not at the next start: ImportInstance writes NewIP into
	// the jail's configuration, and a malformed one is only refused when the
	// jail tries to come up — so the import "succeeded" and left behind an
	// instance that cannot start.
	if req.NewIP != "" {
		if err := validation.ValidateIPAddress(req.NewIP); err != nil {
			s.writeLoggedError(w, http.StatusBadRequest, "Invalid new_ip", err)
			return
		}
	}
	...
	// Detached: the import has already produced the instance, so a client that
	// hung up must not turn this write into a failure that discards it.
	persist, cancelPersist := context.WithTimeout(context.WithoutCancel(r.Context()), time.Minute)
	defer cancelPersist()
	if err := s.datastore.CreateInstance(persist, instance); err != nil {
		// persist, not r.Context(): discardCreatedInstance detaches internally
		// either way, so this changes nothing at runtime — it just puts the
		// detachment where a reader of this line can see it.
		err = s.discardCreatedInstance(persist, prov, handle, err)
...
func validateUserFilePath(p, baseDir string) error {
	...
	// Reject paths that, even after cleaning, still carry a traversal segment.
	// Tested per segment rather than with a substring search: "web..tar" and
	// "my..backup/x.tar" contain ".." and traverse nothing, and refusing them
	// turned a legal filename into an error the caller could not explain.
