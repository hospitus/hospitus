...
func TestHandleGetJob_NotFound(t *testing.T) {
	...
	// 404 exactly. Accepting 400 as well let a change in the response contract
	// pass unnoticed: an unknown but well-formed id is a missing job, not a
	// malformed request.
...
func TestHandleGetJobRaceFree(t *testing.T) {
	...
	// The worker loop exits only when release closes, so every path out of this
	// test has to close it: a t.Fatalf below used to skip the close, leaving
	// the worker spinning and the cleanup's Shutdown waiting out its timeout.
...
func TestHandleListJobs_WithStatusFilter(t *testing.T) {
	...
	// One finished job and one still running: the status alone would pass for
	// a handler that ignores the filter entirely, which is exactly the merge
	// and re-filter step this endpoint exists for.
	done, err := srv.submitJob(context.Background(), "test", "finished", nil,
		func(context.Context, *job.Job) error { return nil })
	if err != nil {
		t.Fatalf("submit finished job: %v", err)
	}
	<-done.Done()
	...
	defer releaseWorker()
	running := make(chan struct{})
	busy, err := srv.submitJob(context.Background(), "test", "still going", nil,
		func(context.Context, *job.Job) error {
			close(running)
			<-release
			return nil
		})
	...
	var listed struct {
		Jobs []job.JobRecord `json:"jobs"`
	}
	if err := json.Unmarshal(w.Body.Bytes(), &listed); err != nil {
		t.Fatalf("decode listing: %v", err)
	}

	ids := make(map[string]job.JobStatus, len(listed.Jobs))
	for _, rec := range listed.Jobs {
		ids[rec.ID] = rec.Status
	}
	if _, ok := ids[busy.ID]; ok {
		t.Errorf("the running job %s was listed under ?status=completed", busy.ID)
	}
	if got := ids[done.ID]; got != job.JobStatusCompleted {
		t.Errorf("completed job %s listed as %q, want completed", done.ID, got)
	}
}
