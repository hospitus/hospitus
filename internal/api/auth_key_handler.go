...
// maxAPIKeyTTLDays bounds ttl_days. time.Duration counts nanoseconds in an
// int64, so a TTL past roughly 106,751 days overflows the multiplication below
// and yields an expiry in the past. A century is well clear of that and well
// past any real key's life.
const maxAPIKeyTTLDays = 36500
...
func (s *Server) handleAuthKeys(w http.ResponseWriter, r *http.Request, keyID string) {
	// "admin", not the literal "*": requiredPermission already asks for "admin"
	// on /api/v1/auth, and hasPerm treats "*" as a wildcard over it. Requiring
	// the wildcard string here meant a key scoped to ["admin"] cleared the
	// middleware and was then refused by the handler — the grant named nothing
	// any key could actually use.
...
func (s *Server) handleCreateAuthKey(w http.ResponseWriter, r *http.Request) {
	...
	// Rejected rather than ignored: a negative ttl_days took the same path as
	// 0 and quietly handed back a key that never expires.
	if req.TTLDays < 0 || req.TTLDays > maxAPIKeyTTLDays {
		s.writeError(w, http.StatusBadRequest,
			fmt.Sprintf("ttl_days must be between 0 and %d", maxAPIKeyTTLDays))
		return
	}
