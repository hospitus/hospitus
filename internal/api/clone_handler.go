...
func (s *Server) handleCloneFromInstance(w http.ResponseWriter, r *http.Request, instanceID string, instance *datastore.Instance, cloneProvider provider.CloneProvider) {
	...
	// Detached: the provider has already made the clone, so a client that hung
	// up must not turn this write into a failure that destroys it.
	persist, cancelPersist := context.WithTimeout(context.WithoutCancel(r.Context()), time.Minute)
	defer cancelPersist()
	if err := s.datastore.CreateInstance(persist, cloneInstance); err != nil {
		...
		if dp, ok := cloneProvider.(provider.Provider); ok {
			err = s.discardCreatedInstance(persist, dp, cloneHandle, err)
		}
