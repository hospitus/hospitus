...
func (c *Client) ExecCommandStream(ctx context.Context, instanceID string, req ExecRequest, stdout, stderr io.Writer) (int, error) {
	...
	exitCode := 0
	// The server ends the stream with "EXIT_CODE: n". Without that line the
	// command's outcome is unknown, and reporting the initial 0 told the
	// caller it succeeded — a connection dropped mid-command read as success.
	sawExitCode := false
	var lineBuffer bytes.Buffer

	// Allocated once. Inside the loop this made a fresh 4 KiB slice for every
	// read of a stream that can run for the length of a package install.
		...
		if n > 0 {
			...
			for {
				...
				if strings.HasPrefix(line, "EXIT_CODE: ") {
					...
					if parseErr != nil {
						// A marker without a number is not an outcome: counting
						// it as one reported success for a stream that never
						// said how the command ended.
						return -1, fmt.Errorf("the command reported a malformed exit code %q", codeStr)
					}
					exitCode = code
					sawExitCode = true
					continue
			...
			if errors.Is(err, io.EOF) || errors.Is(err, io.ErrUnexpectedEOF) || strings.Contains(err.Error(), "EOF") {
				...
				if remaining != "" {
					...
					case strings.HasPrefix(remaining, "EXIT_CODE: "):
						codeStr := strings.TrimSpace(strings.TrimPrefix(remaining, "EXIT_CODE: "))
						code, parseErr := strconv.Atoi(codeStr)
						if parseErr != nil {
							// Same rule as the in-loop parse above: a marker
							// carrying no number is not an outcome.
							return -1, fmt.Errorf("the command reported a malformed exit code %q", codeStr)
						}
						exitCode = code
						sawExitCode = true
	...
	if !sawExitCode {
		return -1, fmt.Errorf("the command's output ended before its exit code; its outcome is unknown")
	}
	return exitCode, nil
}
