...
func (c *Client) FetchImage(ctx context.Context, version string, w io.Writer) error {
	...
	// Set when a terminal marker actually arrives. Reading the last line
	// instead meant a stream that ended with a trailing blank line, or with
	// anything after the marker, was reported as an incomplete download.
	var completed bool
	var hasProgress bool

	for scanner.Scan() {
		line := scanner.Text()
		// The protocol prefixes, not the words anywhere: a progress line
		// reading "…COMPLETE 45%" marked the download finished.
		if strings.HasPrefix(line, "SUCCESS:") || strings.HasPrefix(line, "COMPLETE:") {
			completed = true
		}
