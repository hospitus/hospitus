...
func TestCreateVolume(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// Method and path asserted, as TestUnexposePort does: a client that
		// sends a GET, or reaches the wrong endpoint, still decoded this body.
		if r.Method != http.MethodPost {
			t.Errorf("expected POST, got %s", r.Method)
		}
		if r.URL.Path != "/api/v1/volumes" {
			t.Errorf("path = %q, want /api/v1/volumes", r.URL.Path)
		}
...
func TestGetVolume(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Path != "/api/v1/volumes/v1" {
			t.Errorf("path = %q, want /api/v1/volumes/v1", r.URL.Path)
		}
