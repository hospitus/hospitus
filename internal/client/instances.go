...
func (c *Client) StartInstance(ctx context.Context, id string, w io.Writer) error {
	...
	if resp.StatusCode >= 400 {
		// Through APIError like every other path: re-deriving the message here
		// dropped the status-specific guidance, so a 401 on the fallback said
		// "API error (401)" instead of naming the API key as the thing to check.
		return APIError(resp.StatusCode, readErrorBody(resp.Body))
	}
