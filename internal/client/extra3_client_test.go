...
func TestGetTimeoutFromEnv(t *testing.T) {
	tests := []struct {
		...
		{
			// ParseDuration accepts it. Honoring it disables the timeout
			// entirely, so a hung daemon hangs the CLI forever.
			name:     "zero is not a timeout",
			envValue: "0s",
			want:     DefaultTimeout,
		},
		{
			// Honoring this makes every request fail before it is sent.
			name:     "negative is not a timeout",
			envValue: "-30s",
			want:     DefaultTimeout,
		},
		{
			name:     "negative integer seconds",
			envValue: "-60",
			want:     DefaultTimeout,
		},
	}
