...
type InstanceBackupConfig struct {
	...
	// MinAge is a Go duration ("6h"). It is the only guard that keeps recent
	// backups from being pruned, so leaving it out made retention harsher after
	// every restart, silently.
...
func (ds *Datastore) UpdateBackupConfig(ctx context.Context, id string, config *InstanceBackupConfig) error {
	// A nil config marshals to "null", and scanInstance allocates the struct
	// before unmarshalling, so "null" comes back as a non-nil zero value —
	// "no backup configuration" would read as "a configuration that is empty".
	// CreateInstance stores "{}" for the same reason.
	backupJSON := []byte("{}")
	if config != nil {
...
// Cascade delete also removes the instance's events.
//
// No deletion event is written here: events cascade with the instance, so the
// row would be inserted and removed inside this call and the audit trail would
// never hold it. The deletion is reported to the logger instead; a durable
// record needs an events table that outlives its instance.
func (ds *Datastore) DeleteInstance(ctx context.Context, id string) error {
	ds.logger.Info("Deleting instance", "instance_id", id)
...
func (ds *Datastore) ListInstances(ctx context.Context, filter InstanceFilter) ([]*Instance, error) {
	...
	if filter.Image != "" {
		// ESCAPE has to be declared: SQLite's LIKE has no default escape
		// character, so without it the backslashes below stay literal pattern
		// characters and a filter like "ubuntu_22" matches nothing at all.
		query += " AND image LIKE ? ESCAPE '\\'"
