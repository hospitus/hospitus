...
func TestCleanupOldJobs(t *testing.T) {
	...
	// new-job (pending) should still exist. GetJob answers (nil, nil) for a
	// missing job, so checking only the error would pass on a job the sweep
	// had deleted.
	kept, err := ds.GetJob(ctx, "new-job")
	if err != nil {
		t.Errorf("new-job should still exist: %v", err)
	} else if kept == nil {
		t.Error("the sweep deleted the pending job new-job")
	}
}

// TestCleanupOldJobsRemovesCanceledJobs covers a terminal status the sweep
// skipped: the query used the British spelling while the job package writes
// "canceled", so canceled jobs accumulated for ever.
