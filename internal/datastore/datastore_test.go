...
func TestRenameInstance(t *testing.T) {
	...
	// The spec carries the name too, and it used to stay behind: the row said
	// "rename-new" while its serialized spec still said "rename-old".
	if newInst.Spec.Name != "rename-new" {
		t.Errorf("spec name = %q, want rename-new", newInst.Spec.Name)
	}
