...
// hasExecutableSQL reports whether a migration body contains a statement.
//
// Testing for an empty string is not enough: an irreversible migration is
// written as a comment explaining why (SQLite cannot drop a column), and that
// comment executes as a no-op. The version row would then be deleted while the
// schema still carries the change, so the next Migrate replays an ALTER TABLE
// that fails on a column already there — and the database stays stuck.
...
func (m *Migrator) rollbackMigration(ctx context.Context, migration Migration) error {
	...
	if !hasExecutableSQL(migration.Down) {
		return fmt.Errorf("migration %d is irreversible: its down migration carries no statement, "+
			"and rolling it back would delete the version row while leaving the schema in place",
			migration.Version)
	}
