...
func NewAuditLogger(config AuditLoggerConfig) (*AuditLogger, error) {
	...
	// The mode above applies only when the file is created. A log left behind
	// by an earlier install keeps whatever permissions it had, so tighten it
	// here or the guarantee the comment states is only true on a fresh host.
	if info, statErr := file.Stat(); statErr == nil && info.Mode().Perm()&0o077 != 0 {
		if chmodErr := file.Chmod(0o600); chmodErr != nil {
			_ = file.Close()
			return nil, fmt.Errorf("audit log %s is readable by group or others (%04o) and cannot be tightened: %w",
				config.LogFile, info.Mode().Perm(), chmodErr)
		}
	}
...
func (l *AuditLogger) rotateIfNeeded() error {
	...
	rotatedName, nameErr := uniqueRotationName(l.config.LogFile, time.Now())
	if nameErr != nil {
		return l.reopenAfterRotationFailure(nameErr)
	}
...
// uniqueRotationName picks a name no rotated log already occupies.
//
// The timestamp has one-second resolution, so two rotations inside the same
// second would rename onto the same target and os.Rename replaces it silently:
// the older file, and every event in it, is gone.
func uniqueRotationName(logFile string, now time.Time) (string, error) {
	base := fmt.Sprintf("%s.%s", logFile, now.Format("2006-01-02-15-04-05"))
	candidate := base
	for i := 1; ; i++ {
		...
		if i > 1000 {
			return "", fmt.Errorf("no free rotated-log name for %s", base)
		}
...
func (l *AuditLogger) Close() error {
	...
	// A second Close would close l.done again and panic; the file descriptor
	// must not be closed twice either.
	var err error
	l.closeOnce.Do(func() {
		if l.config.AsyncWrite {
			close(l.done)
...
func (l *AuditLogger) Query(query AuditLogQuery) ([]*AuditEvent, error) {
	...
	for {
		...
		if err := decoder.Decode(&event); err == io.EOF {
			...
			// json.Decoder does not resynchronise after a syntax error: the next
			// Decode returns the same error at the same position, so "skipping"
			// the record spins here for ever. Stop and report what was read.
			return events, fmt.Errorf("audit log %s is malformed after %d events: %w",
				l.config.LogFile, len(events), err)
		}
